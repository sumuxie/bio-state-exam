...

{
  ...
  mustKnow: { en: "Thymine, not uracil, is the difference that actually separates DNA from RNA in the table of nomenclature — the sugar differs too, but the base rule is the cleaner test: uracil almost only in RNA, thymine almost only in DNA. And 'nucleoside' versus 'nucleotide' is not two names for the same thing: a nucleoside is base+sugar, a nucleotide adds the phosphate — the whole vocabulary of the chapter rests on keeping that one word apart.", cn: "胸腺嘧啶而不是尿嘧啶，才是命名表里真正把 DNA 和 RNA 分开的那个差异——糖也不一样，但碱基这条规则更干净：尿嘧啶几乎只出现在 RNA 里，胸腺嘧啶几乎只出现在 DNA 里。而『核苷』和『核苷酸』不是同一个东西的两个名字：核苷是碱基+糖，核苷酸多了一个磷酸——这一章的全部词汇，都建立在把这一个词分清楚的基础上。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "核苷酸的降解及核苷酸代谢 (笔记「二十七」？仅凭标题推测，未直接核对页码)", status: "partial" },
  mustKnow: { en: "Pyrimidine synthesis runs the opposite order from purine synthesis: the ring is built essentially COMPLETE first, and only afterward attached to PRPP — exactly backwards from the purine route, where the ring grows piece by piece already sitting on the sugar. And CTP inhibiting its own pathway's very first committed enzyme (aspartate transcarbamoylase) is the same feedback logic that shows up everywhere else in metabolism: the end product reaches back to shut off the beginning.", cn: "嘧啶的合成顺序和嘌呤正好相反：环几乎是先**整个造好**，之后才接到 PRPP 上——恰恰和嘌呤反过来，嘌呤是已经坐在糖上之后才一块一块地长环。而 CTP 抑制它自己这条通路最开头那个关键酶（天冬氨酸氨甲酰转移酶），正是代谢里到处都能看到的那套反馈逻辑：终产物反过来关掉起点。" },
  summary: {
    ...
    cn: "与嘌呤环——直接在核糖骨架上逐块搭建——不同，嘧啶环基本上是先合成完整、之后才与 PRPP 连接的。所需的两个氮原子都在环连接之前提供：一个来自氨甲酰磷酸，一个来自天冬氨酸。合成途径为 UMP → UTP → CTP，胸腺嘧啶则通过自己的一条路线进入 DNA；第一个定向步骤（committed step）受 CTP 反馈抑制，而为脱氧核糖核苷酸供电子的 NADPH／硫氧还蛋白系统对嘌呤和嘧啶两条系列都通用。"
  },
  points: [
  ...
  terms: [
    ...
    { cz: "aspartáttranskarbamoylasa", en: "aspartate transcarbamoylase (ATCase)", cn: "天冬氨酸转氨甲酰酶", def_en: "The allosteric enzyme catalysing the first committed step (carbamoyl-P + aspartate); feedback-inhibited by CTP.", def_cn: "催化第一个定向步骤（committed step：氨甲酰磷酸+天冬氨酸）的别构酶；受 CTP 反馈抑制。" },
...

{
  ...
  lehNotes: [
    { kind: "conflict",
      node: "L-8-2-1",
      en: "WHAT STABILISES THE HELIX — this node has the ranking the wrong way round. It says the double helix is partly stabilised by hydrogen bonds between the bases, then lists stacking and electrostatics as further contributions. Lehninger states the opposite explicitly: “the hydrogen bonds do not contribute significantly to the stability of the structure” (A p.271). What the hydrogen bonds supply is complementarity — they decide WHICH base pairs with which. The helix is instead “primarily stabilized by metal cations, which shield the negative charges of backbone phosphates”, together with base stacking between successive pairs. The observation that settles it: stacking is stronger between successive GC pairs than AT pairs, so GC-rich DNA is more stable — and note that counting hydrogen bonds, three versus two, predicts the same thing, which is why the wrong explanation survives so easily.",
      cn: "是什么在稳定双螺旋——本节点把主次说反了。它说双螺旋部分地由碱基之间的氢键稳定，然后才把堆积和静电作用列为进一步的贡献。Lehninger 明确指出相反的结论：「氢键对该结构的稳定性并无显著贡献」（A p.271）。氢键提供的是互补性——它们决定哪个碱基与哪个配对。螺旋真正「主要由金属阳离子稳定，它们屏蔽了骨架磷酸的负电荷」，并与相邻碱基对之间的堆积共同作用。能一锤定音的观察是：相邻 GC 对之间的堆积强于 AT 对，所以富含 GC 的 DNA 更稳定——而请注意，数氢键（三个对两个）也能预测出同样的结果，这正是那个错误解释如此容易存活下来的原因。" },
    { kind: "conflict",
      node: "L-8-2-1",
      en: "A-DNA AND Z-DNA IN CELLS — this node overstates one and understates the other. It presents the B-to-A conversion at 75% relative water content as something DNA does, and dismisses Z-DNA as having a biological function not yet known. Lehninger's position is the other way round on both. On the A form: “Whether A-DNA occurs in cells is uncertain” (A p.273), and there is a methodological reason to be careful — the reagents used to promote crystallisation of DNA tend to dehydrate it, so most short DNA crystallises in the A form whether or not that is its cellular state. On the Z form: short Z-DNA tracts have been found in BOTH bacteria and eukaryotes, and may play a role, as yet undefined, “in regulating the expression of some genes or in genetic recombination”. So Z-DNA is the one with demonstrated cellular occurrence.",
...

{
  ...
  lehNotes: [
    { kind: "conflict", node: "L-25-1-1",
      en: "The count is out of date, and so is the sentence about in vivo function. This node says E. coli has three DNA polymerases (I, II, III) and that an in vivo function is known only for polymerase III. Lehninger 8's Table 25-1 (A p.919) compares FIVE, under the subheading “E. coli Has at Least Five DNA Polymerases”: polymerase IV (gene dinB) and polymerase V (gene umuC) were identified in 1999. Both are translesion, mutagenic polymerases that copy past template damage which would stall a replicative polymerase — and they pay for it, being the only two of the five with NO 3'→5' proofreading exonuclease, with a processivity of 1 and 6–8 respectively against polymerase III's 500,000 or more. Lehninger also assigns an in vivo role to each of the five, not only to III: I does cleanup during replication, recombination and repair, II acts in one type of DNA repair, and IV and V in an unusual form of repair. Everything this node says about polymerases I, II and III is otherwise correct and is not affected.",
      cn: "数目已经过时，「体内功能」那句话也过时了。本节点说大肠杆菌有三种DNA聚合酶（I、II、III），并说只有聚合酶III的体内功能是明确的。Lehninger 8 的表25-1（A 第919页）比较的是五种，小标题就叫「大肠杆菌至少有五种DNA聚合酶」：聚合酶IV（基因 dinB）与聚合酶V（基因 umuC）于1999年被鉴定出来。两者都是跨损伤（translesion）、致突变型聚合酶，能越过会让复制型聚合酶停摆的模板损伤——而它们为此付出了代价：它们是五种里仅有的两个不带3′→5′校对外切核酸酶的，持续合成能力分别只有1和6–8，而聚合酶III在500,000以上。此外，Lehninger给五种中的每一种都指派了体内功能，而不只是给III：I在复制、重组和修复中做收尾工作，II参与某一类DNA修复，IV和V参与一种特殊形式的修复。除此之外，本节点关于聚合酶I、II、III的其余内容都是正确的，不受影响。" },
  ],
  mustKnow: { en: "DNA polymerase I is really three enzymes fused into one protein: a polymerase that only extends an existing 3'-OH end, a proofreading exonuclease that trims mismatches from that same end, and a separate nuclease that cuts INSIDE the chain to remove RNA primers and repair damage — and only the last of these is missing from the Klenow fragment, which is why Klenow is still useful for polymerizing but useless for primer removal. And of the three E. coli polymerases, only Pol III is actually the replicase doing the bulk of the work; Pol I is a cleanup enzyme, despite being the one first discovered and named.", cn: "DNA聚合酶I其实是三个酶融合成一个蛋白质：一个只能从已有的 3'-OH 端往下延伸的聚合酶活性、一个从同一个端点校对并剪掉错配碱基的外切酶活性，以及一个完全独立的、专门在链**内部**切开来去除RNA引物和修复损伤的核酸酶活性——而克列诺片段恰恰缺了最后这一个，所以它还能用来聚合，却没法去除引物。而大肠杆菌的三种聚合酶里，真正干大部分复制工作的其实只有Pol III；Pol I只是个清理用的酶，尽管它是最早被发现和命名的那一个。" },
  summary: {
...

{
  ...
  mustKnow: { en: "DNA is never itself the direct template for protein — that step always goes through RNA first, which is the whole content of the central dogma reduced to one sentence. And the different RNA classes are not variations on a theme, they are built for entirely different jobs: mRNA carries the message and is deliberately short-lived and heterogeneous, rRNA is structural and makes up the bulk of a cell's RNA by mass, and tRNA is small enough to diffuse freely precisely because its job is to physically ferry an amino acid across the cytoplasm.", cn: "DNA从来不会直接充当蛋白质合成的模板——这一步永远要先经过 RNA，这正是中心法则压缩成一句话的全部内容。而不同种类的 RNA 不是同一个主题的变体，它们是为完全不同的工作而造的：mRNA 携带信息，天生就是短命、大小不一的；rRNA 是结构性的，按质量算占了细胞里 RNA 的大头；tRNA 之所以小到能自由扩散，正是因为它的工作就是在细胞质里把一个氨基酸physically运过去。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Transcription needs no primer at all, unlike replication — RNA polymerase can simply start a chain from scratch, because unlike DNA polymerase it does not require a pre-existing 3'-OH to extend. And a hairpin terminator only works because the two features it needs point the same way: a G+C-rich stem that folds tightly enough to stall the polymerase, immediately followed by a run of U's that pair only weakly with the DNA template — strong fold, weak grip, and the RNA simply falls off.", cn: "转录完全不需要引物，这一点和复制不一样——RNA聚合酶可以直接从零开始造一条链，因为和DNA聚合酶不同，它不需要一个已经存在的 3'-OH 端才能往下延伸。而发夹式终止子之所以有效，正是因为它需要的两个特征方向一致：一段富含 G-C、能折叠得足够紧、让聚合酶卡住的茎，紧接着一串和DNA模板配对很弱的U——折叠得紧、抓得松，RNA 就这么脱落下来了。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "核苷酸的降解及核苷酸代谢 (笔记「二十七」？仅凭标题推测，未直接核对页码)", status: "partial" },
  mustKnow: { en: "Nucleic acid breakdown ends up feeding two completely different metabolic systems: the sugar-phosphate half rejoins carbohydrate metabolism directly, while the two base classes go their separate ways entirely — purines all the way down to uric acid and then urea, pyrimidines to beta-alanine or 3-aminobutyric acid. And uric acid is not just a chemical way-station: its own poor solubility is exactly what makes it accumulate as crystals in gout, the same physical property that makes it useful for storage in a uricotelic organism's egg.", cn: "核酸降解的产物最终会喂进两套完全不同的代谢系统：糖磷酸那一半直接重新加入糖类代谢，而两类碱基则走完全不同的路——嘌呤一路降解到尿酸、再到尿素；嘧啶降解成β-丙氨酸或3-氨基丁酸。而尿酸不只是一个化学上的中转站：它自身糟糕的溶解性，正是它在痛风里以晶体形式沉积的原因——而同样这个物理性质，也正是它在卵生动物的蛋里适合用来储存的原因。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Twenty amino acids cannot be spelled with a one- or two-letter code — 4^1 and 4^2 both fall short — so the genetic code has to use triplets, and 4^3=64 gives far more combinations than 20 amino acids need, which is exactly why the code is degenerate rather than one codon mapping to each amino acid. And 'universal' has exactly one book-noted exception: UGA, normally a stop codon, is read as selenocysteine when a specific mRNA element (SECIS) is present — the same triplet, two different meanings, decided by context elsewhere on the message.", cn: "二十种氨基酸没法用一个或两个字母的密码来拼——4的1次方和4的2次方都不够用——所以遗传密码必须用三联体，而 4的3次方=64 远远超过二十种氨基酸所需要的数量，这正是为什么密码是简并的，而不是一个密码子对应一个氨基酸。而『通用』这个说法有教材明确指出的一个例外：UGA 通常是终止密码子，但当 mRNA 上存在一个特定元件（SECIS）时，它被读作硒代半胱氨酸——同一个三联体，两种不同的含义，由信息分子别处的上下文决定。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "mRNA翻译产生蛋白质 (笔记无编号，见 pp.223–224)", status: "mapped" },
  mustKnow: { en: "A ribosome's sedimentation value does not simply add: the 30S and 50S subunits together give a 70S particle, not 80S, because S values track shape and density along with mass, not mass alone. And polysomes exist because a ribosome is released the moment it clears the initiation region, freeing that stretch of mRNA to start a fresh ribosome behind it — so translating a single message many times over is simply a matter of not waiting for the first ribosome to finish.", cn: "核糖体的沉降值不是简单相加：30S 和 50S 亚基合起来是 70S，不是 80S，因为 S 值同时跟着形状和密度走、不只是跟着质量走。而多聚核糖体之所以存在，是因为一个核糖体一旦越过起始区就会被释放出来，让那段 mRNA 空出来去启动后面一个新的核糖体——所以一条信息被反复翻译许多遍，说到底只是不用等第一个核糖体做完而已。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "mRNA翻译产生蛋白质 (笔记无编号，见 pp.223–224)", status: "mapped" },
  mustKnow: { en: "An amino acid must be spent energetically before it can join a growing chain — activation to aminoacyl-tRNA is not bookkeeping, it is what makes peptide-bond formation downhill enough to actually happen. And from the moment that bond to tRNA forms, the amino acid is recognised ONLY by its anticodon, never again by its own chemical structure — which is exactly why a tRNA charged with the wrong amino acid gets incorporated at the position its anticodon specifies, not the position its cargo would suggest.", cn: "一个氨基酸必须先在能量上被『花费』，才能加入正在生长的链——活化成氨酰-tRNA不是记账式的手续，它正是让肽键形成变得足够顺坡、真正能发生的原因。而从这个键连到 tRNA 上的那一刻起，氨基酸就**只**由它的反密码子来识别，再也不看它自己的化学结构——这正是为什么一个装错了氨基酸的 tRNA，会被安到它反密码子所指定的位置上，而不是它所携带的那个氨基酸『应该』去的位置。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "mRNA翻译产生蛋白质 (笔记无编号，见 pp.223–224)", status: "mapped" },
  mustKnow: { en: "Termination is chemically a substitution, not a new mechanism: a release factor sitting in the empty A-site redirects the peptidyl-transferase reaction from its usual target (an incoming aminoacyl-tRNA) onto plain water instead, which is the one concrete difference that turns 'add another amino acid' into 'release the finished chain.' And eukaryotes economise where bacteria specialise: one release factor recognising all three stop codons, instead of the two-plus-one division of labour (RF-1/RF-2/RF-3) bacteria use.", cn: "终止在化学上是一次『替换』，不是全新的机制：释放因子占据空着的 A 位点，把肽基转移酶反应的目标从平常那个（进来的氨酰-tRNA）改成普通的水——正是这一个具体的差别，把『再加一个氨基酸』变成了『把做好的链放出来』。而真核生物在细菌讲究分工的地方选择了省事：一个释放因子就能识别全部三个终止密码子，而不是像细菌那样用RF-1/RF-2/RF-3三个因子分工。" },
  summary: {
...

{
  ...
  lehNotes: [
    { kind: "gap",
      node: "L-26-2-1",
      en: "The claim that an invariant GU at the intron's 5' edge and an invariant AG at its 3' edge are NECESSARY AND SUFFICIENT to define a splice site is stronger than the chemistry actually supports. Splicing proceeds by two transesterification steps whose real nucleophile is an internal branch-point adenosine deep inside the intron, not the GU/AG boundary itself — GU and AG mark a site a recognition step reads, they are not what reacts. And roughly 1% of human introns are removed by a separate, minor spliceosome using AU...AC boundaries instead of GU...AG, so the dinucleotide is not even universal. Revising 'necessary and sufficient' as literally true risks a wrong answer if the exam probes the actual mechanism rather than the summary sentence.",
      cn: "「内含子 5′端恒定的 GU 与 3′端恒定的 AG 是定义剪接位点的必要且充分条件」这一说法，比化学实际支持的要绝对。剪接经由两步转酯化反应完成，其真正的亲核基团是深藏在内含子内部的一个分支点腺苷，而不是 GU/AG 边界本身——GU 和 AG 只是某个识别步骤所读取的标记，并非真正发生反应之处。而且大约 1% 的人类内含子是由一种独立的、改用 AU...AC 边界（而非 GU...AG）的次要剪接体来切除的，所以这个二核苷酸本身也并非放之四海而皆准。如果把「必要且充分」当作字面真理来复习，一旦考题问到真实机制而非概述句，就有答错的风险。" }
  ],
  mustKnow: { en: "Introns are transcribed right along with exons and only removed afterward — which means a single primary transcript can, in principle, be spliced different ways to yield more than one protein from one gene, exactly the possibility the book flags when it notes that a few introns escape normal processing and get translated. And the spliceosome does not need to read the whole intron to find its boundaries — it only needs the invariant GU at one end and AG at the other, which is necessary and sufficient regardless of everything in between.", cn: "内含子和外显子是一起被转录出来的，只是之后才被去掉——这意味着同一条初级转录本原则上可以被剪接成不同的方式，从一个基因产出不止一种蛋白质，这正是教材提到『少数内含子会逃过正常加工、被翻译』时所指出的那种可能性。而剪接体不需要读完整个内含子才能找到边界——它只需要一端不变的GU、另一端不变的AG，不管中间是什么，这就足够了，也是必需的。" },
  summary: {
...

{
  ...
  mustKnow: { en: "The lowest-energy conformation and the biologically active conformation are not automatically the same molecule — for many proteins they differ, which is exactly why folding needs to be actively guided by chaperones rather than left to happen on its own, and why several serious neurological diseases trace back to nothing more than a protein settling into the wrong shape. Even a completed chain is not finished: proline's peptide bond is always made trans, yet about 6% end up cis in the native protein, so an isomerase has to go back and deliberately flip specific bonds after the fact.", cn: "能量最低的构象和有生物活性的构象，并不天然是同一个分子——对许多蛋白质来说两者是不同的，这正是为什么折叠需要分子伴侣主动引导、而不能任其自然发生，也是为什么好几种严重的神经系统疾病，追根溯源不过是一个蛋白质定型到了错误的形状。就算一条链合成完了也还没算完：脯氨酸前面那个肽键总是先做成 trans 的，但成品蛋白里大约 6% 最终是 cis 的，所以异构酶必须回过头去，专门把特定的几个键翻转过来。" },
  summary: {
...

{
  ...
  mustKnow: { en: "A signal sequence works by exploiting its own two-part chemistry: a hydrophobic middle that wants to bury itself in the ER membrane, and a charged end that wants to stay outside it — that mismatch is what physically threads the rest of the growing chain through the membrane pore. And insulin's own history is written into its structure: it starts as one long chain (preproinsulin), gets its signal sequence cut off (proinsulin), and only becomes active insulin once a middle piece, the C-peptide, is excised — which is why the finished hormone's A and B chains, no longer connected by a backbone, still need three disulfide bonds to hold them together at all.", cn: "信号序列之所以能起作用，靠的正是它自身分两截的化学性质：中间疏水、想要埋进内质网膜里，末端带电、想要留在膜外——正是这种『不匹配』，把正在生长的链其余部分物理地穿进了膜上的孔道。胰岛素自己的『历史』也写在它的结构里：它先是一条长链（前胰岛素原），信号序列被切掉后（胰岛素原），要等中间那一段——C肽——被切除，才变成有活性的胰岛素——这也是为什么成品激素的 A链和B链，虽然不再有主链连着，却仍然需要三个二硫键才能勉强绑在一起。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "翻译后修饰——已查全部16卷，未发现独立主题", status: "pending" },
  mustKnow: { en: "The six side-chain modifications are not decorative extras on a finished protein — each one changes what the protein IS chemically able to do: glycosylation can redirect a protein to the lysosome instead of secretion just by adding one phosphate to a mannose, carboxylation needs vitamin K because clotting factors literally cannot bind calcium without that extra carboxyl group, and phosphorylation is singled out by the book as probably the single most important reversible switch in the whole cell, because unlike the other five it is easily undone.", cn: "六种侧链修饰不是给成品蛋白加的装饰性花边——每一种都真的改变了这个蛋白质在化学上**能做什么**：只要在一个甘露糖上加一个磷酸，糖基化就能把一个蛋白质从分泌路线改道送进溶酶体；羧化反应离不开维生素K，是因为凝血因子如果没有那个额外的羧基，字面意义上就没法结合钙离子；而磷酸化被教材特别点出来，很可能是整个细胞里最重要的可逆开关——因为和另外五种不同，它很容易被撤销。" },
  summary: {
