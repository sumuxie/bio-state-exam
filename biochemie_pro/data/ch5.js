...

{
  ...
  mustKnow: { en: "Be careful with the claim that only oxidative deamination frees ammonia — this node's own examples deny it: histidine's SIMPLE deamination gives urocanic acid + NH4+, and the DEHYDRATION route for serine and threonine ends in ammonia too. What genuinely does NOT remove nitrogen is TRANSAMINATION, which only shifts the amino group onto another carbon skeleton. Oxidative deamination matters because it is the route MOST amino acids take, and glutamate dehydrogenase is the nodal reaction: nitrogen is first collected onto glutamate, and it is from there that it leaves.", cn: "「只有氧化脱氨能放出游离氨」这句话要当心——本节自己的例子就否定它：histidine 的**简单脱氨**给出 urocanic acid + NH4+，serine / threonine 的**脱水**路线最后也放出氨。真正**不**除去氮的是 **transamination**，它只是把氨基挪到另一个碳骨架上。氧化脱氨之所以重要，是因为**大多数**氨基酸走的是这条路；而 glutamate dehydrogenase 之所以是枢纽反应，是因为氮先被汇集到 glutamate 上，再从那里离开。" },
  summary: {
