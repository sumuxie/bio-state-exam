...
window.BIOCHEM.topics.push(

  {
    ...
    trace: [
      {
        term: "linking number (Lk) 连接数——一个「除非剪断，否则谁也动不了」的整数",
        what: "闭环双链 DNA 的**拓扑不变量**。把其中一条环状链想象成一个曲面的边界（吹泡泡棒上绷着的那层肥皂膜），**Lk 就是另一条链穿过这个曲面的次数**——永远是整数，对 DNA 实际采用的右手螺旋按惯例取正。关键性质只有一条，但它决定了一切：**只要两条链都保持完整，任何连续形变都改变不了它**——弯曲、热运动、蛋白结合，全都不行。只有骨架被**短暂打断又重新接上**时它才会变。反过来说，带缺刻 (nick) 的 DNA **根本没有 Lk**，是未定义的：拓扑性质要求两条链都构成完整闭环。",
        from: "为什么非得搬出这么个抽象的整数？因为「DNA 是超螺旋的」这句话本身**没法测**，而真正需要解释的现象摆在那里：**纯化的闭环质粒，把蛋白全部剥光之后，依然高度超螺旋**，而且来自同一细胞来源的 DNA **每次都回到同一个程度**。如果超螺旋只是「被挤进小空间」的副产品，那么去掉挤压、去掉施加挤压的蛋白，它就该松弛下来——**但它没有**。Lk 正是解释这件事的那个量：它在细胞内由拓扑异构酶设定好之后，**只要环保持闭合就被锁死**——从欠旋态回到松弛态之间，不存在一条不打断链的连续路径，而纯化过程并不打断链。**所以 Lk 把「超螺旋是内在性质、不是包装假象」从一句断言变成了一个可以算的数。**",
        to: "考试里三处会用到。第一，**算 σ**，这是本节最可能出计算题的地方，套路固定三步：先 Lk₀ = bp/10.5，再 ΔLk = Lk − Lk₀，再 σ = ΔLk/Lk₀，**最后别忘了判正负号**（负 = 欠旋 = 负超螺旋）。第二，被问「为什么纯化之后超螺旋还在」——**答「拓扑性质，Lk 在不剪断链的前提下被锁死」**，不要答「残留盐离子固定住了」或「提取过程人为造成的」。第三，**拓扑异构酶的 I/II 型分类，定义本身就是按「每次改变几个 Lk」下的**：I 型切一条链、改变 Lk 1、不要 ATP；II 型切两条链、改变 Lk 2、要 ATP。**先把 Lk 说清楚，I 型 II 型才不是两个要死记的编号，而是「切一条链还是切两条链」的直接后果。**",
        family: "一组围着 Lk 转的量，**分清了这张表就不会算错**：**Lk₀**（同一段 DNA 完全松弛时的 Lk，= bp/10.5）、**ΔLk**（欠旋量，Lk − Lk₀）、**σ**（超螺旋密度，ΔLk/Lk₀，**与分子大小无关**，所以才能拿来比较不同的 DNA）、**拓扑异构体 topoisomers**（碱基序列和数目完全相同、**只在 Lk 上不同**的两个分子，能用琼脂糖凝胶电泳分开，因为更紧凑的跑得更快）。另有两组容易混的概念：①**两条链互绕成双螺旋**是 Lk 的来源，**轴线自身盘绕成超螺旋**是 Lk 偏离 Lk₀ 之后的表现，是两件事；②同一个 ΔLk 有**两种物理形态**——**plectonemic**（游离溶液中，右手、伸展、带分支）和 **solenoidal**（绕着蛋白，左手、紧密），**真正实现细胞内包装的是后者**。",
        numbers: "B 型 DNA **每圈 10.5 bp**，所以 Lk₀ = bp/10.5。一个 **2,100 bp** 的松弛环 Lk₀ = **200**；去掉两圈则 Lk = 198、ΔLk = **−2**、σ = **−0.01**。书上自带的实例值得当计算模板背下来：**4,200 bp** → Lk₀ = 4,200/10.5 = **400**；Lk = 374 时 ΔLk = **−26**、σ = −26/400 = **−0.065**（负超螺旋）；**同一个分子** Lk = 412 时 ΔLk = **+12**、σ = **+0.03**（正超螺旋）——一个分子，两个拓扑异构体，正负号只取决于 Lk。真实细胞 DNA 密集地落在 **σ = −0.05 到 −0.07**，即**欠旋 5%–7%**。另一个小例子：**84 bp** 的松弛环有 **8** 圈（84/10.5），去掉一圈只剩 **7** 圈，就是每圈 **12.0 bp**——对最稳定 B 型的真实偏离。酶这边：**I 型每次改变 Lk 1 个单位、不需要 ATP**；**II 型每次改变 2 个单位**，真核机制**每循环结合并水解 2 个 ATP**。plectonemic 形态的超螺旋轴线（含分支）约为 DNA 轮廓长度的 **40%**。"
      }
    ],
    mustKnow: {
    ...
    points: [
      {
        ...
        en: "UNDERWINDING IS STORED, CELL-REGULATED STRAIN — NOT AN ACCIDENT OF PACKAGING. Purified closed-circular DNA, freed from every protein, is almost never found relaxed, and DNA from a given cellular source shows a CHARACTERISTIC degree of supercoiling every time. That reproducibility is the key evidence: if supercoiling were just an incidental byproduct of squeezing DNA into a small space, it would not come back to the same value after purification, with all the packaging proteins gone. Instead it tells you supercoiling is an INTRINSIC property of DNA tertiary structure that cells actively regulate with enzymes (see below), and that the resulting strained state is a form of stored energy the cell has deliberately paid for. IN THE BOOK'S OWN WORDS: “many circular DNA molecules remain highly supercoiled even after they are extracted and purified” (A p.891); “supercoiling is an intrinsic property of DNA tertiary structure” (A p.891); “DNAs derived from a given cellular source have a characteristic degree of supercoiling” (A p.892).",
