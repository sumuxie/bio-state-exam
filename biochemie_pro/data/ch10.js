...

{
  ...
  mustKnow: { en: "Storing glycogen or fat counts as anabolism in this book's own definition, not a third category alongside building and breaking down — which is why the whole chapter can be organised around just two poles, catabolism and anabolism, instead of three. And degradation itself is listed as one of metabolism's four functions in its own right (turnover), not merely as a means to energy or building blocks — the cell breaks things down on purpose, not only when it needs to.", cn: "把糖原或脂肪存起来，按本书自己的定义就属于合成代谢，不是第三个独立类别——这也是为什么整章都能围绕分解代谢和合成代谢这两极来组织，而不需要第三极。而且降解本身就被列为代谢四项功能之一（周转），不只是获取能量或原料的手段——细胞主动地降解东西，不只是在需要的时候才这么做。" },
  trace: [
    {
      term: "intermediary metabolism 中间代谢",
      what: "「中间」两个字容易让人以为它指某个居中的阶段，其实不是。它指的是**细胞里那一整套彼此严格协调的反应的总和**——之所以叫「中间」，是因为它讲的是营养物进来、废物出去之间的那一大片，也就是所有中间产物（metabolite）经历的过程。",
      from: "教材在定义里特意强调了「严格协调」四个字，这不是修辞。因为如果只是一堆反应各跑各的，那不叫代谢，那叫一锅乱炖。让它成为一个体系的，恰恰是协调——什么时候开、什么时候关、谁的产物给谁用。这也是为什么整门课到后面会花那么大力气讲调控。",
      to: "整章被组织成**两极**：catabolism（分解代谢，把东西拆开）和 anabolism（合成代谢，把东西造起来）。这里有一个容易被当成第三类的东西，教材明确归了队：**把糖原或脂肪存起来，算合成代谢**，不是独立的「储存代谢」。所以你只需要两个筐，不需要第三个。",
      family: "教材把代谢的功能列了四项，其中有一项值得单独留意：**降解（周转）本身就是一项功能**，而不只是「为了拿能量或拿原料」的手段。也就是说，细胞是**主动**在拆自己的东西，不是等到缺了才拆。这一点和直觉不太一样，但它解释了后面很多现象。"
    }
  ],
  summary: {
...

{
  ...
  cnNote: { topic: "十七", title: "代谢总论 (笔记 p.99 — title-match only per the HANDOFF §11 map, these note pages have never actually been read)", status: "partial" },
  mustKnow: { en: "Catabolism is shaped like a funnel: three completely different classes of nutrient — sugars, fats, proteins — all narrow down to one shared intermediate, acetyl-CoA, before being burned to the same two waste products. Biosynthesis runs the same shape backwards, widening from small precursors out to the cell's own macromolecules — which is exactly the symmetry the next node then shows is not quite real.", cn: "分解代谢的形状是一个漏斗：三类完全不同的营养物——糖、脂肪、蛋白质——最终都收窄到同一个共用的中间产物，乙酰辅酶A，然后被烧成同样的两种废物。生物合成把这个形状倒过来跑，从小的前体分子一路扩展到细胞自己的大分子——而这种对称性，恰恰是下一节要说明其实并不完全成立的地方。" },
  summary: {
...

{
  ...
  cnNote: { topic: "十七", title: "代谢总论 (笔记 p.99 — title-match only per the HANDOFF §11 map, these note pages have never actually been read)", status: "partial" },
  mustKnow: { en: "A pathway optimised to release energy cannot simply be run backwards to store it — the same thermodynamics that make the forward direction favourable make the reverse direction unfavourable, so synthesis has to take a different route with different chemistry. And that separateness is not a limitation, it is the whole point: if synthesis were literally degradation in reverse, any signal that sped one up would automatically speed up the other, and the cell would have no way to choose between them.", cn: "一条为释放能量而优化的通路，不能简单地倒着跑回去把能量存起来——让正向有利的那套热力学，恰恰让反向变得不利，所以合成必须走一条不同的化学路线。而这种『分开』不是缺陷，正是整件事的意义所在：如果合成真的只是分解反着来，那任何加速其中一个的信号都会自动加速另一个，细胞就没法在两者之间做选择了。" },
  trace: [
    {
      term: "irreversible step 不可逆步骤",
      what: "通路里那些**回不去的反应**。分解代谢和合成代谢之所以不是同一条路的正反两个方向，症结全在这些步骤上：其余可逆的步骤都可以直接倒着用，唯独它们不行，必须另修一条路绕过去。",
      from: "为什么会有回不去的步骤？因为热力学本来就不对称。分解代谢是为了**放出**能量而优化的，那条路上让它顺畅往下走的东西，反过来看就成了往上爬的陡坡。所以「把反应倒过来跑」在纸上写得通，在细胞里走不通——合成必须换一套化学反应。",
      to: "关键的一句在这里：**这种分开不是设计缺陷，恰恰是全部意义所在**。教材说得很直接——两条路分开走，好处是可以各自独立调控。设想一下如果不分开：任何一个加速分解的信号，都会同时加速合成，细胞就永远没法在「现在该烧」和「现在该存」之间做选择，两边空转互相抵消。所以「不可逆」不是麻烦，是细胞能做主的前提。",
      family: "除了走不同的化学路线，教材还提到另一层分隔：真核细胞里，分解代谢和合成代谢通常**待在不同的细胞区室**（compartmentation）。同一件事在两个地方做，就更不容易互相干扰——这是空间上的分开，和上面化学上的分开是同一个思路的两种实现。"
    }
  ],
  summary: {
...

{
  ...
  cnNote: { topic: "十七", title: "代谢总论 (笔记 p.99 — title-match only per the HANDOFF §11 map, these note pages have never actually been read)", status: "partial" },
  mustKnow: { en: "The word 'pool' does not mean a high concentration — it means the ability to hold a level steady under very fast turnover, and the standing example is ATP: the whole-body pool is a few grams, yet the body makes about 70 kg of it in 24 hours. Metabolism is a set of flows, not a set of stocks, which is why regulation is always about controlling a RATE, never about accumulating more of anything.", cn: "『代谢库』这个词说的不是高浓度——而是在极快的周转下依然能把水平稳住的能力，最典型的例子就是 ATP：全身的库存只有几克，但身体一天之内能造出大约 70 公斤。代谢是一套『流』，不是一套『存量』，这也是为什么调节永远是在控制**速率**，从来不是在囤积更多的什么东西。" },
  summary: {
...

{
  ...
  cnNote: { topic: "十七", title: "代谢总论 (笔记 p.99 — title-match only per the HANDOFF §11 map, these note pages have never actually been read)", status: "partial" },
  mustKnow: { en: "Everything the cell eats — sugar, fat, protein — arrives at the same one molecule, acetyl-CoA, before the cell decides what to do with it: burn it for energy, or invest it in fat or in the whole isoprenoid/steroid family. And what the diagram leaves OUT matters as much as what it draws: there is no arrow back from acetyl-CoA to pyruvate, and that single missing arrow is the entire reason an animal cannot turn fat back into sugar.", cn: "细胞吃进去的一切——糖、脂肪、蛋白质——最终都到达同一个分子：乙酰辅酶A，然后细胞才决定拿它做什么：烧掉换能量，还是投进脂肪或整个类异戊二烯/类固醇家族。而这张图**没画**的东西和画出来的一样重要：从乙酰辅酶A回到丙酮酸没有箭头，正是这一个缺失的箭头，解释了动物为什么没法把脂肪变回糖。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Mobilising stored glycogen is cheaper than importing free glucose, because phosphorolysis uses inorganic phosphate while trapping a free sugar costs an ATP — the same sugar, two different price tags depending on where it came from. And the pentose phosphate pathway is not a rival to glycolysis competing for the same job; it exists to supply exactly what glycolysis cannot — five-carbon sugars for nucleic acids, and NADPH earmarked for biosynthesis rather than burned in the respiratory chain.", cn: "动用储存的糖原比进口游离葡萄糖便宜，因为磷酸解用的是无机磷酸，而困住一个游离糖分子要花一个 ATP——同样是糖，来源不同，价签就不同。而磷酸戊糖途径不是在跟糖酵解抢同一份工作；它存在的意义正是提供糖酵解给不了的东西——供核酸用的五碳糖，以及专门留给生物合成用、不会被烧进呼吸链的 NADPH。" },
  summary: {
...

{
  ...
  cnNote: { topic: "十七", title: "代谢总论 (笔记 p.99 — title-match only per the HANDOFF §11 map, these note pages have never actually been read)", status: "partial" },
  mustKnow: { en: "Gluconeogenesis is not a separate pathway invented from scratch — it is glycolysis run backwards, reusing almost every reversible step, with only the handful of irreversible ones bypassed by different, ATP-spending chemistry. That round trip — a kinase spending an ATP going down, a phosphatase throwing the phosphate away going up — is the concrete, literal cost of the fact that catabolism and anabolism are not mutually reversible.", cn: "糖异生不是从零发明出来的另一条独立通路——它就是倒着跑的糖酵解，几乎每一个可逆步骤都被重复利用，只有那几个不可逆的步骤，才被换成不同的、要花 ATP 的化学反应绕过去。这一个来回——激酶下坡时花一个 ATP，磷酸酶上坡时又把磷酸扔掉——正是『分解代谢和合成代谢彼此不可逆』这句话具体、字面意义上的代价。" },
  trace: [
    {
      term: "gluconeogenesis 糖异生",
      what: "「异生」听着玄，其实字面就是它的意思：**从不是糖的东西造出糖来**（gluco- 糖，neo- 新，genesis- 生成）。它不是一条另起炉灶发明出来的新通路——**它基本上就是倒着跑的糖酵解**。",
      from: "为什么能直接倒着跑？因为糖酵解里绝大多数步骤本来就是可逆的，倒过来用就行，不必重新发明。真正的麻烦只出在那几个**不可逆步骤**上——它们过不去，就得单独修便道绕开。所以这条通路的全部技术含量，就集中在「怎么绕过那几步」。",
      to: "绕法有两处，记住这两处这一节就通了。第一处是丙酮酸变回 PEP 走不通，于是绕一个弯：先把丙酮酸羧化成草酰乙酸（oxaloacetate），再从那里过去。第二处是糖酵解里那两次己糖磷酸化不可逆，合成时只能换**另外的酶**来做——教材把它们叫作水解酶（hydrolase），指的是葡萄糖-6-磷酸酶和果糖-1,6-二磷酸酶，做法不是把磷酸转走，而是直接**水解掉**。",
      family: "这里能看到上一节那句「分解不是合成倒着跑」的字面代价：下坡时激酶花掉一个 ATP 把磷酸装上去，上坡时磷酸酶又把这个磷酸直接扔掉——**这一来一回，那个 ATP 就白花了**。听起来浪费，但买到的是两条路各自独立调控的能力。另外值得知道：动物体内糖异生的碳源主要是无氧糖酵解产生的乳酸，所以这条路首先是一条**回收**路线，不是从零合成。"
    }
  ],
  summary: {
...

{
  ...
  mustKnow: { en: "A fat is metabolically two different molecules stitched together: the glycerol backbone is, chemically, a sugar and rejoins carbohydrate metabolism directly, while the fatty-acid tails are activated at a cost of ATP and fed two carbons at a time into the very same acetyl-CoA pool everything else uses. Ketone bodies are not a separate pathway either — they are simply what happens when that shared pool is flooded faster than the citrate cycle can take acetyl-CoA away.", cn: "从代谢的角度看，一个脂肪分子其实是两种不同的东西缝在一起：甘油骨架本质上就是一种糖，能直接重新加入糖类代谢；而脂肪酸尾巴则先花一个 ATP 被活化，然后一次两个碳地喂进和其他一切共用的同一个乙酰辅酶A库。酮体也不是另一条独立通路——它们只是这个共用库被灌得比柠檬酸循环能消耗掉的速度还快时发生的事。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Fatty acid synthesis LOOKS like degradation run backwards, which is exactly why the book has to spell out that it is not: it starts with a step degradation has no counterpart to (carboxylating acetyl-CoA), it uses NADPH instead of making NADH, and it runs in a different compartment, on a complex that never lets go of its intermediates — so it cannot be reversed or raided by any other pathway. And the two nutrient classes still exchange one thing freely, just not the fuel itself: sugar metabolism supplies the glycerol backbone that turns acyl-CoA into an actual fat molecule.", cn: "脂肪酸合成**看起来**像是分解反着跑，这正是为什么教材必须专门说明它不是：它以一个分解里根本没有对应步骤的反应开场（把乙酰辅酶A羧化），它用 NADPH 而不是生成 NADH，而且它在不同的区室里、在一个从不放开中间产物的复合物上运行——所以它没法被逆转，也没法被别的通路半路截走。而这两类营养物之间仍然有一样东西是自由交换的，只是不是燃料本身：糖代谢提供甘油骨架，把酰基辅酶A变成一个真正的脂肪分子。" },
  summary: {
...

{
  ...
  cnNote: { topic: "十七", title: "代谢总论 (笔记 p.99 — title-match only per the HANDOFF §11 map, these note pages have never actually been read)", status: "partial" },
  mustKnow: { en: "Sugar and fat both funnel down to the same molecule, acetyl-CoA, and which way that molecule goes — burned for energy or invested in more fat — is decided purely by the cell's energy status; that is also why excess of EITHER one, sugar or fat, ends up stored as fat, since fat is the only direction acetyl-CoA can go once the cell has enough energy. But the traffic is one-way: an animal cannot turn acetyl-CoA back into pyruvate, so it can never build sugar from fat — only plants and some microbes, with the glyoxylate cycle, can close that loop.", cn: "糖和脂肪最终都汇聚到同一个分子——乙酰辅酶A——而这个分子往哪个方向走（烧掉换能量，还是投进更多脂肪），纯粹由细胞的能量状态决定；这也是为什么不管是糖还是脂肪，只要摄入过量，最终都会以脂肪的形式存起来——因为一旦能量够了，脂肪就是乙酰辅酶A唯一能去的方向。但这条路是单向的：动物没法把乙酰辅酶A变回丙酮酸，所以永远没法用脂肪造出糖——只有植物和某些微生物，靠乙醛酸循环，才能把这个环闭合起来。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Only one high-energy phosphate, GTP, comes directly out of the citrate cycle — the bulk of acetyl-CoA's energy is not captured until NADH and FADH2 are reoxidised in the respiratory chain, a separate machine the cycle merely supplies. And the cycle is not purely catabolic: it doubles as a supply depot, handing out alpha-ketoglutarate and oxaloacetate for amino acid synthesis and succinyl-CoA for porphyrins — which is exactly why draining those same intermediates is a real problem the cell has to solve.", cn: "柠檬酸循环本身直接产出的高能磷酸只有一个：GTP——乙酰辅酶A里绝大部分的能量，要等 NADH 和 FADH2 在呼吸链里被重新氧化才真正被捕获，而呼吸链是一台独立的机器，循环只是给它供货。而且这个循环不是纯粹的分解代谢：它同时也是一个补给站，拿出 α-酮戊二酸和草酰乙酸去合成氨基酸、拿出琥珀酰辅酶A去合成卟啉——这也正是为什么抽走这些中间产物会是细胞必须解决的一个真实问题。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Without regulation, metabolism fails in two distinct ways, not one: reactions running uselessly (waste), or two systems actively working against each other (conflict) — and the second is the more dangerous, because a synthetic and a degradative pathway both running at full speed does not just burn ATP, it leaves the cell's state undefined. The book's own claim is stronger than 'regulation helps': a high degree of organisation is only POSSIBLE under consistent regulation — a precondition, not an optimisation.", cn: "没有调节，代谢会以两种不同的方式失败，而不是一种：反应白白空转（浪费），或者两套系统互相较劲（冲突）——第二种更危险，因为一条合成通路和一条分解通路同时全速运转，不只是白白烧掉 ATP，还会让细胞的状态变得不确定。教材自己的说法比『调节有帮助』更强：高度的组织化只有在持续的调节之下才**可能**存在——这是一个前提条件，不是一项优化。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Regulating a reaction by its substrate concentration only works in the low, linear part of the Michaelis-Menten curve — and the book's own hedge is that this mechanism is 'not very effective' in general, EXCEPT that real intracellular concentrations happen to sit exactly in that responsive region, which is what makes a small pool regulatory at all. Changing the reaction rate this way is fast but shallow; changing the enzyme's own AMOUNT by induction or repression is slower but sets the ceiling — and it is exactly this second route that steroid hormones use to work.", cn: "靠底物浓度来调节一个反应，只在米氏曲线的低浓度、线性那一段才有效——教材自己也留了一个但书：这个机制总体上『不算很有效』，除非细胞内的实际浓度恰好就落在那个敏感区间里，这也正是为什么一个小小的『库』能起到调节作用。用这种方式改变反应速率快，但影响有限；通过诱导或阻遏去改变酶本身的**数量**则更慢，但决定了上限——而类固醇激素起作用，走的正是后面这条路。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "no matching topic found in the Chinese notes' 27-topic list or its unnumbered tail (checked against the HANDOFF §11 map)", status: "pending" },
  mustKnow: { en: "Allosteric control works precisely because the regulating site sits somewhere OTHER than the active site — which means the regulator need not resemble the substrate at all, and a pathway's own end product can reach back and shut down an enzyme acting on a completely different molecule. That is the entire mechanism behind negative feedback: the final product of a chain inhibits one of the FIRST steps, so the whole chain throttles itself the moment its output is no longer needed.", cn: "别构调控之所以行得通，正是因为调控位点在活性中心**之外**的某个地方——这意味着调控分子根本不需要和底物长得像，一条通路自己的终产物就能反过来关掉一个作用于完全不同分子的酶。这正是负反馈的全部机制：一条链的终产物抑制它自己**最开头**的一步，于是一旦不再需要这个产物，整条链就会自己把油门收回去。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "no matching topic found in the Chinese notes' 27-topic list or its unnumbered tail (checked against the HANDOFF §11 map)", status: "pending" },
  mustKnow: { en: "An irreversible step is where control actually has leverage: a reversible reaction just transmits whatever is done to it, but past a one-way step material cannot flow back, so whoever controls that single step controls the committed flux through everything downstream — which is exactly why phosphofructokinase, not some earlier or later step, is glycolysis's control point, and exactly why those same irreversible steps are what gluconeogenesis has to detour around. The other lever is simply not having enough of something: oxidative phosphorylation is capped not by fuel or oxygen but by how much ADP is actually available to be turned back into ATP.", cn: "一个不可逆的步骤，才是控制真正有杠杆作用的地方：可逆反应只是把加在它身上的东西原样传下去，但过了一个单向步骤，物质就流不回来了，所以谁控制了这一步，谁就控制了下游整条被『锁定』的流量——这正是为什么磷酸果糖激酶、而不是更早或更晚的某一步，是糖酵解的控制点，也正是为什么糖异生要绕开的恰恰是这几个不可逆步骤。另一个杠杆更简单：就是某样东西不够用——氧化磷酸化的上限不是燃料也不是氧气，而是究竟有多少 ADP 可以被重新变回 ATP。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "no matching topic found in the Chinese notes' 27-topic list or its unnumbered tail (checked against the HANDOFF §11 map)", status: "pending" },
  mustKnow: { en: "Phosphorylation and proteolysis both switch an enzyme on or off, but only one of them can be undone: a phosphatase can remove a phosphate, but a proenzyme that has been cut cannot be uncut — which is exactly why proteolytic activation is used where the cell wants a one-way, committed decision, like digestion or blood clotting, and phosphorylation is used where it wants a reversible switch. And compartmentation is not just tidiness: keeping pathways that share intermediates in separate membrane-bound spaces is what lets both run at once, in opposite directions, without one undoing the other.", cn: "磷酸化和蛋白水解都能把一个酶开关掉，但只有一种是可以撤销的：磷酸酶能把磷酸基团拿掉，但一个已经被切开的酶原没法再被『缝回去』——这正是为什么蛋白水解激活被用在细胞需要一次性、不可反悔的决定的地方，比如消化或血液凝固，而磷酸化被用在需要一个可逆开关的地方。区室化也不只是整洁——把共用中间产物的通路分别关进不同的膜隔间里，正是让两者能同时朝相反方向运转、而不会互相抵消的原因。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Specificity of hormone action lives entirely in the RECEIVING cell, not in the signal itself: a hormone travels everywhere the blood reaches, but only cells carrying its receptor respond — so 'target cell' is defined by what the cell has, not by where the hormone goes. And the three-level gland hierarchy feeds back on itself at two levels at once, not one: the peripheral hormone inhibits both the gland directly above it and the one above that — the same negative-feedback logic as an allosteric enzyme, just scaled up from a molecule to a chain of organs.", cn: "激素作用的特异性完全在于**接收**它的细胞，不在于信号本身：激素随血液流到全身各处，但只有带着对应受体的细胞才会响应——所以『靶细胞』是由细胞**拥有什么**定义的，而不是由激素**去了哪里**定义的。而三级腺体层级同时反馈到**两个**层面，不是一个：外周激素同时抑制它正上方的腺体和再上一级——和别构酶一模一样的负反馈逻辑，只是从一个分子放大到了一条器官链。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "A single hormone molecule binding at the surface produces many cyclic AMP molecules, each activating a kinase that phosphorylates many substrates — that cascading multiplication is the whole point of a second messenger, and cyclic AMP's short half-life is not a weakness but a requirement, since a messenger that lingered could never be switched off. And the same messenger means different things in different tissues: the specificity is not in the cAMP itself, it is downstream, in which receptors and which cAMP-sensitive enzymes a given cell happens to carry.", cn: "一个激素分子在细胞表面结合，就能产生许多个环化AMP分子，每一个又激活一个能磷酸化许多底物的激酶——这种层层放大正是第二信使存在的意义，而环化AMP半衰期短不是弱点，是必需的：一个赖着不走的信使就永远没法被关掉。同一个信使在不同组织里意味着不同的事：特异性不在环化AMP本身，而在下游——在某个细胞恰好带着哪些受体、哪些对环化AMP敏感的酶。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "One receptor-triggered cut of one membrane lipid produces TWO separate messengers, not one — and they immediately go two different ways because of a simple physical fact: IP3 is water-soluble and diffuses off into the cytosol to open calcium channels, while diacylglycerol is a lipid and stays put in the membrane, waiting for protein kinase C to come find it. One cleavage, two destinations, two completely different modes of action, from a single hormone binding event.", cn: "受体触发的一次膜脂切割，产生的不是一个、而是**两个**独立的信使——而且因为一个简单的物理事实，它们立刻走向两个不同的方向：IP3 是水溶性的，扩散进细胞质里去打开钙通道，而二酰甘油是脂质，留在膜里原地不动，等着蛋白激酶C来找它。一次切割，两个去处，两种完全不同的作用方式，全都源自一次激素结合事件。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "Insulin's receptor is its own signal amplifier: it phosphorylates its own tyrosines, which makes it MORE active, with no separate second messenger anywhere in the loop — a completely different architecture from the cAMP or phosphoinositide cascades. Steroid and thyroid hormones are different again, in kind: instead of signalling from the surface, they walk straight into the cell and the nucleus, and their receptor is a DNA-binding protein, not an enzyme or a channel — so the output is brand-new protein rather than a modified one, which is exactly why this route is slower to start and longer to fade than the other three.", cn: "胰岛素的受体是它自己的信号放大器：它把自己的酪氨酸磷酸化，这让它**变得更活跃**，整个回路里没有任何独立的第二信使——这和 cAMP 或磷酸肌醇级联完全是不同的架构。类固醇和甲状腺激素则是另一种完全不同的类型：它们不从表面发信号，而是直接走进细胞、走进细胞核，它们的受体是一个**结合DNA的蛋白质**，不是酶也不是通道——所以产出的是全新合成的蛋白质，而不是被修饰过的蛋白质，这也正是为什么这条路比另外三条起效更慢、但作用更持久。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "The same adrenal gland makes two chemically unrelated stress hormones from two different tissues: cortisol, a steroid from the cortex, whose row in the table says only stress, and adrenaline, an amino-acid derivative from the medulla, for short-term stress — same organ, same broad job, completely different chemistry and completely different tissue of origin. Even the table's blank cell is informative: serotonin has no single organ listed, because unlike a classical hormone it is not the product of one dedicated gland.", cn: "同一个肾上腺，用两种化学上毫不相关的激素来应对压力，而且来自两种不同的组织：皮质部分产的类固醇——皮质醇，表里那一栏只写了「压力」；髓质部分产的氨基酸衍生物——肾上腺素，专门负责短期压力。同一个器官，大致同一个任务，化学结构和来源组织却完全不同。就连表格里的空格都有信息量：血清素那一栏『器官』是空的，因为和经典激素不同，它不是某一个专门腺体的产物。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "Reading the anterior pituitary rows together shows the pattern at a glance: almost every one of them is a TROPIC hormone, meaning its job is not to act on ordinary tissue but to switch on another gland downstream (ACTH on the adrenal cortex, TSH on the thyroid, FSH/LH on the gonads) — the pituitary is the middle management level of the whole hormone hierarchy, row by row. And two pairs in the table are explicit antagonists working the same axis in opposite directions — insulin/glucagon for blood sugar, parathormone/thyrocalcitonin for calcium — which is what lets the body correct in either direction, not just one.", cn: "把腺垂体那几行放在一起看，规律一眼就能看出来：它们几乎全都是**促激素**，意思是它们的工作不是直接作用在普通组织上，而是去打开下游的另一个腺体（ACTH 作用于肾上腺皮质、TSH 作用于甲状腺、FSH/LH 作用于性腺）——垂体逐行体现的，正是整个激素层级里的『中层管理』这一级。而表里有两对激素是明确的拮抗对，在同一根轴上朝相反方向发力——胰岛素/胰高血糖素管血糖，甲状旁腺激素/降钙素管钙——这才让身体能朝两个方向都做出纠正，而不只是一个方向。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "All three corticoids share one skeleton and differ only in decoration, and that decoration is not cosmetic — it IS the function: the C18-aldehyde version (aldosterone) is the mineral-handling hormone, the C17-hydroxyl version (cortisol) is the stress/blood-sugar hormone, so the structural difference maps directly onto the functional split. Estradiol is the true outlier of the group: its A ring has gone aromatic, trading the usual ketone for a phenolic OH — which is exactly what makes estrogens weakly acidic and separable from every other steroid on that basis alone.", cn: "三种皮质类固醇共用同一个骨架，区别只在装饰性基团上，而这种『装饰』并不是无关紧要的——它**就是**功能所在：带 C18 醛基的那个（醛固酮）是管矿物质的激素，带 C17 羟基的那个（皮质醇）是管压力和血糖的激素，结构上的差异直接对应功能上的分野。雌二醇是这一组里真正的异类：它的 A 环变成了芳香环，把常见的酮基换成了酚羟基——正是这一点让雌激素呈弱酸性，仅凭这一条就能把它和所有其他类固醇分开。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "One amino acid, tyrosine, is the raw material for two hormone families that work by completely different mechanisms: as catecholamines (dopamine, noradrenaline, adrenaline, each one both a product and the next step's precursor along a single linear path) signalling from the cell surface, and as thyroxine signalling from inside the nucleus. And thyroxine itself is shipped as an inactive prohormone with one extra iodine — the gland secretes the four-iodine form, and the tissue that actually needs it removes one iodine to activate it locally, on demand.", cn: "同一种氨基酸——酪氨酸，是两个作用机制完全不同的激素家族的原料：一边是儿茶酚胺（多巴胺、去甲肾上腺素、肾上腺素，沿着一条线性通路，每一个既是产物又是下一步的前体），从细胞表面发信号；另一边是甲状腺素，从细胞核内部发信号。而甲状腺素本身是以带一个多余碘原子的**无活性前激素**形式运出去的——腺体分泌的是四碘形式，真正需要它的组织再就地去掉一个碘，按需激活。" },
  summary: {
...

{
  ...
  cnNote: { topic: null, title: "激素 Hormones (笔记 p.183, unnumbered appendix topic — strong title match per the HANDOFF §11 map, not yet read)", status: "partial" },
  mustKnow: { en: "Oxytocin and vasopressin differ at only two amino-acid positions out of nine, yet Tab. 10.1 gives them completely different jobs — uterine contraction versus water retention — which is the sharpest illustration in the whole chapter that specificity lives in the receptor, not in how different two ligands look. And the class has no sharp line between 'peptide hormone' and 'protein hormone': it is one continuum from a nine-residue ring up to 34,000-dalton glycoproteins, with parathormone sitting explicitly at the point the book calls the transition.", cn: "催产素和加压素九个氨基酸里只有两个位置不同，但表10.1赋予它们完全不同的任务——一个引发子宫收缩，一个负责保水——这是全章里对『特异性在于受体、不在于两个配体长得有多不像』这条道理最鲜明的例证。而这一类激素在『肽类激素』和『蛋白类激素』之间根本没有明确界线：从一个九肽环一直到 34000 道尔顿的糖蛋白，是一个连续的谱系，甲状旁腺激素就被教材明确放在这个过渡点上。" },
  summary: {
...

{
  ...
  mustKnow: { en: "All four icosanoid families — leukotrienes, prostaglandins, prostacyclins, thromboxanes — are cut from one twenty-carbon fatty acid, arachidonate, which is why the whole class is defined by carbon count rather than by any single function, and why the table gives them one undivided row with no organ at all: they are made locally, wherever they are needed, not by a dedicated gland. And that starting material is not exotic — it is the very same fatty acid the phosphoinositide cascade already named as sitting on the membrane lipid it signals through, so one membrane component is simultaneously a signalling lipid and the raw material for an entire hormone class.", cn: "二十烷酸类的四个家族——白三烯、前列腺素、前列环素、血栓素——全都是从同一种二十碳脂肪酸——花生四烯酸——上切下来的，这也是为什么整个类别是按碳原子数、而不是按某个单一功能来定义的，也是为什么表格里给它们的是不分家的一整行、而且不写器官：它们是就地制造、哪里需要就在哪里做，不是某个专门腺体的产物。而这个起始原料并不陌生——它正是磷酸肌醇级联里提到的、位于那个传信号的膜脂上的同一种脂肪酸，于是同一个膜成分，既是信号脂质，又是一整类激素的原料。" },
  summary: {
