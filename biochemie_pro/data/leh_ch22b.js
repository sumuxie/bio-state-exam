...
window.BIOCHEM.topics.push(

  {
    ...
    trace: [
      {
        term: "同工酶为什么要有好几份——序贯反馈抑制",
        what: "**若干终产物共用同一段上游步骤**时，简单的反馈抑制会闯祸：**某一个终产物过量，就会把共用的那一步关掉，连带切断其他产物的供应**。解法是**同工酶**——**同一步反应由几份独立调控的酶分别催化**，每份听一个不同终产物的话。这就是**序贯反馈抑制**。⚠️ **「多份」正是关键所在**：不先讲清这个问题，那些同工酶的数目就只能死记。",
        from: "书里的实例把这件事摆得很清楚。在 *E. coli* 从**天冬氨酸**出发通向**赖氨酸、甲硫氨酸、苏氨酸和异亮氨酸**的路径中：把天冬氨酸转化为天冬氨酰-β-磷酸的那个酶存在**三个**独立调控的同工酶；接下来两个分支点上的酶（天冬氨酸-β-半醛→高丝氨酸；苏氨酸→α-酮丁酸）**各自存在两个**。芳香族那条路径展现了**同样的设计**：**三个 DAHP 合酶同工酶**催化通向 chorismate 的第一步，**一个被苯丙氨酸抑制、一个被酪氨酸抑制、一个被色氨酸抑制**——**于是这段共用的早期路径依然能对「具体是哪一种芳香族氨基酸过量」做出响应**。还有一层叠在别构之上：**有些同工酶改由「改变酶的合成量」来调控**——相关终产物充足时，**那个不受别构调控的同工酶的合成会被阻遏**。",
        to: "考试里两处。第一，**三种模式要能分清，这是本节明说的可考点**：**简单**＝终产物别构抑制**自己那条路径**的第一个限速酶（干净的例子是**异亮氨酸抑制自己从苏氨酸合成的第一步**，也是**历史上最早被发现的别构反馈抑制实例之一**）；**协同**＝**一个酶**同时整合**多个独立信号**，**合并效果超过简单相加**（*E. coli* 的**谷氨酰胺合成酶被六种**谷氨酰胺衍生产物抑制）；**序贯**＝**共用上游 + 同工酶**，就是本卡。第二，被问「**为什么这一步要有三个同工酶**」——**答那个「防止误关」的理由**，不要只报数目。",
        family: "本节还有一个很值得一起记的具体机制：**色氨酸合酶把自己的中间产物「隧穿」送到下一个活性位点**。它是 **α₂β₂** 结构、**可解离**成各自独立的活性：**α 亚基**把吲哚-3-甘油磷酸转成**吲哚** ＋ 3-磷酸甘油醛，**β₂ 单元**再把这个吲哚（经 **PLP** 依赖的席夫碱）与**丝氨酸**缩合成色氨酸。**机制上要紧的是：α 生成的吲哚并不释放到溶液中**，而是**经一条内部的分子隧道**直接送进 β 的活性位点。这种**物质传送 (channeling)** 可能贯穿从 chorismate 到色氨酸的整条路径，提示它作为一个 **metabolon**（多酶复合物）运作——⚠️ **而这种复合物在常规纯化中通常不能完整保留，这正是它长期未能被直接生化分离出来的原因**（一个「测不到不等于不存在」的好例子）。",
        numbers: "*E. coli* 天冬氨酸族：第一步（天冬氨酸→天冬氨酰-β-磷酸）**3 个**同工酶；后面两个分支点各 **2 个**。芳香族：**3 个 DAHP 合酶**同工酶，分别被**苯丙氨酸 / 酪氨酸 / 色氨酸**抑制。协同抑制的实例：谷氨酰胺合成酶被 **6 种**谷氨酰胺衍生产物抑制，**联合效果超过各自单独作用之和**。色氨酸合酶结构 **α₂β₂**。⚠️ 再记一条整节的框架数：氨基酸生物合成**不是一条路径，而是六条互不相干的匝道**；**十种**氨基酸离某个常见代谢物（α-酮戊二酸、丙酮酸、草酰乙酸、3-磷酸甘油酸）只有一步或几步，哺乳动物保留了这些短匝道；**三种芳香族**需要经 shikimate 和 chorismate 的**独立建环路径**，**哺乳动物从未进化出这条路**——**这正是草甘膦能杀死植物却对你毫无影响的原因。**"
      }
    ],
    mustKnow: {
