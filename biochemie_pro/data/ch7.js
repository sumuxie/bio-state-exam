...

{
  ...
  cnNote: { topic: "二", title: "糖类 (笔记 pp.5-10, read at topic-overview level: monosaccharide structure/stereochemistry, oligo/polysaccharides, glycoconjugates all confirmed present — not yet cross-checked point-by-point against this CZ subsection)", status: "mapped" },
  mustKnow: { en: "Aldolase splits one hexose into two triose phosphates — the cut every sugar must survive before it can be burned. Transketolase moves two-carbon fragments, which is how chain lengths get reshuffled without any oxidation.", cn: "醛缩酶把一个己糖劈成两个磷酸丙糖——**每一个糖要被烧掉，都得先过这一刀。**转酮酶搬运二碳片段，这是在**完全不发生氧化**的情况下重排碳链长度的办法。" },
  summary: {
...

{
  ...
  mustKnow: { en: "Digestion stops at maltose and isomaltose, not at glucose. And we make no cellulase at all, so the β(1→4) bonds of cellulose pass through us untouched — that is what dietary fibre is.", cn: "消化的终点是麦芽糖与异麦芽糖，**不是**葡萄糖。而且我们根本不产纤维素酶，所以纤维素的 β(1→4) 键会原封不动地穿过我们——**这就是膳食纤维。**" },
  summary: {
...

{
  ...
  mustKnow: { en: "One protein kinase does both halves at once: it switches synthase off and phosphorylase on. That is why the two can never run together and burn ATP in a futile cycle.", cn: "**同一个蛋白激酶同时办两件事**：关掉合酶、打开磷酸化酶。这正是两者永远不会同时运行、白白空转烧掉 ATP 的原因。" },
  summary: {
