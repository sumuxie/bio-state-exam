/* =========================================================================
   PESB — Protein Engineering & Synthetic Biology trainer

   All content comes from window.PESB.topics (data/ch*.js). Nothing in this
   file hardcodes subject matter; add a topic to the data and every mode —
   study, flashcards, quiz, oral — picks it up automatically.

   Forked from the Biochemie trainer. Differences that matter if you are
   porting a fix between the two:
     * No Czech layer. Topics carry enTitle/cnTitle only; a point's short
       headword lives in `term`, not `cz`; glossary entries are EN/CN.
     * `slides` replaces `pages` — the source is lecture slides, not a book.
     * `beyondPoints` replaces `gapPoints` and means something different:
       not "pages we failed to read" but "material the slides never covered,
       added here deliberately". Both render as a dashed block, both exist
       so the reader always knows where a claim came from.
     * `figures` is a first-class block.
     * localStorage keys are prefixed "pesbexplain." — four trainers now share
       one origin (bio., pesb., pesbpro., pesbexplain.), so a shared prefix
       would let each overwrite the others' progress. importV1() copies the
       "pesb." state across on request and never writes back.
     * `explain` is the field this fork exists for: a question-led plain-Chinese
       layer answering "why is this true", kept separate from `points` (what the
       slide said) and `beyondPoints` (what the slide left out).
   ========================================================================= */
...
(function () {
  ...
  // Chapters are the five note batches. `lectures` carries the real lecture
  // numbers: lectures 6 and 10 were never delivered, and the gap is left
  // visible rather than closed up, so "Lec 7–8" is correct as written.
  const CHAPTER_TITLES = {
    1: { lectures: 'Lec 1–2',   en: 'From Gene to Clone',                       cn: '从基因到克隆' },
    2: { lectures: 'Lec 3–5',   en: 'From Cell to Pure Protein',                cn: '从细胞到纯蛋白' },
    3: { lectures: 'Lec 7–8',   en: 'Cell-Free Synthesis and the Genetic Code', cn: '无细胞合成与遗传密码改写' },
    4: { lectures: 'Lec 9',     en: 'Diversity, Screening and Selection',       cn: '多样性构建与筛选' },
    5: { lectures: 'Lec 11–12', en: 'Minimal Cells and Metabolic Engineering',  cn: '最小细胞与代谢工程' },
    // Chapter 6 is not a lecture. It re-tells the worked examples the course
    // keeps returning to — EPO, insulin, the minimal cell — as one continuous
    // story each, so a case can be learned once instead of being met in
    // fragments across five chapters. Every fact in it is assembled from
    // nodes 1–5; the nodes say so in their coverageNote.
    6: { lectures: 'Lec 1–12',  en: 'Case Threads Across the Course',           cn: '贯穿全课的案例' }
  };

  const CHAPTERS = [1, 2, 3, 4, 5, 6];

  /* ----------------------------------------------------------- persistence */
  const PREFIX = 'pesbexplain.';
  const V1_PREFIX = 'pesb.';
  ...
  // One-way copy of pesborigin's saved progress. Read-only on the v1 keys —
  // the whole point of the split is that pesborigin can never be corrupted
  // by anything this app does.
  function importV1() {
    const moved = [];
    ['lang', 'theme', 'bionic', 'studied', 'boxes', 'scores'].forEach((k) => {
      const raw = localStorage.getItem(V1_PREFIX + k);
      if (raw === null) return;
  ...
  function pointLi(topicId, path, p) {
    ...
    const m = markOf(key);
    let inner = '';
    if (state.lang !== 'cn' && p.en) inner += `<div class="t-en">${esc(p.en)} ${speakBtn(p.en, 'en-US')}</div>`;
    if (state.lang !== 'en' && p.cn) inner += `<div class="t-cn">${esc(p.cn)} ${speakBtn(p.cn, 'zh-CN')}</div>`;
  ...
  function renderGlossCards() {
    ...
    $('#gloss-fc-def').innerHTML =
      (term.def_en ? `<div class="t-en">${esc(term.def_en)} ${speakBtn(term.def_en, 'en-US')}</div>` : '') +
      (term.def_cn ? `<div class="t-cn">${esc(term.def_cn)} ${speakBtn(term.def_cn, 'zh-CN')}</div>` : '');
  ...
  const COVERAGE_LABEL = {
    full:    { text: 'full slide coverage', cls: 'cov-full' },
  ...
  function renderStudy() {
    ...
    if (!t) {
      ...
      body.innerHTML = `
        <div class="welcome">
          <h1>Protein Engineering &amp; Synthetic Biology</h1>
          <p class="lede">The whole course, rebuilt section by section from the lecture slides.
             Pick a topic on the left to begin. 左侧选一个主题开始。</p>
          <div class="welcome-grid">
            ...
            <div><strong>${figCount}</strong><span>figures</span></div>
          </div>
          <p class="welcome-note">The final exam is <strong>oral, 50 of ~100 points</strong>, and you cannot pass
             without at least 10 of them — so the Oral mode is the one that matters most. Every section has a
             spoken model answer and a checklist to tick off while you talk.</p>
          <p class="welcome-note">Content is marked by where it came from. Ordinary points are from the slides;
             anything in a dashed <span class="badge badge-beyond">beyond the slides</span> block was added to close a
             gap the lecture left — reasoning that was spoken aloud, hidden in a figure, or left as a link. Both are
             worth knowing; only one of them is what the lecturer actually put up.</p>
          <p class="welcome-note">Lectures 6 and 10 were not delivered, and the numbering here keeps the real
             lecture numbers rather than closing the gap.</p>
          <p class="welcome-note">🔊 A speaker icon plays browser-synthesised speech — not a recording — so you can
             check pronunciation. 🎧 in the top bar picks which voice does it, and how fast: English and Chinese
             are set separately, and voices marked ✦自然 are the neural ones, the closest to a real speaker your
             machine has. 🧬 <em>Bionic</em> in the top bar bolds the lead of each Latin-script word, a
             scanning aid some ADHD and dyslexic readers find helps them read faster; Chinese text is left alone
             since the technique doesn't transfer to a logographic script.</p>
        </div>`;
      bionicRefresh();
      return;
    }
    ...
    if (t.mustKnow || (t.explain && t.explain.length)) {
      html += `<section class="block block-explain">
                 <h2>先看懂 <span class="muted">Understand it first</span>
                     <span class="badge badge-explain">解释版</span></h2>`;

      if (t.mustKnow) {
        html += `<div class="must-know">
                   <span class="must-know-label">这一节要带走的</span>
                   <div class="must-know-body">${paras(t.mustKnow)}</div>
                   ${t.skipIt ? `<p class="skip-it"><span class="skip-label">不用背</span>${esc(t.skipIt)}</p>` : ''}
                 </div>`;
      }

      if (t.explain && t.explain.length) {
        html += `<details class="explain-fold">
                   <summary>看不懂？展开追根溯源 · ${t.explain.length} 条</summary>
    ...
    if (t.beyondPoints && t.beyondPoints.length) {
      html += `<section class="block block-beyond">
                 <h2>Beyond the slides <span class="muted">讲义之外的补充</span>
                     <span class="badge badge-beyond">not on the slides</span></h2>
                 <p class="beyond-warn">The lecture did not put this up. It is standard course material added here to
                    close a gap — reasoning that was spoken, buried in a figure, or left as a link. Worth knowing,
                    but do not cite it as "the slide said so".</p>
                 <ol class="points">`;
    ...
    if (t.terms && t.terms.length) {
      ...
      t.terms.forEach((term, i) => {
        ...
        html += `<div class="term${m ? ' hl-' + m : ''}" data-markable>
                   ...
                   <div class="term-def">
                     ${state.lang !== 'cn' && term.def_en ? `<div class="t-en">${esc(term.def_en)} ${speakBtn(term.def_en, 'en-US')}</div>` : ''}
                     ${state.lang !== 'en' && term.def_cn ? `<div class="t-cn">${esc(term.def_cn)} ${speakBtn(term.def_cn, 'zh-CN')}</div>` : ''}
                   </div>
                 </div>`;
      });
      html += `</div></section>`;
    ...
    switch (r.kind) {
      ...
      case 'must':
        return `<div class="must-know-body">${hiParas(d.mustKnow, tk)}</div>`
          + (d.skipIt ? `<p class="skip-it"><span class="skip-label">不用背</span>${hi(d.skipIt, tk)}</p>` : '');
      case 'node':
  ...
  function init() {
    ...
    $('#import-v1').addEventListener('click', () => {
      const status = $('#import-v1-status');
      const moved = importV1();
      if (!moved.length) { status.textContent = 'nothing saved under pesb.'; return; }
      status.textContent = `copied ${moved.join(', ')} — reloading`;
