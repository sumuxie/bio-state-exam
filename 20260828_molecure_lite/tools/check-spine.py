...
# This script lives one level deeper than the biochemie_lite original
# (<app>/tools/ rather than bio-state-exam/tools/), so parent.parent is the APP,
# not the repo. Getting this wrong is silent: the spine glob matches nothing and the
# linter prints "no problems" over zero files, which reads exactly like a pass.
APP = pathlib.Path(__file__).resolve().parent.parent
ROOT = APP.parent
LITE = APP
TOPICS = ROOT / "tools" / "topics.json"

# The shared glossary is BIOCHEMISTRY vocabulary (biochemie_pro's own 3,060 terms).
# It carries almost none of this app's subject, so the COLD check would go quiet on
# exactly the nouns most likely to be dropped on the reader unintroduced -- the whole
# reason these sections were selected in the first place was that no existing node
# teaches them. This list is the molecular-biology half, and it is a floor, not a
# ceiling: add to it whenever a spine introduces a term the linter did not know.
EXTRA_TERMS = """
CRISPR Cas9 Cas12 crRNA tracrRNA sgRNA gRNA PAM protospacer RuvC HNH TALEN
zinc finger nuclease ZFN NHEJ homologous recombination donor template knock-in
knockout knockdown gene targeting ES cell Cre LoxP FLP FRT neo HSV-tk
positive-negative selection gene trap T-DNA Ti plasmid Agrobacterium Vir region
two-hybrid one-hybrid three-hybrid GAL4 UAS LexA activation domain binding domain
pull-down co-immunoprecipitation EMSA gel shift DNase I footprinting ChIP RIP
reporter gene lacZ HIS3 GST tag His-tag
histone acetylation histone methylation HAT HDAC HMT bromodomain chromodomain
histone code nucleosome chromatin remodelling SWI/SNF heterochromatin euchromatin
DNA methylation CpG island DNMT1 DNMT3a MeCP2 5-methylcytosine demethylation
epigenetics imprinting X-inactivation Xist HOTAIR H3K4 H3K9 H3K27 H3K36
noncoding RNA ncRNA miRNA siRNA lncRNA lincRNA circRNA piRNA snoRNA
RNA interference RNAi Drosha Dicer DGCR8 RISC Argonaute AGO2 Exportin-5
pri-miRNA pre-miRNA seed sequence miRNA sponge back-splicing antisense RNA
transposon transposase IS element insertion sequence retrotransposon LTR
inverted repeat target site duplication Ac/Ds Tn3 composite transposon
RNA editing recoding ADAR APOBEC deaminase guide RNA kinetoplastid
pseudouridine m6A m7G cap poly(A) mRNA export nuclear pore
nif gene nifA nifL NtrC sigma-54 nitrogenase glnA two-component system
enhancer silencer insulator mediator transcription factor cis-acting trans-acting
alternative splicing spliceosome IRE IRP eIF2 mTOR ubiquitin proteasome PEST
"""


def extra_terms():
    """One term per whitespace-separated run, plus the multi-word phrases."""
    out = set()
    for line in EXTRA_TERMS.strip().splitlines():
        line = line.strip()
        if not line:
            continue
        for w in line.split():
            if 3 < len(w) < 40:
                out.add(w.lower())
    for phrase in ("zinc finger nuclease", "gene targeting", "ES cell",
                   "positive-negative selection", "gene trap", "Ti plasmid",
                   "Vir region", "activation domain", "binding domain",
                   "gel shift", "reporter gene", "histone code",
                   "chromatin remodelling", "CpG island", "DNA methylation",
                   "histone acetylation", "histone methylation",
                   "noncoding RNA", "RNA interference", "seed sequence",
                   "miRNA sponge", "antisense RNA", "IS element",
                   "insertion sequence", "inverted repeat",
                   "target site duplication", "composite transposon",
                   "RNA editing", "guide RNA", "mRNA export", "nuclear pore",
                   "nif gene", "two-component system", "alternative splicing",
...
def main():
    ...
    # A glob that matches nothing prints "no problems" over zero files, which reads
    # exactly like a pass and is how a wrong path stayed invisible once already.
    # Refuse to report success on an empty run.
    files = sorted((LITE / "data").glob("spine_*.js"))
    if not files:
        sys.exit(f"NO SPINE FILES under {LITE / 'data'} — fix the path before "
                 f"trusting any verdict from this script.")
    if not spines and not bad:
        sys.exit(f"{len(files)} spine file(s) found under {LITE / 'data'} but 0 spines "
                 f"parsed out of them. The node format is not being recognised; a "
                 f"clean report here would be a lie.")
    if bad:
        for f, nid, e in bad:
    ...
    if idx.exists():
        ...
        if m:
            body = m.group(1)
            # biochemie_lite's nodes.js is generated and is bare JSON. This one is
            # written by hand and carries /* ... */ blocks between the groups saying
            # why each section was selected, which json.loads cannot read. Strip them
            # -- but only after checking no string literal contains a comment opener,
            # so the strip can never eat content.
            assert '/*' not in re.sub(r"/\*.*?\*/", "", body, flags=re.S),                 "a string in nodes.js contains '/*'; the comment strip would corrupt it"
            js = json.loads(re.sub(r"/\*.*?\*/", "", body, flags=re.S))
