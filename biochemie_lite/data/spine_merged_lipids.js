...
window.BIOLITE_SPINE["key:storage-lipids"] = {
  ...
  steps: [
    ...
    {
      link_en: "that scatter of jobs is worth counting rather than admiring, and the count is four",
      link_cn: "那一堆五花八门的活儿，与其感叹不如数一数 —— 数出来是四件",
      en: "The main biological functions of lipids come to four, and each number on that list is a different kind of job. One, lipids are the store of energy and of carbon; two, lipids are the chief structural material of every biological membrane. **Three, lipids are precursors of other important substances — vitamins, hormones and regulatory substances. Four, lipids form an insulating barrier against thermal, electrical and other physical shock.**",
      cn: "脂质的主要生物学功能一共四件，而这份清单上的每个编号都是一类不同的活儿。第一，脂质是能量和碳的储存形式；第二，脂质是一切生物膜的主要结构材料。**第三，脂质是其他重要物质的前体 —— 维生素、激素、调节物质。第四，脂质构成抵御热、电及其他物理冲击的隔离屏障。**",
      src: "CZ p.176, §8.1"
    },
    {
    ...
    {
      link_en: "one entry in that table is printed wrong, and it is flagged here rather than quietly fixed",
      link_cn: "那张表里有一处印错了 —— 这里把它标出来，而不是偷偷改掉",
      en: "Row 4 of that table, the phosphoacylglycerol row, prints one of the organic head groups as etnanolamin, and a zoomed re-read of the scan confirms that is exactly what the page shows. **The printed etnanolamin is almost certainly a setting error for ethanolamin, ethanolamine, and it is reproduced here as printed rather than silently corrected.** Ethanolamine is one of the standard head groups of a phosphoacylglycerol — the head group of phosphatidylethanolamine — so nothing about the chemistry is in doubt, only the spelling on that one line. Heading 8.2 immediately below carries no independent text of its own; the writing moves directly into subsection 8.2.1, so the chemical composition and properties of lipids begin there and not under the heading that names them.",
      cn: "那张表的第 4 行，也就是磷酸酰基甘油那一行，把其中一个有机头基印成了 etnanolamin；放大原扫描件重看，确系原文如此。**印出来的 etnanolamin 几乎可以肯定是 ethanolamin（乙醇胺）的排印错误，此处按原文照录并加标注，而不是默默改正。**乙醇胺是磷酸酰基甘油的标准头基之一 —— 磷脂酰乙醇胺的那个头基 —— 所以化学上没有任何疑问，可疑的只是那一行的拼写。紧接其下的标题 8.2 本身没有独立正文；文字直接进入 8.2.1 小节，因此「脂质的化学组成与性质」是从那里开始讲的，而不是在那个给它命名的标题底下。",
      src: "CZ p.176, Tab. 8.1 · CZ p.177, §8.2"
    },
    {
      link_en: "one piece recurs in nearly every one of those classes, so start there",
      link_cn: "上面几乎每一类里都反复出现同一个零件，就从它开始",
      en: "A fatty acid is a carboxylic acid carrying a hydrocarbon chain of 4 to 36 carbons — chemically, a many-carbon aliphatic monocarboxylic acid. It is the hydrophobic spine of a lipid, and its pronounced hydrophobicity is what makes the whole lipid molecule hydrophobic. Most fatty acids are unbranched and fully saturated, meaning no double bond anywhere in the chain; some carry one double bond; some carry several; a few contain three-carbon rings, hydroxyl groups or methyl branches. The ones actually common in nature are unbranched with an even number of carbons between 12 and 24, called the higher fatty acids. **The chain is built two carbons at a time from acetyl units, which is why a natural fatty acid comes out with an even number of carbons and can hardly do otherwise.** Two more regularities: in a fatty acid with a single double bond, that bond usually lies between C-9 and C-10; and in one with several, the double bonds are almost never conjugated but separated by a CH2 group, giving CH=CH-CH2-CH=CH rather than CH=CH-CH=CH, sitting roughly symmetrically about the middle of the chain and almost always in the cis geometry. The name records where these acids were first got from: mastná kyselina, the name this book uses for a fatty acid, originates from its principal source, tuk, which is fat, and mastný means fatty.",
      cn: "脂肪酸 (fatty acid) 是带着一条 4 到 36 个碳的碳氢链的羧酸 —— 化学上说，就是多碳的脂肪族一元羧酸。它是脂类的疏水脊梁，正是它显著的疏水性让整个脂类分子疏水。大多数脂肪酸不分支、完全饱和（链上没有任何双键）；有些带一个双键，有些带好几个；少数含三碳环、羟基或甲基分支。自然界真正常见的那些，是碳数为偶数、长度在 12 到 24 之间的不分支链，叫高级脂肪酸 (higher fatty acids)。**碳链是由乙酰单元一次两个碳搭起来的，所以天然脂肪酸的碳数只能是偶数，几乎没有别的可能。**另外两条规律：只有一个双键的脂肪酸，那个双键通常落在 C-9 与 C-10 之间；有好几个双键的，双键几乎从不共轭，而是被一个 CH2 隔开，形成 CH=CH-CH2-CH=CH 而非 CH=CH-CH=CH，大致对称地分布在链的中段，并且几乎总是顺式。这个名字本身记着这些酸最早是从哪里来的：mastná kyselina —— 本书用来指脂肪酸的那个名称 —— 源自它的主要来源 tuk（脂肪），而 mastný 就是「含脂肪的」。",
    ...
    },
    {
      ...
      en: "A polyunsaturated fatty acid, PUFA for short, is one carrying more than a single double bond. **Two numbering systems exist because what a PUFA does in the body tracks how near its first double bond lies to the methyl end, not to the carboxyl end**, and the two ends are counted in opposite directions. The second count starts at the methyl carbon, the one furthest from the carboxyl group, and calls it 1; that carbon is the omega carbon, written n- in current notation and omega- in the older one. A PUFA whose first double bond falls between C-3 and C-4 of that count is an omega-3 or n-3 acid; between C-6 and C-7, an omega-6 or n-6. Acids sharing a first-double-bond position are spoken of as a series. Eicosapentaenoic acid is the worked example: 20:5(Delta 5,8,11,14,17) in the first convention, n-3 in the second. Others to have: oleic 18:1 Delta 9 is n-9, linoleic 18:2 Delta 9,12 is n-6, alpha-linolenic 18:3 Delta 9,12,15 is n-3, gamma-linolenic 18:3 Delta 6,9,12 is n-6, and arachidonic 20:4 Delta 5,8,11,14 is n-6. Three more complete the table: palmitoleic 16:1 Delta 9 is n-7, the only n-7 on the list; erucic 22:1 Delta 13 is n-9, the same series as oleic acid at a longer chain; and docosahexaenoic acid, DHA, 22:6 Delta 4,7,10,13,16,19, is n-3, the longest and most unsaturated entry there is.",
      cn: "多不饱和脂肪酸 (polyunsaturated fatty acid, PUFA) 指含一个以上双键的脂肪酸。**两套编号之所以并存，是因为一个 PUFA 在体内干什么，取决于它的第一个双键离甲基端有多近、而不是离羧基端有多近，而这两头是从相反方向数的。**第二套从甲基碳（离羧基最远的那个碳）数起，把它记为 1；这个碳就是 omega 碳，现行写法记作 n-，旧写法记作 ω-。第一个双键落在这套编号 C-3 与 C-4 之间的 PUFA 是 omega-3（n-3）脂肪酸；落在 C-6 与 C-7 之间的是 omega-6（n-6）。第一个双键位置相同的一群酸，被称为一个系列 (series)。例子是二十碳五烯酸：第一套写作 20:5(Delta 5,8,11,14,17)，第二套里它是 n-3。其余要记的：油酸 18:1 Delta 9 是 n-9，亚油酸 18:2 Delta 9,12 是 n-6，α-亚麻酸 18:3 Delta 9,12,15 是 n-3，γ-亚麻酸 18:3 Delta 6,9,12 是 n-6，花生四烯酸 20:4 Delta 5,8,11,14 是 n-6。还有三个把这张表补全：棕榈油酸 16:1 Delta 9 是 n-7，表上唯一的 n-7；芥酸 22:1 Delta 13 是 n-9，和油酸同一个系列，只是链更长；二十二碳六烯酸 DHA，22:6 Delta 4,7,10,13,16,19，是 n-3，也是表上最长、双键最多的一个。",
    ...
    {
      link_en: "and that chiral centre has a handedness, which nature almost always picks the same way",
      link_cn: "而那个手性中心是有左右手的，自然界几乎总是挑同一只",
      en: "Acylation of glycerol is set out as a sequence rather than as one event: glycerol, its three carbons numbered C-1, C-2 and C-3 with an H-C-OH at C-2, becomes 1-acyl-L-glycerol carrying one ester at C-1 and one acyl group R, then 1,2-diacyl-L-glycerol with esters at C-1 and C-2 and a second acyl group R', then triacyl-L-glycerol with all three carbons esterified and acyl groups R, R' and R''. **Natural lipids are mostly the L-isomers, so the chiral centre at C-2 is not filled at random — the same handedness recurs from the monoacyl stage all the way through to the finished fat.**",
      cn: "甘油的酰化是被摆成一个序列、而不是一次事件的：甘油的三个碳编作 C-1、C-2、C-3，C-2 上是 H-C-OH；先成 1-酰基-L-甘油，C-1 上一个酯、一个酰基 R；再成 1,2-二酰基-L-甘油，C-1 与 C-2 上各一个酯，多出第二个酰基 R'；最后成三酰基-L-甘油，三个碳全部成酯，酰基分别是 R、R'、R''。**天然脂质大多是 L 型异构体，所以 C-2 上那个手性中心并不是随机填的 —— 同一只手，从单酰阶段一直贯穿到成品脂肪。**",
      src: "CZ p.178–179, §8.2.2"
    },
    {
    ...
    },
    {
      ...
      en: "A wax, also called a ceride, is an ester of a long-chain fatty acid (C14 to C36) with a long-chain alcohol (C16 to C30), and it melts at 50 to 100 °C, higher than any triacylglycerol. Beeswax is myricyl palmitate: palmitic acid, 16:0, esterified to the C30 alcohol myricyl alcohol, also called triacontanol. Other wax acids are lignoceric C24, cerotic C26 and myricic C30. Waxes belong with the storage lipids because in plankton, the free-floating microorganisms at the bottom of the marine food chain, wax is the chief storage form of metabolic fuel. **A wax is indigestible to an animal, because an animal's lipases cannot attack that ester bond at all; only the cerases, hydrolases of plants and microorganisms, can cleave it.** Waxes are very widely distributed in nature, and most often they occur as end metabolites — the last thing a carbon skeleton is turned into, rather than an intermediate on the way to something else. Their shared job is protective in two senses: a wax guards against drying out, and it reinforces the surface of cells and of whole organs and tissues, plant tissues included. Complete insolubility in water and great stability to hydrolysis also make a wax the natural material for a boundary between wet and dry: vertebrate skin glands secrete waxes that keep hair and skin pliable and waterproof, waterfowl spread wax from their preen glands over their feathers, and the shiny leaves and fruit of holly, rhododendron and many tropical plants carry a wax layer that stops water evaporating and keeps parasites out. The same properties are why four named waxes end up in lotions, ointments and polishes: lanolin, which is the wax of sheep's wool, beeswax, carnauba wax from a Brazilian palm, and spermaceti from the whale.",
      cn: "蜡 (wax)，也叫 cerid（ceride），是长链脂肪酸（C14 到 C36）与长链醇（C16 到 C30）形成的酯，熔点 50 到 100 °C，比任何三酰甘油都高。蜂蜡是蜂花酸酯 (myricyl palmitate)：棕榈酸 16:0 与 C30 的醇蜂花醇（也叫三十烷醇）酯化而成。蜡里其他常见的酸有：木蜡酸 C24、蜡酸 C26、蜂花酸 C30。蜡之所以归入储存脂类，是因为在浮游生物 —— 海洋食物链底端那些自由漂浮的微生物 —— 中，蜡是代谢燃料的主要储存形式。**蜡对动物是不可消化的，因为动物的脂肪酶根本啃不动那种酯键；只有植物和微生物的蜡酶 (cerase) 这类水解酶才劈得开它。**蜡在自然界分布极广，而且多半是以最终代谢产物的身份出现 —— 也就是一副碳骨架最后被变成的东西，而不是通往别处的中间体。它们共同的活儿是保护，分两层意思：蜡防止水分流失，也加固细胞以及整个器官、组织（包括植物组织）的表面。完全不溶于水、又极耐水解，这两点还让蜡成为「干湿分界」的天然材料：脊椎动物的皮肤腺分泌蜡，使毛发与皮肤柔韧防水；水禽从尾脂腺取蜡涂抹羽毛；冬青、杜鹃和许多热带植物油亮的叶子与果实上覆着一层蜡，既阻止水分蒸发，也挡住寄生物。正是这些性质，让四种有名字的蜡进了乳液、软膏和上光剂：羊毛脂 (lanolin)，即绵羊羊毛上的蜡；蜂蜡；来自巴西棕榈的巴西棕榈蜡；以及取自鲸的鲸蜡。",
      src: "A p.346, figure 10-5 · CZ p.179, §8.2.2"
    },
    {
      link_en: "a wax has two halves and only one of them has been named so far",
      link_cn: "蜡有两半，到这里为止只有一半被点过名",
      en: "An ester is made of an acid and an alcohol, so the alcohols found in waxes are the other half of the story, and there are three of them: cetyl alcohol, ceryl alcohol at C22, and myricyl alcohol at C30, which is the one already met esterified to palmitic acid in beeswax. **The book prints cetyl alcohol as C11, which is flagged here rather than corrected: cetyl alcohol is normally hexadecanol, C16, so that printed figure is a probable print error.** Set the two lists side by side and the size range of a wax is plain — acids from C14 to C36 against alcohols from C16 to C30 — which is why a wax molecule is roughly twice the length of a fatty acid and packs into a solid that only heat above 50 °C will move.",
      cn: "酯由一个酸和一个醇构成，所以蜡里的醇是这个故事的另一半，一共三个：鲸蜡醇、蜂蜡醇（C22）、蜂花醇（C30）—— 最后这个前面已经见过，就是在蜂蜡里与棕榈酸酯化的那个。**教材把鲸蜡醇印成 C11，这里把它标出来而不是径直改掉：鲸蜡醇通常是十六醇 C16，所以那个印出来的数字很可能是印刷错误。**把两份清单并排看，蜡的尺度就一目了然 —— 酸从 C14 到 C36，醇从 C16 到 C30 —— 这正是蜡分子长度约为脂肪酸两倍、并且要 50 °C 以上的热才推得动的原因。",
      src: "CZ p.179, §8.2.2 · A p.346"
    }
  ],
  terms: [
    { en: "lipid", cn: "脂类",
...
window.BIOLITE_SPINE["key:membrane-lipids"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "A glycoacylglycerol is a 1,2-diacylglycerol carrying a sugar joined by a glycosidic linkage to C-3 and no phosphate anywhere in it. Three are named, and they are the main lipid of the thylakoid membrane, which is the chloroplast membrane system that holds the photosynthetic apparatus: monogalactosyldiacylglycerol (MGDG) with one galactose, digalactosyldiacylglycerol (DGDG) with two stacked galactoses, and sulfoquinovosyldiacylglycerol (SL), whose head is a sugar carrying a CH2-SO3− group — a sulfonate, negatively charged just as a phosphate would be. **Galactolipids make up 70 to 80 per cent of all the membrane lipid of a vascular plant, so the most abundant membrane lipid on this planet is a sugar lipid with no phosphorus in it.** The suggested reason is ecological: phosphate is often the limiting nutrient for a plant in soil, so pressure to save phosphate for more critical roles would favour plants that made phosphate-free lipids. With MGDG and DGDG neutral and SL anionic, a plant has replaced both the neutral head group and the charged one and still spent no phosphate. The tails are as distinctive as the heads: MGDG is drawn in two variants, one carrying 16:3 at one position and 18:3 at the other and one carrying 18:3 at both, so a galactolipid of a thylakoid membrane runs on chains carrying three double bonds each — about as heavily unsaturated as any tail named anywhere in this chain.",
      cn: "糖酰基甘油 (glycoacylglycerol) 是一个 1,2-二酰甘油，在 C-3 上以糖苷键连着一个糖，整个分子不含磷酸。有三个有名字的，它们是类囊体膜（叶绿体中安放光合装置的那套膜系统）的主要脂类：MGDG（单半乳糖基二酰甘油），带一个半乳糖；DGDG（双半乳糖基二酰甘油），带两个叠起来的半乳糖；以及 SL（磺基奎诺糖基二酰甘油），它的头是一个带 CH2-SO3− 的糖 —— 磺酸基，像磷酸一样带负电。**半乳糖脂占维管植物全部膜脂的 70% 到 80%，所以这颗星球上含量最丰富的膜脂，是一种一个磷原子都没有的糖脂。**给出的理由是生态学的：土壤中磷酸常常是植物的限制性营养，因此「把磷酸省下来用于更关键角色」的压力，会让能造无磷脂类的植物占优。MGDG 和 DGDG 是中性的、SL 是阴离子的 —— 植物把中性头基和带电头基都替换掉了，而依然一点磷酸也没花。尾巴和头基一样有特点：MGDG 画出来有两种变体，一种是一个位置 16:3、另一个位置 18:3，另一种两个位置都是 18:3 —— 所以类囊体膜上的半乳糖脂，跑的是每条各带三个双键的链，其不饱和程度在这条链里点过名的所有尾巴中几乎是最高的。",
...
window.BIOLITE_SPINE["key:lipid-signals-and-pigments"] = {
  ...
  steps: [
    ...
    {
      link_en: "and the second inositol lipid works by a completely different logic, which is what makes the pair worth contrasting",
      link_cn: "而第二个肌醇脂质走的是完全不同的逻辑 —— 这正是把两者对着讲最值钱的地方",
      recall_en: "PIP2, cut into two messengers in the step above",
      recall_cn: "上一步里被切成两个信使的那个 PIP2",
      en: "Inositol phospholipids also serve as points of nucleation for supramolecular complexes involved in signaling or in exocytosis, exocytosis being the release of material by fusing a vesicle with the plasma membrane. Certain signalling proteins bind specifically to phosphatidylinositol 3,4,5-trisphosphate, PIP3, initiating the formation of multienzyme complexes at the membrane's cytosolic surface, so making PIP3 in answer to an extracellular signal brings those proteins together into signalling complexes there. **Put the two side by side: PIP2 is consumed — cut, its fragments the message — while PIP3 is not consumed but recognised, the message being that a set of proteins now has an address to assemble at.** One is a chemical signal, the other a physical scaffold, and both start from the same phosphatidylinositol skeleton, differing only in which of the inositol hydroxyls have been phosphorylated.",
      cn: "肌醇磷脂还充当「参与信号传导或胞吐的超分子复合物的成核位点」—— 胞吐（exocytosis）就是把囊泡与质膜融合、从而把内容物释放出去。某些信号蛋白特异地结合磷脂酰肌醇 3,4,5-三磷酸（PIP3），从而在膜的胞质面上启动多酶复合物的组装；于是「应胞外信号而生成 PIP3」这件事，就把这批蛋白聚集成了那里的信号复合物。**把两者并排放：PIP2 是被消耗掉的 —— 它被切开，而它的碎片就是消息本身；PIP3 完全不被消耗，它是被「认出来」的，而消息是「一批蛋白现在有了一个集合地址」。** 一个是化学信号，另一个是物理支架；两者都从同一个 phosphatidylinositol 骨架出发，区别只在肌醇的哪几个羟基被磷酸化了。",
      src: "A pp.354–355, figure 10-16"
    },
    {
      link_en: "which raises a small practical problem, since those position numbers are the only thing telling the two apart",
      link_cn: "这就带出一个很实际的小麻烦：区分两者的，全靠那几个位置编号",
      en: "Inositol is a six-carbon ring carrying a hydroxyl on every carbon, and which hydroxyls bear phosphates is the whole difference between the messenger that gets cut and the scaffold that gets read — so the numbering is not decoration, and it is otherwise arbitrary. A deliberate mnemonic is supplied for it. **Compare the chair conformation of inositol to a turtle: begin numbering with C-1 as the right front flipper and move counterclockwise around the structure, with C-2 the head and C-5 the tail.** Hold the turtle and 4,5-bisphosphate against 3,4,5-trisphosphate stops being two strings of digits.",
      cn: "肌醇是一个六碳环，每个碳上都带一个羟基；而哪几个羟基上挂了磷酸，正是「被切开的那个信使」与「被读取的那个支架」之间的全部差别 —— 所以这套编号不是装饰，可它本身又毫无道理可讲。为此专门配了一个记忆法。**把肌醇的椅式构象比作一只乌龟：从 C-1 开始编号，它是右前肢，然后沿结构逆时针走，C-2 是头，C-5 是尾巴。** 记住这只乌龟，「4,5-二磷酸」与「3,4,5-三磷酸」就不再是两串数字了。",
      src: "A p.355, figure 10-16"
    },
    {
    ...
    {
      link_en: "and one question is still open: what becomes of a plant carotenoid once an animal has eaten it",
      link_cn: "还剩一个问题没答：动物把植物的类胡萝卜素吃下去以后，它们变成了什么",
      recall_en: "the symmetric cleavage to retinal in the step above is only one of the three fates",
      recall_cn: "上一步说的「对称裂解成视黄醛」只是三条去向里的一条",
      en: "After ingestion by an animal, plant carotenoids go three ways. Part is excreted unchanged, having passed through without being touched. Part is stored in fat and nervous tissue, where it stands as a provitamin-A reserve — the store that the cleavage to retinal draws on later. **And some are bound to proteins, as chromoproteins in blood serum, where the antioxidant properties are again prominent and anticarcinogenic effects have been demonstrated.** So the same molecule is a pigment in a plant, a vitamin reserve in fat, and an antioxidant riding a serum protein, without any change to its chemistry.",
      cn: "动物摄入植物的类胡萝卜素之后，它们有三条去向。一部分以原形排出体外，一路穿过而未被动过。一部分储存在脂肪和神经组织里，作为维生素 A 原的储备 —— 后来那次「裂解成视黄醛」取用的就是这份库存。**还有一些与蛋白结合，在血清中形成色蛋白 (chromoprotein)；在那里，抗氧化特性同样显著，而抗癌作用也已被证实。** 于是同一个分子在植物里是色素，在脂肪里是维生素储备，在血清蛋白上是抗氧化剂 —— 而它的化学一点没变。",
      src: "CZ p.187, §8.2.4"
    }
  ],
  terms: [
