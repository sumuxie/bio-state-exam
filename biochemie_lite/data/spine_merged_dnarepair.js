...
window.BIOLITE_SPINE["key:dna-repair"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "In bacteria, replicating past an unreadable lesion belongs to a stress response to heavy DNA damage called the SOS response, in which 40 or more proteins are induced; two of them, UmuC and UmuD, exist for this purpose — umu for unmutable, since deleting the gene removes error-prone repair altogether — and UmuD is cut down to a shorter form that joins UmuC and RecA to make DNA polymerase V, alongside a second induced and equally error-prone enzyme, DNA polymerase IV. **A translesion polymerase has no proofreading exonuclease and an open active site able to hold a damaged template base — both required by the job, and both the source of its 1-in-1,000 error rate.** A tight, shape-selective site would reject the lesion and stall, which is the very thing such a polymerase exists to avoid, and a loose site cannot discriminate correct incoming nucleotides either. This is a desperation strategy: UmuC and UmuD are fully induced only late in the response and only when damage is heavy and every replication fork is blocked, and the mutations kill some cells and damage others. Eukaryotes contain the same risk differently, giving their low-fidelity polymerases specialised repair jobs and only a few nucleotides to write at a time.",
      cn: "在细菌里，「在读不出模板的情况下复制过去」属于一套应对大量 DNA 损伤的应激反应，叫 SOS 反应，其中有 40 多个蛋白被诱导；其中两个 —— UmuC 与 UmuD —— 就是为此而存在的（umu 取自 unmutable「不可突变」：把这个基因删掉，易错修复就整个消失），UmuD 被剪短成一个较短的形式，与 UmuC 和 RecA 组成 DNA 聚合酶 V；另有一个同样被诱导、同样高度易错的酶，DNA 聚合酶 IV。**跨损伤聚合酶没有校对外切酶，活性中心更敞开、能容纳一个受损的模板碱基 —— 这两点都是这份工作要求的，也都是它约千分之一错误率的来源。**一个紧致的、按形状挑选的活性中心会直接拒绝这个损伤并停摆，而那恰恰是这类聚合酶存在的目的所要避免的；可是敞开的活性中心也就分辨不出进来的核苷酸对不对了。这是一种孤注一掷的策略：UmuC 与 UmuD 只在反应晚期才被完全诱导，而且只有在损伤严重、所有复制叉都被堵住时才启用，由此产生的突变会杀死一部分细胞、给另一部分留下有害突变。真核细胞用另一种办法控制同样的风险：让那些低保真度的聚合酶各有专门的修复任务，而且每次只写几个核苷酸。",
      src: "A p.939–940",
      see: [{ id: "L-25-3-1", en: "the accurate avenue, taken in full", cn: "另一条精确的路，完整走一遍" }]
    },
    {
      link_en: "and stepping back from the mechanisms, the ratio they add up to is the argument for all of it",
      link_cn: "从各个机制退开一步，它们加起来得到的那个比值，才是这一切的论据",
      en: "A damaged protein or RNA is thrown away and remade from the gene, so the cell loses nothing but the work. A DNA molecule has no higher copy to be read back from, which is why maintaining the integrity of its information is an imperative rather than a preference. **The genomic DNA of a typical mammalian cell accumulates many thousands of lesions in 24 hours, and fewer than 1 in 1,000 of them becomes a mutation.** Read that ratio carefully, because it says something other than what it first seems to: the cell is not preventing damage, it is tolerating an enormous amount of it and correcting almost all of it afterwards. Damage is continuous and normal; repair is what makes a genome behave as though it were stable.",
      cn: "受损的蛋白质或 RNA 被丢掉，照着基因重造一份，细胞损失的只是那点功夫。而 DNA 分子没有更高一层的拷贝可以读回来——这就是为什么维持它所载信息的完整性是一条绝对律令，而不是一种偏好。**一个典型哺乳动物细胞的基因组 DNA 在 24 小时内积累数以千计的损伤，而其中变成突变的不到千分之一。**这个比值要仔细读，因为它说的和乍看之下不是一回事：细胞并不是在阻止损伤发生，而是在容忍海量损伤、并在事后几乎全部纠正过来。损伤是持续的、正常的；是修复让一个基因组「表现得」像是稳定的。",
      src: "A p.930"
    }
  ],
  terms: [
