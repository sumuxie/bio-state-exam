...
window.BIOLITE_SPINE["key:citric-acid-cycle"] = {
  ...
  steps: [
    {
      en: "Acetyl-CoA is a two-carbon acetyl group carried on coenzyme A through a thioester bond, meaning a bond between an acyl group and a sulfur atom. Sugars, fatty acids and most amino acids all arrive as acetyl-CoA once they have been broken down: from the pyruvate glycolysis makes, from the pyruvate made when alanine hands its amino group to another molecule, and in large amounts from the breakdown of fatty acids. **The citric acid cycle oxidises that acetyl group completely to two molecules of carbon dioxide, and keeps the electrons stripped off it on NAD+ and FAD.** Nine numbered reactions do it, and because citrate carries three carboxyl groups the same pathway is also named the tricarboxylic acid cycle.",
      cn: "乙酰-CoA 是一个两碳的乙酰基，通过硫酯键挂在辅酶 A 上；硫酯键就是酰基与硫原子之间的键。糖、脂肪酸和大多数氨基酸被拆到最后，都以乙酰-CoA 的形式到场：来自糖酵解做出的丙酮酸，来自丙氨酸把氨基交给别的分子之后生成的丙酮酸，以及大量来自脂肪酸的分解。**柠檬酸循环把这个乙酰基彻底氧化成两分子二氧化碳，并把从它身上剥下来的电子留在 NAD+ 和 FAD 上。**做这件事的是九个编号反应；柠檬酸本身带三个羧基，所以这条通路又叫三羧酸循环。",
    ...
    },
    {
      ...
      en: "Succinate dehydrogenase — EC 1.3.99.1 — is embedded in the inner mitochondrial membrane, and in bacteria in the plasma membrane; it is the only cycle enzyme that is. Its FAD is covalently bound, which is unusual, and alongside three iron-sulfur clusters it takes two hydrogens off succinate to give fumarate as the trans isomer, with the electrons running straight into the respiratory chain instead of leaving on a soluble carrier. Those electrons enter the chain lower down than electrons delivered by NADH, which is why this step's FADH2 is worth less ATP than an NADH. **Malonate is a three-carbon diacid resembling succinate, absent from normal cells, that occupies this active site without reacting: add it to isolated mitochondria and the entire cycle stops.**",
      cn: "琥珀酸脱氢酶（EC 1.3.99.1）嵌在线粒体内膜上，细菌中则在质膜上；它是本循环唯一这样的酶。它的 FAD 是共价结合的，这不常见；它连同三个铁硫簇一起，把琥珀酸的两个氢拿掉，生成反式的延胡索酸，而电子直接跑进呼吸链，不经任何可溶载体带走。这些电子进入呼吸链的位置比 NADH 送来的电子低，所以这一步的 FADH2 换得的 ATP 比一个 NADH 少。**丙二酸是一个长得像琥珀酸的三碳二元酸，正常细胞里没有，它占着这个活性位点却不反应：加到分离的线粒体里，整个循环就停摆。**",
