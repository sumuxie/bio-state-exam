...
window.BIOLITE_SPINE["key:membrane-architecture"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "Flip-flop is rare because it means dragging a polar or charged head group out of the water and through the oily core of the bilayer, which is a large positive free-energy change, so at body temperature it happens very slowly if it happens at all. **The slowness of flip-flop maintains the asymmetry of lipid composition between the two leaflets — cerebrosides and gangliosides sit in the outer one — and maintains it for free, by nothing crossing.** How pronounced the asymmetry is varies with the type of membrane, with its function, and even with its age. Under its other name, the překlopný mechanismus, flip-flop is introduced as no more than a mechanism allowing lipids to be exchanged between the two monolayers, with its biological significance stated to be not yet precisely defined — so the next steps, on what the crossing is catalysed for and what it signals, are where that significance gets filled in.",
      cn: "翻转之所以罕见，是因为它意味着把一个极性或带电的头基从水里拽出来、拖过双层中间那层油，这是一个很大的正自由能变化 —— 所以在体温下，它就算发生也极慢。**正是这份慢，维持着两个小叶之间脂类组成的不对称性（asymmetry）—— 脑苷脂和神经节苷脂都待在外层 —— 而且维持它是免费的：靠没有东西跨过去。**不对称到什么程度，随膜的种类、随它的功能、甚至随它的「年龄」而变。翻转还有另一个名字 překlopný mechanismus，在那里它只被描述成「一种允许脂类在两个单层之间交换的机制」，其生物学意义则被说成尚未被精确界定 —— 所以接下来讲的「这次跨越被催化来做什么、又意味着什么」，正是把这份意义补上。",
...
window.BIOLITE_SPINE["key:membrane-transport"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "Passive transport is a substance crossing a membrane by diffusion, from higher to lower concentration, with no energy spent, and it comes in two kinds, simple and facilitated. **Simple diffusion is a lipophilic substance dissolving straight into the membrane lipids and coming out the other side: no protein, no energy, nothing but solubility and a gradient.** Water and small molecules also cross through a micropore inside an integral protein and through transient defects in the bilayer — and a route that runs through a protein is facilitated diffusion by the standard definition, not simple diffusion, which is worth stating, since the three routes are sometimes listed together under one heading. The printed definition contradicts itself inside one sentence, and it is flagged here rather than smoothed over: 'Prosta difuze se uskutecnuje bez jakekoliv ucasti membranovych proteinu, k transportu dochazi primo pres lipidni dvojnou vrstvu, nahodilymi pory v membrane nebo kanalky vytvorenymi integralnimi membranovymi proteiny protinajicimi membranu.' One clause denies protein involvement and the next lists protein channels as a route, and route (B) of the accompanying figure is drawn as a micropore inside integral proteins.",
      cn: "被动转运 (passive transport) 就是物质靠扩散跨膜、从高浓度走向低浓度、不花能量；它分两种：简单的和易化的。**简单扩散 (simple diffusion) 就是亲脂物质直接溶进膜脂、再从另一侧出来：不要蛋白、不要能量，只要溶解度和一个梯度。**水和小分子还会经由整合蛋白内部的微孔、以及双层上一时的缺陷通过 —— 而按通行定义，走蛋白孔道的那条路属于易化扩散、不属于简单扩散；这一点值得说明，因为这三条路有时被并在同一个标题下列出。印出来的定义在同一句话里自相矛盾，此处加以标记而不是抹平：「Prostá difúze se uskutečňuje bez jakékoliv účasti membránových proteinů, k transportu dochází přímo přes lipidní dvojnou vrstvu, nahodilými póry v membráně nebo kanálky vytvořenými integrálními membránovými proteiny protínajícími membránu.」前半句否认膜蛋白参与，后半句又把整合蛋白形成的通道列为途径之一；配图的途径 (B) 画的正是整合蛋白里的微孔。",
    ...
    },
    {
      ...
      cn: "GLUT1 是一个整合膜蛋白，有 12 条跨膜螺旋，其中好几条是两亲性的：一面的极性残基排出中央的亲水孔道，另一面的非极性残基贴着膜脂。**转运让它在 T1（葡萄糖位点只朝外开）与 T2（该位点只朝内开）之间循环，而整个循环里没有任何一处花掉 ATP。**人有 12 个 GLUT 蛋白，组织分布与 Kt 值各不相同；肌肉、脂肪和心脏里的 GLUT4 是受胰岛素控制的那一个 —— 两餐之间约 90% 的它停在细胞内的囊泡里，胰岛素让这些囊泡在几分钟内与质膜融合，把葡萄糖摄取抬高十五倍以上。",
      src: "A pp.388–390, figures 11-32 and 11-33, table 11-1, box 11-1"
    },
    {
      link_en: "and the twelve of them are not twelve copies — the numbers say what each one is for",
      link_cn: "而这十二个并不是十二份复制品 —— 数字说出了每一个是干什么用的",
      en: "Humans have 12 GLUT proteins, each with its own tissues and its own Kt, and the spread of those Kt values is what makes them different jobs rather than copies of one carrier. GLUT4 of muscle, fat and heart has a Kt of 5 mM, right at blood glucose, so it runs loaded whenever glucose is normal. The liver has the opposite job, because it is the tissue that keeps a store of glucose as glycogen — the branched polymer glucose is put away in — and hands it back to the blood between meals. **GLUT2 of the liver has a Kt of 17 mM or more, far above blood glucose, so it never saturates and it carries glucose outward when glycogen breakdown has raised the sugar inside the hepatocyte.**",
      cn: "人有 12 个 GLUT 蛋白，各有各的组织分布、各有各的 Kt；正是这些 Kt 值拉开的差距，让它们成为不同的岗位，而不是同一个载体的复制品。肌肉、脂肪和心脏里的 GLUT4，Kt 是 5 mM，正压在血糖上，所以血糖正常时它一直满载。肝脏的活儿正好相反：它是那个把葡萄糖以糖原 (glycogen) —— 也就是葡萄糖被收起来时的那种支链多聚物 —— 存起来的组织，两餐之间再把糖还给血液。**肝脏的 GLUT2，Kt 达 17 mM 甚至更高，远在血糖之上，所以它从不饱和；而且它把糖往外送 —— 糖原分解抬高了肝细胞内的葡萄糖之后，由它把葡萄糖运出去。**",
      src: "A pp.388–390, table 11-1"
    },
    {
      link_en: "and the insulin-driven parking of GLUT4 is reversible, which is what a clinic can measure",
      link_cn: "而胰岛素驱动的 GLUT4 停放是可逆的 —— 这正是临床能量出来的东西",
      en: "Insulin's effect on GLUT4 is a round trip and not a switch thrown once: the vesicles fuse and uptake rises, and then, when blood glucose has come back to normal, GLUT4 is pulled off the surface again by endocytosis and parked back inside. **Failure to mobilise GLUT4 is exactly what the glucose tolerance test detects, so type 1 diabetes can be read at the level of a single transporter that never reached the surface.** A rare defect at the other end of the same family starves the brain instead: loss of GLUT1 function causes De Vivo disease — seizures together with movement and language disorders — and it is managed with a ketogenic diet, which supplies ketone bodies as a fuel the brain can burn when glucose delivery fails.",
      cn: "胰岛素对 GLUT4 的作用是一次往返，而不是扳一次就完的开关：囊泡融合、摄取上升；等血糖回到正常，GLUT4 又被胞吞作用从表面撤下来，重新停回细胞内。**GLUT4 动员不起来，正是糖耐量试验查出来的东西；于是 1 型糖尿病可以在「一个转运蛋白始终没上到表面」这个层面上被读出来。**而在同一个家族的另一端，一个罕见的缺陷反过来让大脑挨饿：GLUT1 功能丧失会导致 De Vivo 病 —— 癫痫发作，并伴运动与语言障碍 —— 治疗上用生酮饮食，也就是提供酮体，作为葡萄糖供应不上时大脑还能烧的燃料。",
      src: "A pp.389–390, box 11-1"
    },
    {
      link_en: "one phrase, carrier protein, covers two machines that are not alike",
      link_cn: "「载体蛋白」这一个说法，罩着两台并不相像的机器",
      en: "A channel and a transporter differ in three measurable ways. Rate: a channel passes 10⁷ to 10⁸ ions per second, close to the physical limit set by diffusion, while a transporter is far slower, the Na⁺K⁺ ATPase managing about 100 per second. Saturability: a transporter has a maximum rate and saturates like an enzyme, a channel does not. **Gating: a channel has one gate and is open or shut, whereas a transporter has two gates on opposite sides that are never open at once, so it can never let its substrate leak straight through.** A single channel usually stays open only milliseconds, so its activity is measured electrically rather than watched: patch-clamping seals the tip of a fine glass micropipette onto a scrap of membrane holding one or a few channels and records the picoampere current through it — a trace that gives three numbers for one molecule: how often it opens, how long each opening lasts, and how large the current through it is.",
      cn: "通道与载体有三个可测量的差别。速率：通道每秒通过 10⁷ 到 10⁸ 个离子，接近扩散设定的物理极限，而载体慢得多 —— Na⁺K⁺ ATP 酶大约每秒 100 个。可饱和性：载体像酶一样有最大速率、会饱和，通道不会。**门控：通道只有一个门，非开即关；而载体有两个门，分处膜的两侧，且绝不同时打开 —— 所以它永远不可能让底物直接漏过去。**单个通道通常只开几毫秒，所以它的活动不是「看」出来的，而是用电测的：膜片钳把细玻璃微吸管的尖端封在含一个或几个通道的一小片膜上，记录流过的皮安级电流 —— 这条曲线一次给出关于单个分子的三个数：开放的频率、每次开放持续多久、以及流过它的电流有多大。",
    ...
    },
    {
      ...
      en: "In primary active transport the energy comes straight from ATP and the cleavage of a macroergic bond, which is why such pumps are called ATPase pumps. P-type ATPases are a family of at least 70 human cation pumps, and the P stands for phosphorylation: a conserved aspartate on the pump is phosphorylated and then dephosphorylated once per transport cycle, and every member is blocked by vanadate, an ion shaped like the transition state of phosphate transfer. SERCA is the sarco/endoplasmic reticulum Ca²⁺ ATPase, the calcium pump of muscle and of the endoplasmic reticulum, and the two shapes it alternates between are named E1, the one with high affinity for calcium, and E2, the one with low affinity. **SERCA begins in a shape with two high-affinity calcium sites facing the cytosol, and phosphorylating that aspartate flips it into a shape whose same two sites face the lumen with low affinity.** The calcium falls off on the far side, dephosphorylation resets the pump, and two calcium ions move per ATP hydrolysed.",
      cn: "在初级主动转运里，能量直接来自 ATP 与一个高能键的断裂，所以这类泵被叫作 ATP 酶泵。P 型 ATP 酶是一个至少 70 个成员的人类阳离子泵家族，P 代表磷酸化 (phosphorylation)：泵上一个保守的天冬氨酸在每个转运周期里先被磷酸化、再被去磷酸化；家族每个成员都被钒酸盐抑制，那是一个形状像磷酸转移过渡态的离子。SERCA 的全名是肌质网／内质网 Ca²⁺ ATP 酶，也就是肌肉与内质网上的那个钙泵；它来回切换的两个构象各有名字 —— 对钙高亲和力的那个叫 E1，低亲和力的那个叫 E2。**SERCA 起始于这样一个构象：两个高亲和力的钙位点朝向胞质；而把那个天冬氨酸磷酸化，就把它翻成另一个构象 —— 同样这两个位点改朝腔内，亲和力变低。**钙于是在另一侧脱落，去磷酸化让泵复位，每水解一个 ATP 搬两个钙离子。",
...
window.BIOLITE_SPINE["key:working-with-lipids"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "Phospholipids are degraded by hydrolytic enzymes called phospholipases, which break the ester-type bonds inside the molecule, and four types are distinguished by their site of action on a phosphatidylcholine, also called lecithin. **Phospholipase A1 cuts the ester bond at sn-1 and A2 the one at sn-2, each releasing a fatty acid; phospholipase C cuts on the glycerol side of the phosphate and D on the far side, next to the choline.** Reading the four cuts by what each one leaves behind fixes them better than reading them by position: A1 or A2 leaves a lysophospholipid still carrying one chain, C leaves a diacylglycerol with the phosphoryl alcohol coming away as the other half, and D leaves phosphatidic acid with the choline released free. The sn-2 chain is the one drawn with the kink of a double bond, which is a reminder that the two positions of a phosphatidylcholine do not usually carry the same kind of fatty acid.",
      cn: "磷脂由一类叫磷脂酶 (phospholipase) 的水解酶降解，它们断开分子内部的酯型键；按在磷脂酰胆碱（也叫卵磷脂）结构上的作用位点，区分出四种。**磷脂酶 A1 切 sn-1 位的酯键、A2 切 sn-2 位的酯键，各释放一个脂肪酸；磷脂酶 C 切磷酸靠甘油那一侧的键，D 切磷酸另一侧、紧挨胆碱的那根键。**按「各自留下什么」来记这四刀，比按位置记牢得多：A1 或 A2 留下的是还带着一条链的溶血磷脂；C 留下二酰甘油，另一半以磷酰醇的形式离开；D 留下磷脂酸，胆碱被游离出来。图里 sn-2 那条链是画着双键弯折的那条——这提醒人，磷脂酰胆碱的两个位置通常并不挂同一类脂肪酸。",
    ...
    {
      link_en: "two reasons are given for dropping it, and they are not the same kind of reason",
      link_cn: "省掉这一步给了两条理由，而这两条理由不是同一种东西",
      en: "The shotgun route is justified twice over, and the two justifications are of different kinds: one is about throughput, the route being simply faster, and the other is about accuracy. **Every purification step loses material and does not lose it evenly, so a fractionation done before quantification biases the answer in a way that is hard to correct.** Skipping the step removes the bias along with the work, and that argument reaches far beyond lipids. The classical route is kept alongside it for comparison — separate the major classes first by thin-layer chromatography, adsorption chromatography, gas chromatography or HPLC, then determine each lipid's complement of fatty acids by mass spectrometry — and both routes end at the same box, labelled lipidome, because they are answering the same question.",
      cn: "霰弹枪这条路被论证了两次，而这两条理由不是同一种东西：一条讲通量，就是这条路更快；另一条讲准确性。**每一步纯化都会损失物质，而且损失得并不均匀；所以在定量之前做的分级分离，会以一种难以校正的方式让结果产生偏倚。**跳过这一步，偏倚和工作量一起消失 —— 而这个论证的适用范围远远超出脂类。经典路线仍被并排保留以供对照 —— 先用薄层层析、吸附层析、气相色谱或 HPLC 把主要类别分开，再用质谱测定每一种脂类的脂肪酸组成 —— 两条路最后落到同一个方框上，标着「脂质组 (lipidome)」，因为它们回答的是同一个问题。",
      src: "A p.363, figure 10-25"
    },
    {
      link_en: "and a scheme that sorts thousands of molecules needs numbers under the eight letters",
      link_cn: "而一个要给成千上万个分子分类的体系，八个字母底下还得有数字",
      en: "Within each of the eight categories the finer distinctions are numbered classes and subclasses, and the worked example shows how specific that gets: all glycerophosphocholines are GP01, the subgroup carrying two fatty acids in ester linkage is GP0101, and the subgroup with one fatty acid ether-linked at position 1 and one ester-linked at position 2 — the alkyl ether lipids — is GP0102. **Specific fatty acids are then given numbers of their own, so every lipid has a unique 12-character identifier called the LM_ID, and the scheme leaves room for lipid types not yet discovered.** One organising factor behind the eight categories is biosynthetic rather than structural: the prenol lipids, which include the dolichols and vitamins E and K, are grouped together because all of them are built from isoprenyl precursors. Each category comes with its own examples: oleate, stearoyl-CoA and palmitoylcarnitine under FA; di- and triacylglycerols under GL; phosphatidylcholine, phosphatidylserine and phosphatidylethanolamine under GP; sphingomyelin and the ganglioside GM2 under SP; cholesterol, progesterone and the bile acids under ST; farnesol, geraniol, retinal and ubiquinone under PR; lipopolysaccharide under SL; and tetracycline, erythromycin and aflatoxin B1 under PK.",
      cn: "八大类别里面，更细的区分是编号的 class 与 subclass，那个做实的例子显示它能细到什么程度：所有甘油磷酸胆碱都是 GP01，其中两个脂肪酸都以酯键相连的亚组是 GP0101，而 1 位以醚键、2 位以酯键各连一个脂肪酸的那个亚组 —— 也就是烷基醚脂 —— 是 GP0102。**具体的脂肪酸再各自用数字指定，于是每一种脂质都有一个唯一的 12 字符标识符，叫作 LM_ID；这个体系还给「尚未被发现的脂质类型」留了位置。**八大类别背后有一条组织依据不是结构的、而是生物合成的：prenol 脂这一类，包括 dolichol 以及维生素 E 和 K，被归在一起是因为它们都由 isoprenyl 前体建成。每一类还各带自己的例子：FA 类有 oleate、stearoyl-CoA、palmitoylcarnitine；GL 类有二酰甘油与三酰甘油；GP 类有 phosphatidylcholine、phosphatidylserine、phosphatidylethanolamine；SP 类有 sphingomyelin 与神经节苷脂 GM2；ST 类有胆固醇、孕酮与胆汁酸；PR 类有 farnesol、geraniol、retinal、ubiquinone；SL 类有脂多糖；PK 类有四环素、红霉素与黄曲霉毒素 B1。",
      src: "A pp.363–364, table 10-2"
    },
    {
      link_en: "and then comes the admission that makes the whole scheme honest",
      link_cn: "接着是那句让整个体系变得诚实的自认",
      en: "The eight chemical categories do not coincide with the looser classification by biological function used elsewhere, and the mismatch is stated outright: the structural lipids of a membrane include both glycerophospholipids and sphingolipids, which the chemical scheme keeps in two separate categories. Each method of classification has its advantages. **A chemical cut, a functional cut and the simple-complex-derived cut are three different questions asked about the same molecules, not three rival answers to one question.**",
      cn: "八大化学类别，与别处所用的那种较不正式的、按生物学功能划分的分类，并不吻合；而这一点是被明说出来的：膜的结构脂质既包含甘油磷脂也包含鞘脂，而化学体系把它们分在两个类别里。每一种分类方法各有其优点。**化学的切法、功能的切法、以及「单纯／复合／衍生」的切法，是对同一批分子提出的三个不同问题，而不是对同一个问题给出的三个互相竞争的答案。**",
      src: "A p.364, table 10-2"
    },
    {
      link_en: "and what all this weighing and sorting is finally for",
      link_cn: "而这一切的称量与分类，最后是为了什么",
      en: "High-throughput, high-resolution mass spectrometry can provide a quantitative catalogue of all the lipids present in one specific cell type under one particular set of conditions — that catalogue is the lipidome — and of the ways in which the lipidome changes with differentiation, with a disease such as cancer, or with drug treatment. **Notice what kind of claim that is: not a claim about function but a claim about a measurable quantity that changes, and it is the changes that will eventually suggest what the functions are.**",
      cn: "高通量、高分辨的质谱，可以给出某一特定细胞类型在某一特定条件下所含全部脂类的定量目录 —— 这份目录就是脂质组 (lipidome) —— 以及脂质组如何随分化、随癌症之类的疾病、或随药物处理而改变。**注意这是哪一类主张：它不是关于功能的主张，而是关于「一个会变化的可测量量」的主张；而最终提示功能是什么的，正是那些变化。**",
      src: "A p.364"
    }
  ],
  terms: [
