...
window.BIOLITE_SPINE["key:nitrogen-metabolism"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "Nitrite reductase carries out the second step, a six-electron reduction of nitrite straight to NH4+, taking its electrons one at a time from ferredoxin through a 4Fe-4S centre and then a heme-like cofactor called siroheme. Six electrons demands an unusually strong reductant, one of very low reduction potential, and where that reductant can be found decides where the step can run. **In a leaf it is ferredoxin reduced by the light reactions of photosynthesis, so nitrogen assimilation is a direct consumer of light; in a root the same job is paid for out of respiration.** Nitrite reduction can occur in the non-photosynthesizing parts of a plant, the roots above all, and there it is evidently a process coupled with respiration, since respiration is where the reducing power must come from in a tissue that never sees light. In plants the enzyme sits in the chloroplast, next to the light reactions that feed it; non-photosynthetic microorganisms use a different nitrite reductase that takes NADPH as its donor.",
      cn: "第二步由亚硝酸还原酶完成：把亚硝酸盐一次六电子直接还原成 NH₄⁺，电子由铁氧还蛋白一次一个地供给，经一个 4Fe-4S 中心，再经一个类血红素的辅因子 siroheme。六个电子要求一个特别强、还原电位极低的还原剂，而这个还原剂能从哪里来，就决定了这一步能在哪里跑。**在叶片里，它是被光反应还原的铁氧还蛋白，所以氮同化是光合作用的直接消费者；在根里，同样这件活儿由呼吸作用买单。** 亚硝酸盐还原可以发生在植物的非光合部位，首先是根；在那里它显然是一个与呼吸作用相偶联的过程 —— 因为在一个从不见光的组织里，还原力只能由呼吸提供。在植物中这个酶位于叶绿体，就挨着供给它的光反应；非光合微生物用的是另一种亚硝酸还原酶，供体是 NADPH。",
    ...
    },
    {
      ...
      en: "The equation is worth memorising whole: N2 + 10 H+ + 8 e− + 16 ATP → 2 NH4+ + 16 ADP + 16 Pi + H2. **Eight electrons rather than six, because six of them reduce N2 while the other two make one molecule of H2, and releasing that H2 is an obligatory part of the mechanism.** Sixteen ATP, because the ATP arithmetic follows from the transfer geometry: the electrons are handed over one at a time, a reduced reductase binding, passing a single electron, then dissociating, and each turn of that cycle requires the hydrolysis of two ATP by the dimeric reductase — eight cycles at two ATP each. The summary of the cost is blunt: nitrogen fixation is energetically costly, and 16 ATP and 8 electrons yield only 2 NH3. A simpler stoichiometry is still widely printed and still examined, N2 + 6 H+ + 6 e− → 2 NH3 at one ATP per electron and therefore 6 ATP, with no H2 among the products, and it is internally consistent; the modern value differs from it and is written here with ammonium rather than ammonia. What settles the disagreement is the obligatory hydrogen evolution: H2 comes off every time nitrogen is fixed, and a six-electron equation has nothing to make it out of.",
      cn: "这条方程值得整条背下来：N₂ + 10 H⁺ + 8 e⁻ + 16 ATP → 2 NH₄⁺ + 16 ADP + 16 Pᵢ + H₂。**为什么是八个电子而不是六个：六个用来还原 N₂，另外两个生成一分子 H₂，而放出这个 H₂ 是反应机制中必然的一部分。** 为什么是十六个 ATP：ATP 的算术从「传递的几何」推出来——电子是一次一个交出去的，一个还原态的还原酶结合上去、递出一个电子、随即解离，而每一轮循环都需要由这个二聚体还原酶水解两分子 ATP，八轮，每轮两个。对代价的总结毫不客气：固氮在能量上是昂贵的，16 个 ATP 与 8 个电子只换来 2 分子 NH₃。另有一个更简单的化学计量至今仍被广泛印行、也仍在考：N₂ + 6 H⁺ + 6 e⁻ → 2 NH₃，每电子一个 ATP，因而是 6 个 ATP，产物中没有 H₂；它自身是自洽的。现代数值与它不同，这里是用铵而不是用氨写出来的。真正了结这场分歧的是那个强制性的氢气释放：每固定一次氮就放出一次 H₂，而六电子方程里根本没有东西可以用来生成它。",
    ...
    },
    {
      ...
      en: "The scheme drawn with the six-electron equation makes its ATP accounting explicit: the process is ATP-dependent and probably requires 1 ATP for each transferred electron, so 6 ATP → 6 ADP + Pi is written alongside the six electrons that pass from cellular metabolism through ferredoxin and flavodoxin into the two nitrogenase proteins. **One ATP per transferred electron, rather than two per delivery cycle, is the whole of the difference between a scheme showing 6 ATP and an equation demanding 16.** The scheme is internally consistent, because its own statement of one ATP per electron gives exactly the 6 ATP it shows; what it differs from is the value in current standard references, which give eight electrons, 16 ATP and an H2 by-product. It is reproduced as printed and flagged rather than silently updated, so if asked, give these figures and note the modern value only if you are confident of it.",
      cn: "与六电子方程一起画出的那张图，把 ATP 的账算得很明白：该过程依赖 ATP，很可能每传递一个电子需要 1 个 ATP，因此在那六个电子旁边写着 6 ATP → 6 ADP + Pᵢ；这六个电子从细胞代谢经铁氧还蛋白、黄素氧还蛋白，进入固氮酶的两个蛋白。**每传递一个电子算一个 ATP，而每一轮递送算两个 ATP——6 ATP 的图与 16 ATP 的方程，差别全部在这里。** 这张图自身是自洽的，因为它自己那句「每电子 1 个 ATP」恰好给出图中所示的 6 个 ATP；它与之不同的，是当前标准文献所给的数值——八个电子、16 个 ATP，外加一个 H₂ 副产物。此处按原文照录并加标记，而不是默默更新；所以若被问到，先给这套数字，只在有把握时再补充现代数值。",
      src: "B §9.4, p.215"
    },
    {
      link_en: "for scale, put the two ways of breaking that bond side by side",
      link_cn: "为了有个尺度感，把打断那根键的两种办法放在一起比",
      en: "The N–N triple bond is very stable, and ammonia is produced industrially by the Haber process, which overcomes that barrier at 400 to 500 degrees C and hundreds of atmospheres of pressure over a special catalyst. **A bacterium does it at ordinary temperature and 0.8 atm of nitrogen, paying in ATP instead of in heat and pressure**, whereas biological fixation must occur at biological temperatures, and the reaction is exergonic either way — so the ATP buys passage over an activation barrier. That is the single most useful thing to be able to say about fixation: the difficulty was never in the thermodynamics.",
      cn: "N≡N 三键非常稳定；工业上用哈伯法（Haber process）制氨，靠 400 到 500 ℃、几百个大气压外加特殊催化剂越过这道壁垒。**一个细菌在常温、0.8 atm 氮分压下做成同一件事，而生物固氮必须在生理温度下完成；代价从高温高压换成了 ATP；而这个反应无论哪条路都是放能的——所以那些 ATP 买的是翻过活化能垒。** 关于固氮，这是最值得能说出口的一句话：难点从来不在热力学。",
    ...
    },
    {
      ...
      en: "Taken as printed, the chloroplast equation runs L-glutamate + NH4+ + ATP → L-glutamine + AMP + 2 Pi, in which the ATP is cleaved at the α–β position instead of losing only its terminal phosphate. **Products of AMP and two inorganic phosphates would mean two high-energy-bond equivalents are spent, not one, which drives the step effectively to completion.** Spending two bonds to buy irreversibility is exactly the chemistry of a synthetase, so the reading is coherent on its own terms even where the observed γ-glutamyl phosphate intermediate points to ADP and Pi. The name is worth getting right along with the chemistry. One spelling writes the enzyme as glutaminsynthasa — that is, as a synthase, the class of enzyme that joins two pieces without spending a nucleoside triphosphate — while standard references name it glutamine synthetase, a ligase, which is the class that does spend ATP. The chemistry written there, ATP going to AMP plus two Pi, is a synthetase's, so the mismatch is noted rather than silently corrected, and synthetase is the name to give.",
      cn: "照印出来的样子读，叶绿体里那条方程是 L-谷氨酸 + NH₄⁺ + ATP → L-谷氨酰胺 + AMP + 2 Pᵢ，其中 ATP 是在 α–β 位断开的，而不只是掉一个末端磷酸。**产物是 AMP 加两个无机磷酸，就意味着消耗掉的是两个高能键当量而不是一个，正是这一点把该步骤推向完全。** 花两个键去买不可逆性，恰恰就是 synthetase 这一类酶的化学，所以这种读法在它自己的框架里是自洽的——即便实际观察到的 γ-谷氨酰磷酸中间体指向的是 ADP 和 Pᵢ。名字也值得和化学一起弄准：有一种写法把这个酶写作 glutaminsynthasa（合酶），而标准文献称它为 glutamine synthetase（合成酶）——一种使用 ATP 的连接酶（ligase）。那里写出来的化学，ATP → AMP + 2 Pᵢ，正是合成酶的化学；所以这处不一致只作注明，不默默更正，而答题时该给的名字是 synthetase。",
      src: "B §9.4, p.215"
    },
    {
    ...
    {
      link_en: "and that same enzyme matters most when it is read backwards",
      link_cn: "而同一个酶，反着读的时候才最要紧",
      recall_en: "the reversible one-step amination just above, now run the other way",
      recall_cn: "就是上一步那个可逆的一步氨化，只是这次反着跑",
      en: "One account presents the one-step amination of α-ketoglutarate as a minor pathway and another calls it the most important step, and the way to hold both is to notice which direction each is describing. Forward, in microbes and in plants, it contributes significantly only when NH3 is added artificially, so it is minor for mammalian assimilation. **Run in reverse it is a deaminase, stripping the amino group off glutamate to release NH4+, and in eukaryotes it sits in the mitochondrial matrix — exactly where the urea cycle needs NH4+.** So the same reversible reaction is a side road going in and a main road coming out.",
      cn: "对于「α-酮戊二酸一步氨化」，一种说法把它呈现为次要途径，另一种说法称它是最重要的一步；要同时握住这两句，办法是看清各自描述的是哪个方向。正向：在微生物与植物里，只有人为加入 NH₃ 时它才有显著贡献，所以对哺乳动物的同化来说它确实次要。**反向跑，它就是一个脱氨酶：把氨基从谷氨酸上摘下来、放出 NH₄⁺；而在真核生物中这个酶位于线粒体基质 —— 尿素循环需要 NH₄⁺ 送到的，正是那里。** 于是同一个可逆反应，进来时是条小路，出去时是条大路。",
      src: "A p.802–803 · B §9.4, p.215",
      see: [{ id: "L-18-2-1", en: "the urea cycle that NH4+ is delivered into", cn: "这些 NH₄⁺ 被送进去的那个尿素循环" }]
    },
    {
    ...
    },
    {
      ...
      en: "**Ammonia is the hub of the whole cycle: bacterial fixation, the breakdown of urea, and death and decomposition all converge on NH3**, and from NH3 nitrogen either goes up into nitrite and nitrate or across into organic compounds. The organic loop runs plants first — NH3 is assimilated into the organic compounds of a plant, which pass as food into the organic compounds of an animal, which come back out in excretion as urea, H2NCONH2, the excretory nitrogen compound of that loop, and plants and microbes break that urea back down to NH3, while death and decomposition return organic nitrogen to NH3 directly. Around that loop stand the bacteria, each drawn as its own arrow: N2 is converted to NH3 by bacteria containing nitrogenase — bakterie obsahující nitrogenasu — while denitrifying bacteria return NO2− to N2, and nitrifying bacteria oxidize in the other direction, towards NO2− and NO3−, building the soil store; anammox bacteria reach N2 by another road again. Nothing in the loop is optional; take out the bacteria and the nitrogen stops moving.",
      cn: "**氨是整个循环的枢纽：细菌固氮、尿素分解、死亡与腐解，全都汇聚到 NH₃；而氮又从 NH₃ 出发，或向上进入亚硝酸盐和硝酸盐，或横向进入有机化合物。** 有机那一圈是从植物开始的——NH₃ 被同化进植物的有机化合物，这些化合物作为食物进入动物的有机化合物，再经排泄以尿素（H₂NCONH₂）的形式出来——尿素就是这一圈里那个排泄性的含氮化合物；植物与微生物把它分解回 NH₃，而死亡与腐解让有机氮直接回到 NH₃。围着这一圈站着的是细菌，每一类都画成自己的一支箭：N₂ 由「含固氮酶的细菌」（bakterie obsahující nitrogenasu）变成 NH₃；反硝化细菌把 NO₂⁻ 送回 N₂；硝化细菌则朝相反方向氧化，通向 NO₂⁻ 与 NO₃⁻，筑起土壤里的贮库；anammox 细菌又走另一条路抵达 N₂。这圈里没有哪一环是可有可无的：把细菌抽走，氮就不动了。",
