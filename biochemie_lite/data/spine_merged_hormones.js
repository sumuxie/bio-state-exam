...
window.BIOLITE_SPINE["key:tissue-specific-metabolism"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "**The animal organism cannot convert acetyl-CoA into pyruvate, the starting compound for saccharide synthesis, and therefore cannot form saccharides from fats.** The restriction to animals is deliberate and correct: plants and some microorganisms have the glyoxylate cycle, which does exactly this conversion, and that is how a germinating oilseed turns stored fat into sugar. One partial exception is worth keeping in view — the glycerol half of a fat does reach sugar metabolism; what cannot become sugar in an animal is the acyl carbon, which is the great majority of a fat's mass. Read the two steps together and the asymmetry is the whole of it: sugar becomes fat freely, fat becomes sugar not at all, and every fasting programme in this chain exists because of that. That is precisely the arrow missing from the overview diagram of acetyl-CoA metabolism: the diagram makes the same claim by omission that this paragraph makes in words.",
      cn: "**动物机体无法把乙酰辅酶 A 转变为丙酮酸（糖合成的起始化合物），因而也无法由脂肪生成糖。**限定在「动物」上是有意为之、而且正确：植物与某些微生物拥有乙醛酸循环，它恰恰完成这一转变 —— 萌发中的油料种子就是这样把贮存的脂肪变成糖的。有一个局部例外值得留意：脂肪中甘油的那一半确实能进入糖代谢；在动物体内不能变成糖的是酰基上的碳，也就是脂肪质量的绝大部分。把这两步合起来读，全部要点就是这个不对称：糖可以随意变成脂肪，脂肪一点也变不回糖 —— 而本链条里所有那些禁食程序之所以存在，正是因为这一条。这也正是那张乙酰辅酶 A 代谢总览图里缺掉的那条箭头：图以「省略」的方式，作出了本段用文字作出的同一个论断。",
      src: "B §10.1.3, p.220",
