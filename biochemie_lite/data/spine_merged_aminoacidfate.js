...
window.BIOLITE_SPINE["key:amino-group-metabolism"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "Aminotransferases sit in both the cytosol and the mitochondria of a eukaryotic cell, and the two compartments cooperate: glutamate made in the cytosol is carried into the mitochondrion by a specific active-transport carrier. In the matrix, glutamate dehydrogenase, E.C. 1.4.1.2, relative molecular mass 330,000, strips that amino group off as ammonium and hands 2-oxoglutarate back; the nicotinamide coenzyme oxidises the carbon first to alpha-iminoglutarate, an imine that water then splits into the 2-oxo acid and the ammonium ion. It is unusual in accepting either NAD+ or NADP+, which the plant and microbial versions do not, so the dual specificity is a mammalian peculiarity and not a general property of the enzyme; it is built from six identical subunits, activated by ADP and inhibited by GTP, and the direction makes sense, because GTP is a citric acid cycle product signalling that 2-oxoglutarate is already plentiful and there is no reason to tear down glutamate to make more of what the cycle has. The allostery is demonstrably real: mutations damaging its GTP site leave the enzyme permanently on, giving hyperinsulinism-hyperammonemia syndrome, with insulin over-secreted after a protein meal, ammonia high in blood and blood glucose low. Read that phenotype backwards and it is a proof of the mechanism — an enzyme that can no longer be switched off tears down glutamate continuously, so ammonium rises, while the 2-oxoglutarate it keeps producing feeds the cycle and drives insulin release after a protein meal instead of after a carbohydrate one, which is why the blood glucose falls. **An aminotransferase and glutamate dehydrogenase working as a pair is called transdeamination, and it is the whole answer to how twenty amino acids lose their nitrogen through one controlled exit.**",
      cn: "在真核细胞里，氨基转移酶在胞质和线粒体中都有，两个区室是配合着干的：胞质里生成的谷氨酸由一套专一的主动转运载体送进线粒体。在基质里，谷氨酸脱氢酶（E.C. 1.4.1.2，相对分子质量 330,000）把这个氨基以铵离子的形式剥下来，同时把 2-氧戊二酸交还回去；烟酰胺辅酶先把那个碳氧化成 α-亚氨基戊二酸（一个亚胺），再由水把它劈成 2-氧代酸和铵离子。它不寻常的地方在于 NAD+ 和 NADP+ 它都收，而植物和微生物版本做不到 —— 所以这种双重专一性是哺乳动物的特点，不是这个酶的普遍性质；它由六个相同亚基组成，ADP 激活、GTP 抑制，而这个方向是说得通的：GTP 是柠檬酸循环的产物，意味着 2-氧戊二酸已经不缺，没理由再拆谷氨酸去多造循环里已有的东西。这套变构是可以被证明的：破坏它 GTP 位点的突变让酶永远开着，结果是高胰岛素血症-高氨血症综合征 —— 蛋白质餐后胰岛素分泌过多、血氨升高、血糖偏低。把这个表型倒着读，它就是机制的证明：一个再也关不掉的酶会不停地拆谷氨酸，于是铵离子上升；与此同时它源源不断产生的 2-氧戊二酸不断喂进循环，由此使胰岛素在蛋白质餐后、而不是糖类餐后被释放，血糖也就因此掉下去。**一个氨基转移酶加上谷氨酸脱氢酶，这一对合起来叫转脱氨基作用；二十种氨基酸如何经由一个受控的出口卸掉氮，答案全在这一对上。**",
      src: "A p.631 · B §5.2.1, pp.119-120"
    },
    {
      link_en: "and that one reversible reaction is worth pulling out on its own, because two whole subjects meet inside it",
      link_cn: "而这一个可逆反应值得单独拎出来，因为有两门学问在它内部交汇",
      recall_en: "the same reaction as the step above, read for what it leaves behind rather than for the nitrogen it removes",
      recall_cn: "还是上一步那个反应，只是这次读的不是它拿走的氮，而是它留下的东西",
      en: "The interconversion of glutamate and 2-oxoglutarate runs in both directions, and both directions are used: forwards it strips nitrogen off the pool, backwards it fixes ammonium onto a carbon skeleton. **Glutamate and 2-oxoglutarate are a nodal reaction of amino acid metabolism — the point where nitrogen metabolism and carbon metabolism are the same reaction, so the two can be regulated together.** What comes out is not waste: the 2-oxoglutarate left when glutamate is deaminated goes either round the citric acid cycle to be burned or into gluconeogenesis to be rebuilt into glucose, which is why an enzyme named for handling nitrogen is also an important intersection of carbon traffic. A few amino acids skip the pairing altogether and are deaminated oxidatively on their own, without an aminotransferase in front of them.",
      cn: "谷氨酸与 2-氧戊二酸之间的互变是双向的，而且两个方向都在用：正向把氮从氨基酸池里剥下来，逆向把铵离子固定到一副碳骨架上。**谷氨酸 ⇌ 2-氧戊二酸是氨基酸代谢的一个枢纽反应 —— 在这里，氮代谢与碳代谢是同一个反应，因而两者可以被一起调控。**出来的东西并不是废物：谷氨酸脱氨后留下的 2-氧戊二酸，或者进柠檬酸循环被烧掉，或者进糖异生被重新造成葡萄糖 —— 这正是一个以处理氮命名的酶同时也是碳流量的重要交叉口的原因。另有少数几种氨基酸干脆跳过这套配对，前面不接氨基转移酶，自己直接进行氧化脱氨。",
      src: "A p.631 · B §5.2.1, pp.118-119"
    },
    {
    ...
    },
    {
      ...
      en: "Which form depends largely on how much water the animal has. The grouping does not follow ancestry — sharks are ureotelic and are not short of water — so water is not the only pressure at work. There is a second reading of these molecules worth having: the carbons of urea and of uric acid are highly oxidised, which says that an organism throws carbon away only after taking most of the oxidation energy out of it. **Most aquatic species release ammonia and let the water around them dilute it, most terrestrial animals excrete urea instead, and birds and reptiles excrete uric acid** as a near-solid paste that costs almost no water. A land animal has two demands to satisfy at once, not one — keep the toxicity down and keep the water loss down — and urea is the compromise between them. Plants make no such compromise, because they recycle virtually all their amino groups and excrete nitrogen only in rare circumstances; and whatever is excreted, in whatever form, is taken back up almost at once by the microorganisms that are everywhere, so nitrogen leaves an animal without ever leaving the biosphere.",
      cn: "以哪种形式离开，主要取决于这种动物有多少水。这个分组并不跟着亲缘关系走 —— 鲨鱼排尿素，而鲨鱼并不缺水 —— 所以「水」不是唯一在起作用的压力。这些分子还有第二种读法值得拿着：尿素和尿酸的碳都被高度氧化了，这说明生物只有在把碳的氧化能量榨得差不多之后，才会把它扔掉。**多数水生物种直接放出氨，让周围的水稀释它；多数陆生动物改为排出尿素；鸟类和爬行类排出的是尿酸，接近固体的糊状，几乎不费水。**陆生动物要同时满足的是两个要求而不是一个 —— 既要压住毒性，又要少丢水 —— 而尿素正是这两者之间的折中。植物不需要作这个折中，因为它们几乎把全部氨基都回收利用，只有在罕见情形下才排氮；而不论以什么形式排出去的氮，都会被无处不在的微生物几乎立刻重新吸收回去 —— 氮离开了动物，却从未离开生物圈。",
      src: "A pp.626-627"
    },
    {
...
window.BIOLITE_SPINE["key:amino-acid-degradation"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "Gluconeogenesis — making glucose out of smaller precursors — starts from oxaloacetate, and every intermediate of the citric acid cycle can be carried round the cycle to top the oxaloacetate pool up. **A skeleton arriving at pyruvate, 2-oxoglutarate, succinyl-CoA, fumarate or oxaloacetate can therefore be turned into glucose and into glycogen, and such amino acids are called glucogenic.** They are the large majority, and this is why feeding amino acids to a starved body drives gluconeogenesis and the laying down of glycogen instead of simply being burnt. The route out of the cycle is worth naming, because it is where a four-carbon acid becomes a three-carbon one: oxaloacetate is decarboxylated to phosphoenolpyruvate, written PEP, and PEP is the intermediate from which the sugar is built back up, so on a summary diagram the arrow to D-glucose leaves the cycle by way of PEP.",
      cn: "糖异生（由更小的前体造出葡萄糖）从草酰乙酸起步，而柠檬酸循环的每一个中间物都能沿着循环走到草酰乙酸那里，把草酰乙酸池补上去。**所以，凡是能抵达丙酮酸、2-氧戊二酸、琥珀酰-CoA、延胡索酸或草酰乙酸的骨架，都能被转成葡萄糖和糖原，这样的氨基酸就叫生糖氨基酸。** 它们是绝大多数；给一个饥饿的身体补上氨基酸之所以能推动糖异生、能把糖原重新堆起来，而不只是被烧掉，原因就在这里。离开循环的那条路值得点名，因为四碳酸正是在那里变成三碳酸：草酰乙酸脱羧生成磷酸烯醇丙酮酸（PEP），而糖正是由 PEP 重新搭起来的 —— 所以在总结图上，通向 D-葡萄糖的那支箭头是经由 PEP 离开循环的。",
    ...
    },
    {
      ...
      en: "A carbon skeleton is under no obligation to stay in one piece: different fragments of one skeleton can end up at different destinations, so an amino acid can deliver part of itself to acetyl-CoA and the rest to fumarate. An amino acid therefore fails to be glucogenic only if its entire skeleton lands on acetyl-CoA or acetoacetyl-CoA. **Only leucine and lysine do that, and they are therefore the two exclusively ketogenic amino acids — every other amino acid can make some contribution to gluconeogenesis.** Seven reach acetoacetyl-CoA or acetyl-CoA in whole or in part and can therefore yield ketone bodies, which are small water-soluble fuels made out of surplus acetyl units: phenylalanine, tyrosine, isoleucine, leucine, tryptophan, threonine and lysine. Take leucine and lysine out of those seven and five are left — tryptophan, phenylalanine, tyrosine, threonine and isoleucine — each of them ketogenic and glucogenic at once, because part of its skeleton goes one way and part the other. Threonine earns its place on that dual list by turning up at acetyl-CoA and at succinyl-CoA alike, and an amino acid degraded to pyruvate is potentially ketogenic too, since pyruvate itself can be converted to acetyl-CoA. One printed sentence in B §5.2.3 says that of its own examples 'purely ketogenic is however only Leu', and that sentence's list of examples never mentions lysine at all — but the family pathway diagram and the summary figure both draw lysine on exactly the same acetyl-CoA-only route as leucine, with no glucogenic branch anywhere. Where a running sentence and a diagram disagree in this way, the diagram is the more complete source and the sentence is a list of examples rather than a full membership list.",
      cn: "一副碳骨架没有义务保持完整：同一副骨架的不同碎片可以去往不同的终点，所以一个氨基酸完全可以把自己的一部分送到乙酰-CoA、其余送到延胡索酸。因此，一个氨基酸要做不到生糖，唯一的可能是它的整副骨架都落在乙酰-CoA 或乙酰乙酰-CoA 上。**做到这一点的只有亮氨酸和赖氨酸，它们因此是仅有的两个纯生酮氨基酸；除它们之外，每一个氨基酸都能对糖异生有所贡献。** 有七个氨基酸会全部或部分地抵达乙酰乙酰-CoA 或乙酰-CoA，因而能产出酮体（酮体是由多余的乙酰单位做成的小分子水溶性燃料）：苯丙氨酸、酪氨酸、异亮氨酸、亮氨酸、色氨酸、苏氨酸、赖氨酸。把亮氨酸和赖氨酸从这七个里去掉，剩下五个 —— 色氨酸、苯丙氨酸、酪氨酸、苏氨酸、异亮氨酸 —— 这五个既生酮又生糖，因为各自骨架的一部分走这边、另一部分走那边。苏氨酸之所以也算在这份「两栖」名单上，是因为它在乙酰-CoA 和琥珀酰-CoA 两处都出现；而被降解成丙酮酸的氨基酸同样具有潜在的生酮能力，因为丙酮酸本身可以转成乙酰-CoA。B §5.2.3 里有一句话说：在它列举的例子中「纯生酮的只有 Leu」，而那句话的举例清单里根本没有提到赖氨酸 —— 但第二族的通路图与总结图都把赖氨酸画在与亮氨酸完全相同的、只通向乙酰-CoA 的路线上，没有任何一条生糖支路。当正文的一句话与图示这样彼此矛盾时，图示是更完整的来源，而那句话是一份举例清单，不是一份完整名单。",
    ...
    {
      link_en: "one reaction in that list settles a question about food, and it is worth lifting out of the list",
      link_cn: "上面那串反应里，有一个决定着一个关于「吃」的问题，值得从名单里单独拎出来",
      recall_en: "the hydroxylation named two sentences into the step above",
      recall_cn: "上一步开头第二句里点到的那个羟化反应",
      en: "Phenylalanine hydroxylase puts one hydroxyl group onto phenylalanine and the product is tyrosine — one reaction, and it runs one way only. **That single arrow is why phenylalanine is essential, meaning the diet has to supply it, while tyrosine is only conditionally dispensable: it can be made, but only out of phenylalanine.** Cut the phenylalanine supply, or break the hydroxylase, and tyrosine stops being dispensable at all — so a classification of amino acids into essential and non-essential is a statement about which reactions a body has, and not about the molecules themselves.",
      cn: "苯丙氨酸羟化酶给苯丙氨酸加上一个羟基，产物就是酪氨酸 —— 一个反应，而且只朝一个方向走。**正是这一支箭头，使苯丙氨酸成为必需氨基酸（身体造不出来，必须由饮食供给），而酪氨酸只是「条件性非必需」：身体总能造出酪氨酸，但只能拿苯丙氨酸来造。**断掉苯丙氨酸的供应，或者把这个羟化酶弄坏，酪氨酸就一点也不「非必需」了 —— 所以把氨基酸分成必需与非必需，说的是这具身体手上有哪些反应可用，而不是分子本身的性质。",
      src: "A p.650 · B §5.2.3, pp.126-127"
    },
    {
    ...
    },
    {
      ...
      en: "Arginine, histidine, glutamate, glutamine and proline all enter the citric acid cycle as 2-oxoglutarate, and the immediate precursor in every case is glutamate. Glutamine joins at glutamate directly. Histidine is deaminated to urocanate and hydrolysed onward to glutamate. Proline is a closed ring, and proline dehydrogenase opens it by oxidation to delta-1-pyrroline-5-carboxylate, which sits in equilibrium with the open-chain glutamate gamma-semialdehyde; arginine reaches that same semialdehyde by way of ornithine and one transamination. **Glutamate gamma-semialdehyde is oxidised to glutamate, and glutamate is then deaminated to 2-oxoglutarate, so five unalike skeletons enter the citric acid cycle through a single door.** One sentence at the top of B p.128 has to be flagged rather than copied: it says the semialdehyde is degraded all the way to acetoacetyl-CoA, and that contradicts three things on its own page — the heading printed above it, which reads 'conversion to 2-oxoglutarate'; the diagram printed directly beneath it, which draws arginine and proline to the semialdehyde, the semialdehyde to glutamate and glutamate to 2-oxoglutarate, with no arrow to acetoacetyl-CoA anywhere; and the next sentence, which takes histidine through urocanate to glutamic acid. Glutamate gamma-semialdehyde dehydrogenase makes glutamate and has no route to acetoacetyl-CoA at all, so the diagram is the reliable side and the printed clause looks like a slip carried over from the glutaryl-CoA discussion one page earlier.",
      cn: "精氨酸、组氨酸、谷氨酸、谷氨酰胺、脯氨酸都以 2-氧戊二酸的身份进入柠檬酸循环，而它们的直接前体无一例外都是谷氨酸。谷氨酰胺直接汇入谷氨酸。组氨酸脱氨成尿刺酸，再水解、一路降解成谷氨酸。脯氨酸是一个闭合的环，脯氨酸脱氢酶把它氧化成 Δ1-吡咯啉-5-羧酸，从而把环打开；后者与开链的谷氨酸 γ-半醛处于平衡之中。精氨酸则经由鸟氨酸再转氨一次，抵达同一个半醛。**谷氨酸 γ-半醛被氧化成谷氨酸，谷氨酸再脱氨成 2-氧戊二酸 —— 于是五副毫不相像的骨架，从同一扇门进入柠檬酸循环。**B 第 128 页最上面有一句话必须标出来，而不能照抄：它说这个半醛「经一系列反应完全降解为乙酰乙酰-CoA」，而这与同一页上的三处内容相矛盾 —— 它所属那一节的标题写的是「转化为 2-氧戊二酸」；紧挨在它下面的图画的是精氨酸、脯氨酸 → 半醛 → 谷氨酸 → 2-氧戊二酸，没有任何一支箭头指向乙酰乙酰-CoA；紧接着的下一句又把组氨酸经尿刺酸带到谷氨酸。谷氨酸 γ-半醛脱氢酶的产物是谷氨酸，根本没有通往乙酰乙酰-CoA 的路线，所以图示才是可信的一侧，而正文那一句看上去是从前一页戊二酰-CoA 那段误植过来的笔误。",
