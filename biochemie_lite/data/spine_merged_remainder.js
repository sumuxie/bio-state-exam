...
window.BIOLITE_SPINE["key:amino-acid-derived-molecules"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "In higher eukaryotes, mammals included, glycine condenses with succinyl-CoA taken straight out of the citric acid cycle, coenzyme A and carbon dioxide are lost, and the product is delta-aminolevulinate, also written 5-aminolevulinate because the amino group sits on carbon 5; the enzyme is delta-aminolevulinate synthase. Making the two molecules that the next step needs runs that condensation twice and so loses two coenzyme A and two carbon dioxide in all. In plants, algae and most bacteria the starting material is glutamate instead, by a strikingly roundabout route: the glutamate is first esterified onto a transfer RNA — the molecule of protein synthesis — reduced by NADPH to glutamate 1-semialdehyde, cut off the tRNA, and handed to an aminotransferase. **Heme is built out of two things a cell is already making anyway, glycine and a citric acid cycle intermediate, so its synthesis is a side branch off metabolism rather than a pathway of its own.**",
      cn: "在包括哺乳动物在内的高等真核生物里，甘氨酸与直接取自柠檬酸循环的琥珀酰-CoA 缩合，失去辅酶 A 和二氧化碳，产物就是 δ-氨基乙酰丙酸（也写作 5-氨基乙酰丙酸，因为氨基位于第 5 号碳上）；催化的酶叫 δ-氨基乙酰丙酸合酶。下一步需要两分子，于是这个缩合要跑两遍，一共失去两分子辅酶 A 和两分子二氧化碳。在植物、藻类和多数细菌里，起始材料换成了谷氨酸，路线绕得惊人：谷氨酸先被酯化到一个转运 RNA（蛋白质合成用的那种分子）上，由 NADPH 还原成谷氨酸-1-半醛，再从 tRNA 上切下来，交给一个转氨酶。**血红素是拿细胞本来就在造的两样东西搭出来的 —— 甘氨酸，加上一个柠檬酸循环中间物 —— 所以它的合成是代谢分出的一条支路，而不是一条自成一体的通路。**",
    ...
    {
      link_en: "and one enzyme in that sequence is the one the cell actually turns up and down",
      link_cn: "而这一串酶里，细胞真正在调高调低的只有一个",
      recall_en: "the committed precursor named at the start of this chain, and the enzyme that makes it",
      recall_cn: "这条链开头点名的那个承诺前体，以及造它的那个酶",
      en: "A branching pathway is controlled at the step that commits material to it, and here that step is the first one. **Delta-aminolevulinate synthase, which condenses glycine with succinyl-CoA, is the key regulatory point of porphyrin synthesis, and heme itself is the feedback inhibitor that shuts it down.** Heme is the finished product at the far end of the pathway, so nothing about the control has to be memorised separately: enough heme means no reason to start another ring, so the end of the pathway reaches back and closes its own beginning, and the intermediates in between are never allowed to accumulate in a healthy cell.",
      cn: "一条会分叉的通路，控制点在「把原料承诺给这条路」的那一步上，而在这里，那一步就是第一步。**δ-氨基乙酰丙酸合酶 —— 把甘氨酸与琥珀酰-CoA 缩合起来的那个酶 —— 是卟啉合成的关键调控点；而通路最远端的成品血红素，正是把它关掉的反馈抑制剂。**这一条不需要另外去背：血红素够了，就没有理由再开一个新环，于是通路的末端反手把自己的开头关上，中间那些中间物在健康细胞里也就从不允许堆积起来。",
      src: "A p.818 · B §6.3, p.146"
    },
    {
...
window.BIOLITE_SPINE["key:cholesterol-and-isoprenoids"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "For storage and transport, that hydroxyl condenses with a fatty acid to form a sterol ester — cholesterol with its one polar group consumed in an ester bond. The membrane form and the transport form of cholesterol therefore differ by exactly one ester bond. **Capping the only polar group turns an amphipathic membrane component into a fully nonpolar molecule, fit for the inside of a lipid droplet or the core of a lipoprotein particle.** It is the same manoeuvre a triacylglycerol makes: esterify glycerol's three hydroxyls against three fatty-acid carboxylates and nothing polar is left in the molecule either, which is why the two storage lipids end up in the same droplet. The A-to-D lettering of the rings exists for exactly this kind of talk — it is how a derivative of the steroid nucleus gets referred to by the ring it differs at.",
      cn: "为了储存和运输，那个羟基与一个脂肪酸缩合，形成固醇酯。于是胆固醇的「膜形式」与「运输形式」之间，恰好只差一个酯键。**把唯一的极性基团封起来，一个两亲性的膜组分就变成了彻底非极性的分子 —— 正适合待在脂滴内部或脂蛋白颗粒的核心里。**这和三酰甘油玩的是同一手：把甘油的三个羟基与三个脂肪酸的羧基都酯化掉，分子里也就什么极性都不剩 —— 这正是这两种储存脂类最后待在同一个脂滴里的原因。环的 A 到 D 字母标注，正是为这类讨论而设的：类固醇核的某个衍生物差在哪个环上，就靠这套字母来指认。",
      src: "A p.352"
    },
    {
...
window.BIOLITE_SPINE["key:oxygen-binding-proteins"] = {
  ...
  steps: [
    ...
    {
      link_en: "and that door forces a correction to the picture everyone carries of a protein",
      link_cn: "而那扇门，逼着我们修正每个人心里那幅关于蛋白质的图像",
      recall_en: "the nanosecond gate of the step above, taken for what it implies about the whole molecule",
      recall_cn: "上一步那扇纳秒级的闸门，这里读的是它对整个分子意味着什么",
      en: "A rigid protein would be useless here. The heme is buried deep in the folded chain with very few direct paths from the surrounding solution to the iron, so if nothing moved, oxygen could neither get in nor get out, and the burial that protects the iron would have destroyed the protein's only function. **What resolves it is that rapid flexing of the side chains opens transient cavities, so a protein structure diagram is a time-average and not a photograph of a rigid object.** Computer simulations of myoglobin find many such paths, not one, and the distal His is only the gate on the largest of them. The same residue has now done two unrelated jobs in two steps — hydrogen-bonding the bound oxygen, and opening the door — which is why a residue is worth knowing by number.",
      cn: "在这里，一个刚性的蛋白质是没用的。血红素被深埋在折叠好的链里，从周围溶液通到铁的直接通路极少；如果什么都不动，氧既进不去也出不来，那么「把铁埋起来」这个保护措施，反倒把蛋白质唯一的功能毁掉了。**解开这一点的是：侧链的快速摆动会不断打开瞬时空腔 —— 所以一张蛋白质结构图是一个时间平均，而不是一个刚性物体的照片。**对肌红蛋白的计算机模拟找到的是许多条这样的通路，而不是一条，远端组氨酸只是其中最大那条上的闸门。同一个残基在两步之内已经干了两件毫不相干的事 —— 与结合上的氧形成氢键，以及开门 —— 这正是一个残基值得按编号记住的原因。",
      src: "A p.153"
    },
    {
    ...
    },
    {
      ...
      en: "Archibald Hill put a number on it in 1910: plot the logarithm of the ratio of filled to empty sites against the logarithm of oxygen pressure and the result is a straight line, whose slope the equation predicts should equal the number of binding sites. It does not. The measured slope reports how strongly the sites interact and is called the Hill coefficient; a value of 1 means no cooperativity at all, above 1 is positive cooperativity, and below 1 is negative cooperativity, where one bound ligand hinders the next, of which well-documented examples are rare. It could equal the number of sites only if every site filled at the same instant, so it is always smaller: haemoglobin has four sites and its highest observed value is about 3, and reading that 3 as a count of subunits is a plain error. Two models describe how the switching happens. **Monod, Wyman and Changeux's concerted model of 1965 has all subunits switching at one instant, so no partly switched molecule exists; Koshland's sequential model of 1966 switches them one at a time.** Each is worth stating in its own terms. The concerted model, also called the all-or-none model, takes the subunits to be functionally identical, each able to exist in at least two conformations; the two conformations stand in equilibrium with each other, oxygen binds to both but far more tightly to R, and each successive binding shifts that equilibrium further towards R. The sequential model instead lets one bound ligand change the conformation of its own subunit, which makes the same change more likely in a neighbour, and it therefore allows a great many intermediate states that the concerted model forbids outright.",
      cn: "Archibald Hill 在 1910 年给它配上了一个数：把「已占位点与空位点之比」的对数对「氧分压」的对数作图，得到一条直线；按方程，它的斜率应该等于结合位点数。事实并非如此。实测的斜率报告的是这些位点彼此相互作用有多强，被称为 Hill 系数；等于 1 表示完全没有协同，大于 1 是正协同，小于 1 是负协同（一个配体的结合妨碍下一个），而有充分记载的例子很少见。它只有在所有位点同一瞬间被填满时才可能等于位点数，所以它永远更小：血红蛋白有四个位点，观测到的最大值约为 3，而把这个 3 当作亚基数目来读是一个干脆的错误。描述「怎么一起转换」的有两个模型。**Monod、Wyman 与 Changeux 1965 年的协同模型让所有亚基在同一瞬间转换，因而不存在部分转换的分子；Koshland 1966 年的序变模型则允许它们一个一个地转。**两个模型各自的内容都值得说清楚。协同模型（也叫「全或无」模型）假定各亚基在功能上完全相同、每个至少能以两种构象存在；两种构象彼此处于平衡之中，氧与两者都能结合，但与 R 态结合得紧得多，而每一次结合都把这个平衡进一步推向 R。序变模型则允许一个已结合的配体改变它自己那个亚基的构象，并使相邻亚基发生同样变化的可能性升高；因此它容许大量中间状态，而协同模型干脆禁止这些状态存在。",
      src: "A pp.157-160",
