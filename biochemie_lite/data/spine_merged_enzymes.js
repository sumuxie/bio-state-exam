...
window.BIOLITE_SPINE["key:enzyme-kinetics"] = {
  ...
  steps: [
    ...
    },
    {
      ...
      en: "**Km has the units of a molar concentration, and it does not measure how tightly an enzyme holds its substrate except under one condition that most enzymes fail to meet.** For the two-step scheme Km equals (k2 + k-1)/k1, a mixture of how fast the substrate binds, how fast it falls off again, and how fast the chemistry happens. It measures how tightly the enzyme holds its substrate under one condition only: if the chemical step is much slower than falling off, k2 becomes negligible, Km reduces to k-1/k1, and that ratio is the dissociation constant Kd of the ES complex. That condition does not hold for most enzymes. Run it the other way — k2 much larger than k-1 — and Km becomes k2/k1, a number about chemistry with nothing to do with binding; and more commonly the reaction passes through several further steps after ES forms, which makes Km a complicated function of many constants.",
      cn: "**Km 的量纲是摩尔浓度；而它并不衡量酶把底物抓得多紧 —— 只有在一个条件下它才衡量，而多数酶都不满足那个条件。**对那个两步机制，Km = (k2 + k-1)/k1 —— 里面掺着「底物结合有多快」「它散掉有多快」「化学反应有多快」三件事。它衡量「酶把底物抓得多紧」只在一种条件下成立：当化学步骤远慢于散架时，k2 可以忽略，Km 退化成 k-1/k1，而这个比值正是 ES 复合物的解离常数 Kd。这个条件对多数酶并不成立。反过来，若 k2 远大于 k-1，Km 就变成 k2/k1 —— 一个关于化学、与结合毫无关系的数；更常见的情况是 ES 形成之后反应还要再走好几步，那时 Km 会变成许多速率常数的复杂函数。",
      src: "A p.192–193, §6.3"
    },
    {
      link_en: "so if Km is not an affinity, what is it actually good for?",
      link_cn: "那 Km 既然不是亲和力，它到底有什么用？",
      en: "**The Km of an enzyme tends to be close to the concentration of its substrate inside the cell, so Km states where on its own curve that enzyme normally sits in life.** An enzyme working on a scarce metabolite usually has a low Km and an enzyme working on an abundant one a high Km. That places it: an enzyme sitting near saturation runs at a rate that barely answers to substrate supply, while one sitting on the near-linear stretch of the curve has a rate that tracks supply directly. Every later argument about which enzyme in a pathway responds to how much substrate arrives rests on this one empirical rule, and it is an observation rather than a theorem — nothing in the derivation above requires it.",
      cn: "**一个酶的 Km，往往与它底物在细胞里的实际浓度相近；所以 Km 说的是这个酶在活体中平时待在自己那条曲线的哪一段。**作用于稀有代谢物的酶通常 Km 低，作用于丰富代谢物的酶通常 Km 高。这就把它定了位：靠近饱和区的酶，速率对底物供应几乎不作反应；而待在近似线性那一段的酶，速率随供应直接变化。后面所有关于「一条通路里哪个酶会对底物到货量作出响应」的论证，都压在这一条经验规律上 —— 而它是一个观察，不是一个定理：上面那套推导里没有任何一步要求它成立。",
      src: "A p.193, §6.3",
      see: [
