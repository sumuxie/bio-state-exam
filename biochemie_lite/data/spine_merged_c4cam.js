...
window.BIOLITE_SPINE["key:photorespiration-c4-cam"] = {
  ...
  steps: [
    {
      en: "Rubisco is the enzyme that attaches CO2 to ribulose 1,5-bisphosphate, a five-carbon sugar written RuBP, and it is not specific about what it attaches. **The same active site also accepts O2, and both activities compete for the same molecule of RuBP, so the oxygen a leaf makes in the light works directly against the carbon it is trying to fix.** Its full name records both jobs: ribulose-1,5-bisphosphate carboxylase/oxygenase. The two-carbon product of the O2 reaction has to be recovered by a route that consumes more O2 and releases CO2 again, and that recovery is what the word photorespiration names. The observable fact underneath the word is plain: most plants, during illumination, consume oxygen as well as producing it photosynthetically. The name is a trap all the same. Photorespiration differs substantially from mitochondrial respiration, meaning the respiratory chain: that chain conserves energy as ATP, and this conserves none at all, lowering net biomass instead — so the name is an analogy about gas exchange and not about mechanism. Everything below is a response to that one defect.",
      cn: "rubisco 是把 CO₂ 接到核酮糖-1,5-二磷酸（写作 RuBP，一种五碳糖）上的那个酶，而它对「接什么」并不专一。**同一个活性位点也接受 O₂，而这两种活性抢的是同一分子 RuBP，于是叶片在光下自己产生的氧，直接在跟它想固定的碳作对。** 它的全名把两份差事都记了下来：核酮糖-1,5-二磷酸羧化酶/加氧酶。加氧反应产生的那个二碳产物必须由一条回收途径救回来，而这条途径还要再耗 O₂、再放 CO₂ —— 这套回收正是「光呼吸（photorespiration）」这个词所指的东西。这个词底下那个可以观察到的事实很朴素：大多数植物在光照期间，除了光合产氧之外也消耗氧。但这个名字仍是个陷阱。光呼吸与线粒体呼吸（也就是呼吸链）有实质区别：呼吸链把能量存成 ATP，光呼吸一点也不存，反而压低净生物量 —— 所以这个名称是关于气体交换的类比，而不是关于机理的类比。下面的一切，都是对这一处缺陷的应对。",
    ...
    },
    {
      ...
      src: "B §9.2, pp.212–213, Obr. 9.7",
      beyond: true,
      beyondNote: "The oxygenase-versus-oxidase distinction, and the name glycolate oxidase, are not on the cited pages: those pages call the enzyme glycolate oxygenase — glykolatoxygenasa — in both the running text and the label of Obr. 9.7, while drawing the reaction glycolate + O2 giving glyoxylate + H2O2, with catalase then disposing of the peroxide, which is oxidase chemistry. Standard references name it glycolate oxidase. The account is internally consistent; the naming inconsistency is a mismatch between the name printed and the reaction drawn, and it is reported as printed rather than silently corrected."
    },
    {
      ...
      src: "B §9.2, p.212",
      openQuestion_en: "Why photorespiration exists at all is unsettled, and the reasons why plants consume oxygen during illumination are stated outright as not yet entirely clear — a hedge worth preserving rather than resolving. The protective role is offered with an explicit 'probably', and glycine and serine can be made by other routes, so neither proposal rules the other out or closes the question.",
      openQuestion_cn: "光呼吸为什么会存在，目前没有定论；对于「植物在光照期间为何还消耗氧」，书上直言原因尚不完全清楚 —— 这一保留值得原样保存，而不要替它下结论。保护作用是带着一个明写的「大概」提出来的，而甘氨酸和丝氨酸也能由别的途径合成 —— 所以两种说法既不互相排除，也都没有把这个问题合上。"
    },
    {
    ...
    },
    {
      ...
      en: "PEP carboxylase attaches inorganic carbon to phosphoenolpyruvate — PEP, the three-carbon compound sitting one step before pyruvate in glycolysis — giving the four-carbon acid oxaloacetate, and it does so at inorganic-carbon concentrations far too low for rubisco to work at. Its Km for that carbon is lower than rubisco's, Km being the substrate concentration at which an enzyme runs at half its maximum rate, so a low Km is precisely what 'works where the substrate is dilute' means. **Part of the chemistry behind that low Km is the substrate itself: PEP carboxylase takes bicarbonate, HCO3−, the abundant form of inorganic carbon at the pH of a plant cytosol**, made from the arriving gas by carbonic anhydrase. That enzyme goes under a second name, carbonate dehydratase — karbonatdehydratasa — and it is shown converting atmospheric CO2 to HCO3− before PEP-carboxylase acts, so the actual substrate is bicarbonate and not CO2 at all. Fix carbon cheaply where it is dilute, carry it as a four-carbon acid, release it concentrated next to rubisco: both alternative pathways are built out of that one move.",
      cn: "PEP 羧化酶把无机碳接到磷酸烯醇式丙酮酸上 —— PEP 就是糖酵解里位于丙酮酸前一步的那个三碳化合物 —— 生成四碳酸草酰乙酸；而且它在无机碳浓度低到 rubisco 根本干不了活的时候就能工作。它对这种碳的 Km 比 rubisco 低；所谓 Km，就是让一个酶跑到最大速率一半时所需的底物浓度，因此「Km 低」正是「底物稀薄也照样干活」的意思。**这个低 Km 背后有一部分化学在于底物本身：PEP 羧化酶用的是碳酸氢根 HCO₃⁻，即植物胞质 pH 下无机碳的主要存在形式，由碳酸酐酶从刚进来的气体现做出来。** 这个酶还有第二个名字：碳酸脱水酶（karbonátdehydratasa）；图上画的正是它在 PEP 羧化酶动手之前，把大气里的 CO₂ 转变成 HCO₃⁻ —— 所以真正的底物是碳酸氢根，根本不是 CO₂。在碳稀薄的地方廉价地把它固定住，用一个四碳酸运走，再在 rubisco 旁边高浓度地放出来：两条替代途径都是由这一个动作搭起来的。",
    ...
    },
    {
      ...
      en: "M. Hatch and C. Slack found plants that fix CO2 into four-carbon acids rather than into the three-carbon 3-phosphoglycerate, and the loop they described is called the Hatch–Slack cycle. PEP carboxylase makes oxaloacetate, and oxaloacetate is then metabolised in one of two ways: by transamination to aspartate, or by hydrogenation to malate, that reduction being carried out by malate dehydrogenase using the coenzyme NADPH. The four-carbon acid travels, and the decarboxylating enzyme waiting for it — labelled the malic enzyme, jablecny enzym — takes it back to pyruvate with the CO2 released; pyruvate phosphate dikinase then rebuilds PEP from pyruvate at the cost of ATP and closes the loop. **The CO2 released at the end is the same CO2 captured at the start, so this loop fixes no carbon on balance: it is a carbon taxi paid for in ATP, and the Calvin cycle still does the real fixing.** The label C-4 counts the carbons of the acid doing the carrying, which is why aspartate serves as well as malate does.",
      cn: "M. Hatch 与 C. Slack 发现了一类植物：它们把 CO₂ 固定成四碳酸，而不是固定成三碳的 3-磷酸甘油酸；他们描述的这个环路就叫 Hatch–Slack 循环。PEP 羧化酶生成草酰乙酸；草酰乙酸随后可经两种方式代谢：经转氨作用生成天冬氨酸，或经加氢生成苹果酸——生成苹果酸的这次还原由苹果酸脱氢酶使用辅酶 NADPH 完成。这个四碳酸走一段路，等着它的那个脱羧酶——被标作「苹果酸酶」(jablečný enzym)——把它变回丙酮酸并放出 CO₂；丙酮酸磷酸双激酶随后消耗 ATP 把丙酮酸重新做成 PEP，环路就此闭合。**末尾放出来的那个 CO₂，就是开头抓住的那个 CO₂ —— 所以这个环路净固定的碳是零：它是一辆用 ATP 付费的碳出租车，真正固定碳的仍然是 Calvin 循环。** 「C-4」这个标签数的是负责运载的那个酸有几个碳，所以天冬氨酸和苹果酸一样能胜任。",
      src: "B §9.3, p.212, Obr. 9.8",
      beyond: true,
      beyondNote: "The year of the Hatch and Slack discovery is printed on that page as 1970; standard references date their original publication on the C-4 pathway to 1966. Reproduced as printed and flagged rather than silently corrected — if the date matters in an answer, give the pathway name and the mechanism rather than leaning on the year."
    },
    {
      link_en: "and the acid carries more than carbon, which is where a second coenzyme balance closes",
      link_cn: "而这个酸运走的不只是碳 —— 第二笔辅酶的账，也在这里合上",
      en: "Reducing oxaloacetate to malate spends one NADPH where the trapping happens, and the malic enzyme regenerating pyruvate hands that reducing power back where the release happens, because its decarboxylation of malate is an oxidation that reduces NADP+ again. **Malate carries two things at once along the same route — the CO2 rubisco needs and the NADPH the Calvin cycle needs — so carbon delivery and reducing-power delivery are one journey.** Aspartate, the transamination alternative, carries only the carbon, which is one concrete reason a plant would run either branch rather than only the cheaper-looking one.",
      cn: "把草酰乙酸还原成苹果酸，在「抓碳」的地方花掉一个 NADPH；而再生丙酮酸的苹果酸酶又在「放碳」的地方把这份还原力交还回来 —— 因为它对苹果酸的脱羧同时是一次氧化，会把 NADP⁺ 重新还原。**于是苹果酸沿同一条路一次运两样东西：rubisco 需要的 CO₂，和 Calvin 循环需要的 NADPH —— 送碳和送还原力是同一趟行程。** 而走转氨那一支的天冬氨酸只运碳，这就是植物为什么会两支都跑、而不是只跑看上去更省的那一支的一个具体理由。",
      src: "B §9.3, p.212, Obr. 9.8 · A p.730"
    },
    {
      link_en: "and the enzyme that closes the loop is where the whole surcharge is incurred",
      link_cn: "而闭合这个环路的那个酶，正是全部附加费产生的地方",
      en: "Pyruvate phosphate dikinase is the ATP-consuming enzyme that phosphorylates pyruvate, forming PEP at the expense of ATP, so that the cycle regenerating the primary CO2 acceptor is closed. It is a dikinase because it moves two phosphoryl groups from one ATP at once: one to pyruvate and one to inorganic phosphate, leaving AMP and pyrophosphate rather than ADP. **Two phosphoanhydride bonds go in each turn instead of one, and that is the arithmetic behind the C-4 surcharge quoted later as two extra ATP per CO2** — the acceptor has to be rebuilt, and rebuilding it is what costs.",
      cn: "丙酮酸磷酸双激酶是那个耗 ATP 的酶：它把丙酮酸磷酸化，消耗 ATP 重新生成 PEP，使再生初级 CO₂ 受体的这个循环得以闭合。它之所以叫「双激酶」，是因为它一次从同一个 ATP 上搬走两个磷酰基：一个给丙酮酸，一个给无机磷酸，剩下的是 AMP 和焦磷酸，而不是 ADP。**于是每转一圈花掉的是两个磷酸酐键而不是一个 —— 后面说的「C-4 每个 CO₂ 多花两个 ATP」，那笔账正是从这里来的**：受体必须被重建，而重建它就是要花钱的地方。",
      src: "B §9.3, p.212 · A p.732",
      beyond: true,
      beyondNote: "That the dikinase cleaves ATP to AMP plus pyrophosphate, and that this is what makes the cost two phosphoanhydride bonds rather than one, is general nucleotide chemistry joining two statements that are on the cited pages — the enzyme name and the two-bond figure — and is on neither page itself."
    },
    {
    ...
    },
    {
      ...
      en: "The pathway of CO2 assimilation has a greater energy cost in C-4 plants than in C-3 plants, because each CO2 assimilated requires one molecule of PEP to be regenerated at the expense of two phosphoanhydride bonds of ATP, so the bill per CO2 completely reduced is 5 ATP against a C-3 plant's 3, with 2 NADPH in both cases. Below about 28–30 °C the C-3 plant wins: photorespiration is tolerable and the two extra ATP are not worth paying. Above that temperature, rubisco's falling affinity for CO2 makes the oxygenase reaction expensive enough that the gain in efficiency from its elimination more than compensates for the energetic cost. Read the whole comparison as a trade rather than as levels of sophistication: three routes are three points on a trade between carbon gain and water loss, and which one wins depends on the climate. **Neither of those extra ATP buys any carbon at all — they buy the ability to keep the stomata shut — which is why crabgrass, a C-4 weed, outgrows the C-3 lawn grasses around it in summer.** As any experienced gardener can attest.",
      cn: "C-4 植物中 CO₂ 同化通路的能量代价高于 C-3 植物，因为每同化一个 CO₂ 就必须再生一分子 PEP，其代价是 ATP 上的两个磷酸酐键；于是每完全还原一个 CO₂ 的账单是：C-4 用 5 个 ATP，C-3 用 3 个，两者都用 2 个 NADPH。约 28–30 °C 以下，C-3 植物占优：光呼吸还忍得了，那两个额外的 ATP 不值得花。越过这个温度，rubisco 对 CO₂ 亲和力的下降会让加氧反应贵到「省掉它所得的效率收益」超过这笔额外的能量代价。整个比较应当读作一笔交换，而不是三个先进程度的档次：三条路线是「碳收益 vs 水损失」这笔交换上的三个点，哪一个赢取决于气候。**多出来的那两个 ATP 一点碳也没买到 —— 它们买的是「气孔可以关着」这件事 —— 这就是为什么夏天里马唐草（一种 C-4 杂草）长得比周围的 C-3 草坪草都快，任何有经验的园丁都能作证。**",
    ...
    },
    {
      ...
      en: "Cactus, prickly pear, orchid and pineapple are CAM plants; maize, sugar cane and crabgrass are C-4 plants. Succulents get filed under C-4 often enough to be worth guarding against, and the two really do share their chemistry — the same PEP carboxylase, the same four-carbon carrier, the same concentrating of CO2 around rubisco. **The test that separates them is the clock: a C-4 plant fixes no CO2 at night, and a CAM plant does nearly all of its fixing then.** The two groupings are not always listed the same way — cactus-like succulents are sometimes listed alongside sugarcane among the C-4 plants, while a table of worked examples lists cactus, prickly pear, orchid and pineapple as CAM and maize, sugarcane and crabgrass as C-4 — and the disagreement is worth knowing rather than resolving, since sugarcane is C-4 in either listing. Sharper still, an option stating that CO2 fixation occurs at night is marked wrong as an answer about the advantage of the C-4 mechanism: correct about C-4, and exactly what a CAM plant does. So a sentence about CO2 being fixed at night is false of the C-4 pathway and is the definition of CAM. Holding both the classification and that clock test is what lets you handle a question about cactus, pineapple or night fixation without contradicting yourself. All three routes work around rubisco's lack of specificity and none of them repairs it.",
      cn: "仙人掌、仙人掌果、兰花、菠萝是 CAM 植物；玉米、甘蔗、马唐草是 C-4 植物。多肉植物被归到 C-4 那一栏的情况多到值得专门提防，而这两类确实共用同一套化学 —— 同一个 PEP 羧化酶、同一种四碳载体、同样是把 CO₂ 在 rubisco 周围浓缩起来。**把它们分开的判据是时钟：C-4 植物在夜间不固定任何 CO₂，而 CAM 植物几乎全部的固定都发生在夜间。** 这两组的归类并不总是一致：仙人掌一类的多肉有时被与甘蔗一起列在 C-4 植物之中，而一张给出实例的表则把仙人掌、仙人掌果、兰花、菠萝列为 CAM，把玉米、甘蔗、马唐草列为 C-4 —— 这处分歧值得知道，而不必替它裁决，因为甘蔗在两种列法里都是 C-4。更锋利的是：「让 CO₂ 在夜间固定」这个选项，被当作「C-4 机制的优势是什么」的错误答案 —— 作为关于 C-4 的说法它没错，而那恰恰正是 CAM 植物做的事。所以「CO₂ 在夜里被固定」这句话，用来描述 C-4 是错的，用来描述 CAM 则正是定义。同时握住这个分类和那个时钟判据，才能在被问到仙人掌、菠萝或夜间固定时答得前后不矛盾。三条路线都是在绕开 rubisco 的不专一，没有一条是在修它。",
