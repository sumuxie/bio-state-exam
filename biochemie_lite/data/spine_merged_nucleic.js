...
window.BIOLITE_SPINE["key:transcription"] = {
  ...
  steps: [
    ...
    {
      link_en: "and it leaves out the other thing a DNA polymerase does, which fixes how accurate the copy is",
      link_cn: "它还漏掉了 DNA 聚合酶会做的另一件事 —— 而这件事决定了抄本能有多准",
      en: "RNA polymerase is also missing a proofreading site: it has no separate 3'-to-5' exonuclease — an activity that runs backwards along the new chain and cuts off a residue just put in — of the kind many DNA polymerases carry. Transcription is correspondingly the less accurate copying, with roughly one wrong ribonucleotide for every 10,000 to 100,000 laid down, which is several orders of magnitude worse than the copying of chromosomal DNA. **A cell can afford that because one gene yields many RNA copies and nearly all of them are degraded and replaced, so an error in an RNA costs it far less than an error in DNA**, where the information is permanent and every descendant cell inherits it. Fidelity is paid for where information is kept and skipped where it is disposable, which is the same argument that explains why replication proofreads at all.",
      cn: "RNA 聚合酶还少一样东西：校对位点。它没有那种独立的 3′→5′ 外切酶 —— 即沿着新链倒着走、把刚放进去的残基切掉的活性 —— 而许多 DNA 聚合酶都带着一个。转录因此是较不准的那种抄写：大约每放下 10,000 到 100,000 个核糖核苷酸就出一个错，比染色体 DNA 的复制差好几个数量级。**细胞出得起这个价，是因为一个基因会产出许多份 RNA 拷贝、而且几乎全都会被降解替换，所以 RNA 里的一个错误，代价远小于 DNA 里的一个错误** —— DNA 里的信息是永久的，之后每一个子细胞都会继承它。保真度花在信息被留存的地方，在信息用完即弃的地方就省掉 —— 这与「复制为什么要校对」是同一套论证。",
      src: "A p.963",
      openQuestion_en: "Bacterial RNA polymerase and Pol II do pause when a mispaired base goes in, and can take a mismatched nucleotide back off the 3' end by running the polymerase reaction in reverse; whether that counts as true proofreading, and how much it contributes, is not known.",
      openQuestion_cn: "细菌 RNA 聚合酶和 Pol II 确实会在掺入错配碱基时停顿，也确实能通过逆转聚合反应把 3′ 端的错配核苷酸取下来；但这算不算真正的校对、对保真度贡献多少，目前并不知道。"
    },
    {
    ...
    },
    {
      ...
      en: "The enzyme opens a short stretch of the helix and carries the opening along with it. About 17 bp are held unwound — that opening is a transcription bubble — with an 8 bp RNA-DNA hybrid inside it, after which the RNA peels away and the duplex closes again behind. The chain grows at 50 to 90 nucleotides per second, and the enzyme covers about 35 bp of DNA while it does so. The likelier picture of the motion is that the RNA travels in a straight line while the DNA rotates beneath it, and in most cellular DNA that rotation is restricted by bound proteins and other structural barriers. So **waves of positive supercoils pile up ahead of the bubble and negative ones behind it** — supercoils being over- and under-winding of the double helix itself — and this has been seen both in a test tube and inside living bacteria; topoisomerases, the enzymes that cut DNA, let it turn and reseal it, are what take them out again.",
      cn: "这个酶把螺旋撑开一小段，并带着这个开口一起往前走。约 17 bp 被撑开 —— 这个开口就叫转录泡 —— 泡里有一段 8 bp 的 RNA-DNA 杂合链，过了这段 RNA 就剥离，双链在后面重新合上。链的延长速度是每秒 50 到 90 个核苷酸，酶在 DNA 上覆盖约 35 bp。关于这个运动，更可能的图景是：RNA 沿直线前进，而 DNA 在它下方转动；而细胞里的 DNA 大多被结合蛋白和其他结构性障碍挡着转不动。**于是泡的前方一波波堆出正超螺旋、后方堆出负超螺旋 —— 所谓超螺旋，就是双螺旋自身的过度缠绕或缠绕不足 —— 而把它们拆掉的是拓扑异构酶，即那类会把 DNA 切开、让它转动、再接回去的酶。** 这一现象在试管里和活细菌体内都被观察到过。",
    ...
    },
    {
      ...
      en: "RNA synthesis is processive, meaning the enzyme adds a great many nucleotides before letting go, and it has to be: an enzyme that released a transcript early could not resume that same RNA and would have to start again from the beginning of the gene. So stopping is signalled, in two ways. A rho-independent terminator is a G+C-rich palindrome, which is a sequence followed by its own reverse complement, so the new RNA folds back on itself into a hairpin centred 15 to 20 nucleotides before the end, and next to that G+C-rich stretch sits an A-T-rich run of 4 to 10 A residues in the template, read out as the oligo(U) that ends the transcript. A rho-dependent terminator has neither the hairpin nor that run; it carries a CA-rich rut element instead, where the protein rho binds the RNA and walks along it 5' to 3' on ATP until it catches the paused enzyme. Rho is itself an enzyme — it spends nucleoside triphosphates to unwind both an RNA-DNA and an RNA-RNA double helix — and rho-dependent termination needs its own recognition sequences upstream of the stopping point, which appear to sit on the new RNA rather than on the DNA. **The hairpin works by tearing apart several A=U pairs of the RNA-DNA hybrid, which are the weakest pairing available, so the transcript falls off by itself**; where the sequence cannot manage that, an enzyme is spent prising it off instead.",
      cn: "RNA 合成是持续性的，意思是酶要接上非常多个核苷酸才松手 —— 而且必须如此：一个提前放开转录本的酶没法接着抄同一条 RNA，只能从基因开头重来。所以「停」是要被信号通知的，方式有两种。不依赖 rho 的终止子是一段富含 G+C 的回文序列 —— 回文即一段序列后面跟着它自己的反向互补序列 —— 于是新生 RNA 折回自身形成一个发夹，中心位于终点前 15 到 20 个核苷酸处；紧挨着这段富含 G+C 的序列的，是模板上一段富含 A-T 的区域，4 到 10 个 A，被抄成收尾的寡聚(U)。依赖 rho 的终止子既没有发夹也没有这串 A；它带的是一段富含 CA 的 rut 元件，蛋白 rho 在那里结合到 RNA 上，靠 ATP 沿 RNA 从 5′ 向 3′ 走，直到追上停顿的酶。rho 本身就是一个酶 —— 它消耗核苷三磷酸，把 RNA-DNA 和 RNA-RNA 两种双螺旋都解开 —— 而依赖 rho 的终止还需要终止位点上游一段自己的识别序列，这段序列似乎位于新生 RNA 上而不是 DNA 上。**发夹起作用的方式，是把 RNA-DNA 杂合链里好几对 A=U 拽开 —— A=U 是所有配对里最弱的 —— 于是转录本自己脱落；而序列自己办不到的地方，就得花一个酶去把它撬下来。**",
    ...
    },
    {
      ...
      en: "Pol II is a twelve-subunit enzyme of more than 510,000 in aggregate molecular weight, and that bulk hides how conserved the machine is: its largest subunit RBP1 is closely homologous to the bacterial β' subunit, RBP2 resembles bacterial β, and RBP3 and RBP11 resemble the two bacterial α subunits. The extra complexity is there for a reason — Pol II must work on a larger genome whose DNA is far more elaborately packaged, and most of the difference is the protein-protein contact needed to get through that packing. What it cannot do alone is start. It needs general transcription factors, which are proteins required at every Pol II promoter, and they assemble in a fixed order: TBP, the TATA-binding protein, binds a short A-and-T-rich TATA box about 30 bp upstream, or else arrives inside a large complex called TFIID at the roughly 80% of human promoters that have no TATA box — there TFIID works as a scaffold, an elongated complex anchored by TBP on the DNA and stretching over more than 70 base pairs with the initiator element near its middle, which is what fixes where Pol II loads when there is no TATA box to measure from. Then come TFIIB, TFIIA, and TFIIF together with Pol II, TFIIF both aiming the polymerase at promoters and cutting down its grip on nonspecific DNA, and finally TFIIE and TFIIH, whose helicase opens the DNA exactly as the bacterial closed-to-open transition does. The finished assembly is the preinitiation complex, and counting every subunit it can exceed fifty polypeptides against six for the bacterial holoenzyme. The largest subunit of Pol II ends in a long tail made of many repeats of the same seven amino acids, -YSPTSPS-, 26 repeats in yeast and 52 in mouse and human, and kinases add phosphates to it at initiation and keep changing them through elongation. **The pattern of phosphates on that tail determines which RNA-processing machinery is bound to the complex**, so capping, splicing and tailing are not a later stage beginning when transcription ends — they are recruited onto a transcript that is still attached to the polymerase making it.",
      cn: "Pol II 是一个十二亚基的酶，总分子量超过 510,000；而这份体量掩盖着它有多保守：最大亚基 RBP1 与细菌的 β′ 亚基高度同源，RBP2 像细菌的 β，RBP3 与 RBP11 像细菌那两个 α。多出来的复杂性是有理由的 —— Pol II 要在一个更大、DNA 包装得精细得多的基因组上干活，而差别的大部分就是穿过这层包装所需的蛋白-蛋白接触。它自己做不到的事是「开始」。它需要通用转录因子，即每个 Pol II 启动子都要用到的那些蛋白，而且它们按固定顺序装配：TBP（TATA 结合蛋白）结合上游约 30 bp 处一段富含 A 和 T 的 TATA 盒；在没有 TATA 盒的启动子上（约占人类启动子的 80%），TBP 改为藏在一个叫 TFIID 的大复合物里前来 —— 这时 TFIID 充当脚手架：一个延展的复合物，一端由 TBP 锚在 DNA 上，沿 DNA 铺开 70 多个碱基对，起始元件大致落在中间，这正是「没有 TATA 盒可量」时 Pol II 装载位置的定法。接着是 TFIIB、TFIIA、带着 Pol II 的 TFIIF —— TFIIF 既把聚合酶瞄向启动子，又削弱它对非特异 DNA 的抓握 —— 最后是 TFIIE 与 TFIIH，后者的解旋酶把 DNA 撑开，干的正是细菌里从闭合到开放那一步的活。装好的这一坨叫前起始复合物，把所有亚基数进去可以超过五十条多肽，对照细菌全酶的六个。Pol II 最大的那个亚基末端拖着一条长尾巴，由同样七个氨基酸 -YSPTSPS- 重复很多次构成，酵母 26 个重复、小鼠和人 52 个；起始时激酶在它上面加磷酸，延长过程中还不断改变这些磷酸。**这条尾巴上的磷酸模式决定了哪一套 RNA 加工机器结合在复合物上 —— 所以加帽、剪接、加尾并不是转录结束后才开始的下一阶段，它们是被召集到「还挂在聚合酶上的转录本」上来的。**",
      src: "A p.969"
    },
    {
      link_en: "and the phosphates on that tail go on and come off in a fixed order, once per transcript",
      link_cn: "而这条尾巴上的磷酸，是按固定顺序上、按固定顺序下的 —— 每条转录本走一遍",
      en: "The tail is held away from the body of Pol II by an intrinsically disordered linker, which is a stretch of polypeptide with no fixed fold, so it can reach a long way from the enzyme. At initiation a kinase carried inside one of the TFIIH subunits puts phosphates on many positions along it, and that alone changes the shape of the whole assembly and starts transcription. Other kinases keep working on it as the chain grows, chief among them CDK9, which sits inside a complex named pTEFb, positive transcription elongation factor b, and which phosphorylates mainly the Ser residues of each seven-residue repeat. Elongation factors, which are proteins that ride along with the polymerase once a chain is under way, stop it pausing and hold the processing machines in step with each other; some of them are bound to the phosphorylated tail itself. **At termination every phosphate is taken off again and the whole apparatus is recycled, so the tail works as a clock that is wound at the start of a transcript and reset at its end.**",
      cn: "这条尾巴通过一段本征无序的连接子与 Pol II 的主体隔开 —— 本征无序即这段多肽没有固定折叠 —— 所以它能从酶身上伸出很远。起始时，藏在某个 TFIIH 亚基里的一个激酶在它的许多位点上接磷酸，光是这一下就改变了整个装配体的构象、把转录发动起来。链在延长的过程中还有别的激酶继续在它上面动手，其中最主要的是 CDK9：它待在一个叫 pTEFb（正性转录延伸因子 b）的复合物里，主要磷酸化每个七残基重复中的 Ser。延伸因子 —— 即链一旦开跑就随聚合酶一同前进的那些蛋白 —— 不让它停顿，并把各台加工机器彼此对齐；其中一些就结合在磷酸化的尾巴上。**到终止时，所有磷酸又被一一摘掉、整套装置被回收再用 —— 所以这条尾巴像一只钟：每开始一条转录本上一次弦，到它末尾归零。**",
      src: "A p.968 · A p.969 · A p.971"
    },
    {
    ...
    },
    {
      ...
      en: "The first nucleotide RNA polymerase lays down keeps its 5'-triphosphate, and the cap is built directly onto it: a guanosine residue is joined back-to-front through a 5'-to-5' triphosphate linkage that no ordinary RNA bond resembles, and then methyl groups are added — one to N-7 of that guanine, making it 7-methylguanosine, and in some transcripts one or two more onto the 2'-hydroxyls of the first and second nucleotides of the message, which is what grades a cap as 0, 1 or 2. The 3' end receives a poly(A) tail, which is a long run of adenylate residues added after transcription and copied from no template at all. Capping, splicing and tailing are not three independent workshops: their complexes are organised in association with one another and with the phosphorylated tail of Pol II, so a single tail schedules all three. Influenza carries no capping enzymes and steals one instead — its polymerase grabs a capped host transcript, cuts a short capped piece off the 5' end, and uses that stolen fragment to start viral RNA synthesis, which is the most direct evidence that a message without a cap is not usable. The tail is the opposite kind of lesson: many bacterial mRNAs get poly(A) tails as well, and there the tail stimulates decay instead of preventing it, so what a tail means has to be read off the organism and not off the chemistry. **A capped 5' end no longer looks like an end, so the nucleases that chew inwards from one cannot get a grip on it**, and the cap doubles as the landmark the protein-making machinery uses to find the message at all.",
      cn: "RNA 聚合酶放下的第一个核苷酸保留着自己的 5′-三磷酸，帽子就直接搭在它上面：一个鸟苷残基以头对头的方式、通过一个 5′-对-5′ 三磷酸连接接上去 —— 普通 RNA 里没有任何键长这样 —— 随后再加甲基：一个加在那个鸟嘌呤的 N-7 上，使它成为 7-甲基鸟苷；某些转录本还会在信息头两个核苷酸的 2′-羟基上再加一到两个，帽子分成 0 型、1 型、2 型，靠的正是甲基化走到了哪一步。3′ 端得到的是 poly(A) 尾巴，即转录之后接上去的一长串腺苷酸，不抄自任何模板。加帽、剪接、加尾并不是三个各干各的车间：它们的复合物彼此关联，也与 Pol II 磷酸化的那条尾巴关联，所以一条尾巴同时给三件事排了班。流感病毒不带加帽酶，于是去偷一个：病毒聚合酶抓住宿主一条已加帽的转录本，从 5′ 端切下一小段带帽的片段，用它来起头合成病毒 RNA —— 这是「没有帽子的信息不能用」最直接的证据。尾巴给的是相反方向的教训：许多细菌 mRNA 也会加 poly(A) 尾，而在那里尾巴促进降解、而不是阻止降解 —— 所以尾巴意味着什么，要看是什么生物，而不是看化学。**加了帽的 5′ 端不再像一个「端」，那些从 5′ 端往里啃的核酸酶因此无从下口；同时帽子还兼作地标，造蛋白的机器正是靠它才找得到这条信息。**",
      src: "A p.973–975"
    },
    {
    ...
    },
    {
      ...
      en: "One pre-mRNA can be spliced in more than one way: entire exons can be left out or put in, and a splice site a few nucleotides away from the usual one can be used instead. This is alternative splicing, and it produces several different proteins from one gene. It affects a small fraction of yeast transcripts and more than 95% of human genes, which resolves a real puzzle — how complex an organism is does not track how many protein-coding genes it has. Spinal muscular atrophy shows what forcing that decision is worth. The disease is caused by the loss of a working SMN protein, which patients cannot make because their SMN1 gene is gone; they still carry a near-identical paralog of it, a duplicate gene called SMN2, which by default skips exon 7 and therefore makes an unstable, useless protein. **Nusinersen is an antisense oligonucleotide, a short synthetic piece of nucleic acid built to be complementary to the exact sequence in the SMN2 transcript that causes exon 7 to be skipped**. It pairs with that sequence and hides it from the splicing machinery, exon 7 goes in, working SMN protein appears, and the disease stops progressing — splicing is a drug target, and this is what it looks like when the target is hit.",
      cn: "同一条前体 mRNA 可以有不止一种剪法：整个外显子可以被跳过或被保留，也可以改用离常规位点只差几个核苷酸的另一个位点。这叫可变剪接，它让一个基因产出好几种不同的蛋白。它影响酵母转录本中的一小部分，以及超过 95% 的人类基因 —— 这解决了一个真实的困惑：一个生物有多复杂，并不随它有多少编码蛋白的基因而变化。脊髓性肌萎缩症则说明了「强行改变这个决定」值多少钱。这个病的起因是缺失有功能的 SMN 蛋白：患者的 SMN1 基因没了，做不出它；但他们身上还有一个几乎完全相同的旁系同源基因（即一个重复出来的基因）SMN2，而 SMN2 默认会跳过第 7 外显子，因此做出一个不稳定、没有用的蛋白。诺西那生是一条反义寡核苷酸，即一段人工合成的短核酸，被造成与 SMN2 转录本上「导致第 7 外显子被跳过」的那段序列互补。**它与那段序列配对、把它从剪接机器眼前遮住，第 7 外显子于是被留下，有功能的 SMN 蛋白出现，病程停止推进 —— 剪接是一个药物靶点，而打中这个靶点，长的就是这个样子。**",
      src: "A p.981"
    },
    {
    ...
    {
      link_en: "and one thing is still missing from the account: how long any of these RNAs is allowed to last",
      link_cn: "而叙述里还缺一样东西：这些 RNA 被允许存在多久",
      en: "How long an RNA survives is a regulated quantity in itself, and a developing organism has to be able both to make a message at the right moment and to destroy it at the right moment. The numbers are far apart: the half-life of a bacterial mRNA, meaning the time in which half of it is gone, is about 1.5 minutes, against roughly 3 hours for the average mRNA of a vertebrate cell — a bacterium rewrites its protein output on the timescale of a changing medium, while a vertebrate cell needs most of its gene products for much longer. The routes differ to match. Bacterial decay opens with an endonuclease — an enzyme that cuts a chain in the middle — striking inside the transcript in spite of the 5'-triphosphate that would otherwise guard its end. A eukaryotic message is decapped first and its poly(A) tail shortened, and only then can exonucleases, which are enzymes that chew a chain in from an end, get hold of it; one of them is a large multi-subunit machine called the exosome. **The steady-state amount of any RNA is set by how fast it is made against how fast it is destroyed, so turnover is one of the places regulation actually happens.**",
      cn: "一条 RNA 能活多久，本身就是一个受调控的量；一个正在发育的生物必须既能在正确的时刻造出某条信息，也能在正确的时刻把它毁掉。两边的数字差得很远：细菌 mRNA 的半衰期（即一半被清掉所需的时间）约为 1.5 分钟，而脊椎动物细胞里一条 mRNA 的平均半衰期约为 3 小时 —— 细菌要在培养基变化的时间尺度上重写自己的蛋白产出，而脊椎动物细胞的大多数基因产物需要维持得久得多。路径也相应不同。细菌的降解从一个核酸内切酶开始 —— 那是一类从链的中间切开的酶 —— 它直接切进转录本内部，尽管本该守住 5′ 端的三磷酸还在。真核信息则先被脱帽、poly(A) 尾被缩短，此后核酸外切酶（即从链的一端往里啃的酶）才够得着它；其中一台是叫外泌体（exosome）的大型多亚基机器。**任何一种 RNA 的稳态含量，都由「造得多快」对「毁得多快」定出来 —— 所以周转本身，正是调控真正发生的场所之一。**",
      src: "A p.987"
    },
    {
      link_en: "and a whole class of small RNA exists to aim that destruction at one message and no other",
      link_cn: "而有一整类小 RNA 的存在，就是为了把这份销毁精确地对准某一条信息",
      en: "A microRNA is a single-stranded RNA about 22 nucleotides long whose sequence is complementary to a particular region of some messenger RNA, and pairing with that region either gets the message degraded or blocks its translation. About 1,500 human genes encode one, and between them they touch the expression of most protein-coding genes, so this is a layer of control over the genome rather than a special case. Making one runs as a two-cut pipeline through two compartments. A long primary transcript called the pri-miRNA — some of them lying inside the introns of other genes and made whenever those host genes are made — is cut in the nucleus by an enzyme named Drosha into a short hairpin precursor, the pre-miRNA. That is exported to the cytoplasm and cut again by Dicer into the mature 22-nucleotide RNA, which is loaded into a protein complex called RISC. **RISC cleaves a target message outright where the pairing is nearly perfect and only blocks its translation where the pairing is partial, so how well two RNAs match decides the fate of a gene.**",
      cn: "microRNA 是一条长约 22 个核苷酸的单链 RNA，序列与某条信使 RNA 上的某个特定区域互补；一旦与那个区域配上对，这条信息要么被降解，要么翻译被阻断。人类约有 1,500 个基因编码 microRNA，而它们合起来触及大多数编码蛋白的基因的表达 —— 所以这是覆盖基因组的一层控制，不是一个特例。造一条 microRNA 是一条穿过两个区室、切两刀的流水线。一条较长的初级转录本叫 pri-miRNA —— 其中一些就躺在别的基因的内含子里，宿主基因一被合成它也跟着被合成 —— 在细胞核里被一个叫 Drosha 的酶切成一段短的发夹前体，即 pre-miRNA。它被输出到细胞质，再被 Dicer 切成成熟的那条 22 核苷酸 RNA，随后装载进一个叫 RISC 的蛋白复合物。**配得几乎完美时，RISC 直接把靶信息切断；只是部分互补时，它仅仅阻断翻译 —— 所以两条 RNA 配得有多准，决定了一个基因的命运。**",
      src: "A pp.985–986"
    }
  ],
  terms: [
...
window.BIOLITE_SPINE["key:nucleotides"] = {
  ...
  steps: [
    {
      en: "Nucleic acids stand at one of the first places in the hierarchy of substances living matter needs, because they are the substances that carry the information for the course of every life process. A nucleic acid is a chain of one repeating unit, and that unit has exactly three parts: a nitrogen-containing ring called a base, a five-carbon sugar called a pentose, and a residue of phosphoric acid, the phosphate. A unit carrying all three is a nucleotide; a base joined to a pentose with no phosphate at all is a nucleoside, so the two words differ by exactly one phosphate group. The base is held to carbon 1 of the pentose by an N-glycosidic bond, which runs from a nitrogen of the base to that sugar carbon — from N9 in a purine and from N1 in a pyrimidine. **That single bond is the whole of what holds a base onto the chain: break it and the chain survives intact, carrying a sugar with no base at all.**",
      cn: "在生命物质所需的各类物质里，核酸排在最前面的位置之一，因为它们正是承载着一切生命过程如何进行的那份信息的物质。核酸是一条由同一种重复单元串成的链，而这个单元恰好由三部分组成：一个含氮的环，叫碱基；一个五碳糖，叫戊糖；以及一个磷酸残基，即磷酸。三部分齐全的单元就是核苷酸；碱基只接在戊糖上、一个磷酸都没有，那叫核苷 —— 所以这两个词的差别恰好是一个磷酸基团。碱基通过一根 N-糖苷键接在戊糖的第 1 位碳上，这根键从碱基上的一个氮出发连到糖的那个碳 —— 嘌呤用 N9，嘧啶用 N1。**把碱基固定在链上的，全部就是这一根键：断掉它，链完好无损，只是那个糖上没有碱基了。**",
    ...
    },
    {
      ...
      en: "Five bases carry almost all of the work. Adenine and guanine are purines, meaning the ring system is two rings fused together; cytosine, uracil and thymine are pyrimidines, a single six-membered ring. In a written sequence each is one letter: A, G, C, U, T. The names then fall into a ladder with regular endings — adenine gives the nucleoside adenosine and the nucleotide adenylate, abbreviated AMP; guanine gives guanosine and GMP; uracil gives uridine and UMP; cytosine gives cytidine and CMP — so a purine nucleoside ends in -osine, a pyrimidine nucleoside ends in -idine, and adding the phosphate turns the ending into -ylate. Replace the ribose with 2-deoxy-D-ribose and every name takes the prefix deoxy-: adenine gives deoxyadenosine and deoxyadenylate, written dAMP, guanine gives deoxyguanosine and dGMP, cytosine gives deoxycytidine and dCMP, and each of those abbreviations means the 5'-monophosphate. That row contains no uracil at all, because thymine stands in its place, giving deoxythymidine and dTMP. Each pyrimidine can also sit in either of two tautomeric forms, meaning the same atoms with one hydrogen moved: the lactam form, which is the more stable, and the lactim form, which is weakly acidic. **Uracil occurs practically only in RNA and thymine predominantly in DNA, which makes the base the quicker of the two tests: find a U and you are holding RNA.**",
      cn: "承担几乎全部工作的碱基有五种。腺嘌呤（adenine, A）和鸟嘌呤（guanine, G）属于嘌呤，意思是它们的环系由两个环稠合而成；胞嘧啶（cytosine, C）、尿嘧啶（uracil, U）和胸腺嘧啶（thymine, T）属于嘧啶，只有一个六元环。写成序列时每一种只用一个字母：A、G、C、U、T。名字随后排成一道词尾规整的阶梯 —— 腺嘌呤给出核苷腺苷、核苷酸腺苷酸（缩写 AMP）；鸟嘌呤给出鸟苷和 GMP；尿嘧啶给出尿苷和 UMP；胞嘧啶给出胞苷和 CMP —— 所以嘌呤核苷以 -osine 结尾，嘧啶核苷以 -idine 结尾，加上磷酸后词尾变成 -ylate。把核糖换成 2-脱氧-D-核糖，每个名字都加上前缀 deoxy-：腺嘌呤给出脱氧腺苷与脱氧腺苷酸（dAMP），鸟嘌呤给出脱氧鸟苷与 dGMP，胞嘧啶给出脱氧胞苷与 dCMP —— 这些缩写指的都是 5′-单磷酸。这一行里根本没有尿嘧啶，因为那个位置由胸腺嘧啶顶上，给出脱氧胸苷与 dTMP。每一种嘧啶还可以处在两种互变异构形式中的任一种（即同一批原子、挪动一个氢）：酰胺式（lactam）更稳定，亚胺醇式（lactim）呈弱酸性。**尿嘧啶实际上只出现在 RNA 里，胸腺嘧啶主要出现在 DNA 里，所以碱基是两者中更快的判据：看到 U，手上拿的就是 RNA。**",
    ...
    },
    {
      ...
      en: "Friedrich Miescher isolated DNA in 1869 and named the phosphorus-containing material he had nuclein, and for the seventy years after that there was no compelling evidence that it carried heredity. The double helix was worked out from four experiments, and the first of them supplied that evidence. Pneumococcus comes in two forms: S bacteria have a polysaccharide capsule and kill mice; R bacteria have no capsule and do not. Add an extract of heat-killed S bacteria to living R bacteria and the R bacteria turn into S bacteria, permanently and heritably. **Avery, MacLeod and McCarty purified that extract step by step, and what was left when they had finished was DNA.**",
      cn: "Friedrich Miescher 在 1869 年就分离出了 DNA，把这种含磷的物质命名为「核素」（nuclein）；此后的七十年里，并没有令人信服的证据表明它携带遗传信息。双螺旋是从四组实验推出来的，其中第一组补上了这个证据。肺炎链球菌有两型：S 菌有多糖荚膜，能让小鼠死亡；R 菌没有荚膜，不致死。把加热杀死的 S 菌提取物加进活的 R 菌里，R 菌变成了 S 菌，而且这个性状会一代代传下去。**Avery、MacLeod 与 McCarty 一步步纯化那份提取物，纯化到最后剩下的是 DNA。**",
      src: "A p.269",
      beyond: true,
    ...
    },
    {
      ...
      en: "Chargaff measured base composition species by species through the late 1940s and found A equal to T and G equal to C every time, from which purines equal pyrimidines and A + G = T + C. These are Chargaff's rules. The ratio between the four bases varies from one organism to another, but within one species the composition is the same in different tissues and does not shift with age, environment, nutritional state or generation, so it is a property of the organism rather than of the sample. **Any model put forward for the shape of DNA had to produce those two equalities.**",
      cn: "Chargaff 在 1940 年代末一个物种一个物种地测碱基组成，每次都得到 A = T、G = C；由此可推出嘌呤总量等于嘧啶总量，即 A + G = T + C。这就是 Chargaff 规则。四种碱基之间的比例因生物而异，但在同一物种内部，不同组织的碱基组成相同，也不随年龄、环境、营养状态或世代而改变 —— 所以它是这个生物的属性，不是这份样品的属性。**任何为 DNA 形状提出的模型，都必须给出这两个等式。**",
      src: "A p.270"
    },
    {
    ...
    {
      link_en: "and set side by side the three forms differ by measured amounts, not by adjectives",
      link_cn: "而把三种形式并排放，它们的差别是量出来的数，不是形容词",
      en: "Handedness: A and B are right-handed, Z is left-handed. Diameter: about 26 Å for A, 20 for B and 18 for Z — A is the fat one and Z the thin one, which is the reverse of what calling Z slender suggests. Base pairs per helical turn: 11, 10.5 and 12. Rise along the axis per base pair: 2.6, 3.4 and 3.7 Å. Tilt of the base pairs away from perpendicular to the axis: 20 degrees in A, 6 in B, 7 in Z, so only A has its pairs noticeably tipped, which is what makes it both wider and shorter per pair. Sugar pucker, meaning which carbon of the sugar ring is pushed out of the plane of the other four: C-3' endo in A, C-2' endo in B, and in Z the two alternate, C-2' endo under pyrimidines and C-3' endo under purines. The glycosidic bond is in the anti conformation throughout, except in Z: **there the purines flip to syn and alternate with anti pyrimidines, and that alternation is what throws the backbone into the zigzag Z is named after.**",
      cn: "螺旋手性：A 与 B 是右手，Z 是左手。直径：A 约 26 Å，B 约 20 Å，Z 约 18 Å —— A 是最粗的，Z 是最细的，与「Z 更细长」这个说法给人的印象正好相反。每圈碱基对数：11、10.5、12。每对沿轴的上升：2.6、3.4、3.7 Å。碱基对相对于「垂直于轴」的倾角：A 为 20 度，B 为 6 度，Z 为 7 度 —— 所以只有 A 的碱基对明显倾斜，而这正是它每对更宽、也更矮的原因。糖环褶皱（sugar pucker），即糖环上哪一个碳被推出其余四个所在的平面：A 是 C-3′ endo，B 是 C-2′ endo，Z 则交替，嘧啶下面用 C-2′ endo、嘌呤下面用 C-3′ endo。糖苷键的构象一律是 anti，只有 Z 例外：**在 Z 里嘌呤翻成 syn、与 anti 的嘧啶交替出现，而正是这种交替把骨架扭成了那道锯齿 —— Z 的名字就是从它来的。**",
      src: "A p.273, figure 8-17"
    },
    {
      link_en: "and two of those three forms need a caveat about where they actually occur, running opposite ways",
      link_cn: "而其中两种形式各需要一句关于「它究竟出现在哪里」的补充 —— 而这两句的方向相反",
      en: "The A form is favoured in solutions relatively devoid of water, and there is a catch of method attached to it: the reagents used to make DNA crystallise tend to dehydrate it, so most short DNA molecules crystallise as A whether or not that is the shape they hold in a cell. Z runs the other way. Short tracts of Z-DNA have been found in both bacteria and eukaryotes, so its presence in living cells is not in doubt, and the role suggested for those tracts is in regulating the expression of some genes or in genetic recombination. **A form can be thoroughly characterised in a crystal and still uncertain in a cell, so what shape DNA can take and what shape DNA does take are two separate questions.**",
      cn: "A 型在相对缺水的溶液中占优；而这里附带一个方法学上的陷阱：用来促使 DNA 结晶的试剂往往使它脱水，所以大多数短 DNA 分子都以 A 型结晶 —— 不管它在细胞里是不是这个形状。Z 型的情况反过来。在细菌和真核生物中都找到了短的 Z-DNA 区段，所以它在活细胞里的存在没有疑问；而这些区段被认为可能参与某些基因表达的调控、或参与遗传重组。**一个构型可以在晶体里被表征得很透彻，同时在细胞里仍然存疑 —— 所以「DNA 能取什么形状」与「DNA 实际取什么形状」是两个分开的问题。**",
      src: "A p.272 · A p.273",
      openQuestion_en: "Whether A-DNA occurs in cells at all is uncertain, and the dehydrating crystallisation reagents are why the crystal structures cannot settle it.",
      openQuestion_cn: "A-DNA 在细胞里究竟出不出现，目前不确定；而使人无法用晶体结构来定论的，正是那些让样品脱水的结晶试剂。"
    },
    {
    ...
    },
    {
      ...
      en: "The product of transcribing DNA is always a single strand of RNA, and the rest of RNA structure follows from that one fact. A lone strand still stacks its bases, taking up a right-handed conformation dominated by that stacking, and purine-purine stacking is strong enough that a pyrimidine caught between two purines is often pushed out of the stack so the two purines can reach each other. Wherever the strand finds a nearby stretch complementary to itself, it folds back and pairs. **That fold is a hairpin, and a hairpin is a signal: one appearing in a growing transcript is what makes RNA polymerase let go and stop transcribing.** Long runs of perfect complementarity are uncommon, so breaks in the paired region are normal and appear as bulges and internal loops; hairpins are for that reason the commonest secondary structure in RNA. A few short loop sequences, UUCG among them, are known to form particularly tight and stable loops and may be where folding starts, which is an RNA counterpart of a protein's folding nucleus.",
      cn: "转录 DNA 的产物永远是一条单链 RNA，而 RNA 结构的其余一切都由这一条推出。一条孤零零的链照样会堆积自己的碱基，取一个由堆积主导的右手构象；而嘌呤与嘌呤之间的堆积强到这种程度：夹在两个嘌呤中间的嘧啶常被挤出堆积之外，好让两个嘌呤彼此够到。只要这条链在附近找到与自身互补的一段，它就折回去配对。**这个折叠就是发夹 —— 而发夹是一个信号：正在延长的转录本上出现一个发夹，RNA 聚合酶就会松开、停止转录。** 长段的完美互补并不常见，所以配对区里出现断口是常态，表现为凸起（bulge）和内环；发夹也正因如此成为 RNA 中最常见的二级结构。少数几种短环序列（UUCG 是其中之一）已知会形成特别紧密而稳定的环，并可能就是折叠的起点 —— 相当于蛋白质折叠核心的 RNA 版。",
      src: "A p.275–277"
    },
    {
      ...
      en: "An RNA strand ends up double-stranded in exactly two situations: it folds back and pairs with itself, which is the hairpin above, or it pairs with the DNA template it is being copied from, which is the hybrid inside the polymerase. Both of those short duplexes are A-form, never B-form — the B form of RNA has never been observed, and Z-form RNA has only been made in the laboratory at very high salt or very high temperature — and the rest of the molecule is single-stranded and no helix at all. The partners are not restricted to Watson-Crick pairs either: G can pair with U, but only when strands already synthesised fold up or anneal to each other, never while a strand is being made. **DNA gets to pick B because its sugar has no 2'-hydroxyl; RNA has one, it does not fit B-form geometry, and so a paired stretch of RNA has no choice — A is the only form open to it.**",
      cn: "一条 RNA 只有两种情况会变成双链：一是它折回来和自己配对，就是上一步说的发夹；二是它和正在被抄写的那条 DNA 模板配对，就是聚合酶内部那段杂合链。这两种短双链都是 A 型、绝不会是 B 型 —— RNA 的 B 型从未被观察到，Z 型 RNA 也只在实验室里、在极高盐或极高温下被造出来过 —— 而分子的其余部分是单链，根本不构成螺旋。配对的搭档也不限于 Watson-Crick：G 可以与 U 配，但只在已经合成好的链折叠起来、或彼此退火时才行，绝不发生在一条链正被合成的过程中。**DNA 之所以能选 B 型，是因为它的糖上没有 2′-羟基；RNA 有，而这个羟基塞不进 B 型的几何 —— 所以配上对的 RNA 没得选，A 型是它唯一能用的形式。**",
