/* Biochemie PRO 速通简洁版 — the spine, and three things around it.

   The full app carries five or more layers per node (必背 411 characters median,
   概要 1,479, 逐条要点 1,955, plus terms and an oral answer), and none of them is
   short. Measured over the corpus, one body sentence in five is about the textbook
   rather than about biochemistry, and 409 glossary terms are used before the node
   that defines them — a median of 30 nodes early. Read straight through, concepts
   arrive from nowhere and there is no main thread.

   This app is the thread: one chain per node, each step one claim, in order. Around
   it, only what the reader asked for — 题目, 星标, 语音. Nothing else.

   It does NOT copy the prose. data/nodes.js and data/questions.js are generated from
   the full app's own data; data/spine_*.js is written by hand. Forking the 9 MB of
   text would mean repairing every defect twice and the copies drifting in silence.

   Storage prefix `biolite.` — its own box, separate from `biopro.`. */
...
(function () {
  ...
  /* The full app routes on `#/t/<id>`, NOT on a bare `#<id>`. Getting this wrong is
     the quiet kind of broken: every 详见 link would still open the app, just on
     whatever node it had open last, and nothing would report an error. Checked
     against topicFromHash() in biochemie_pro/app.js, and asserted by the link check
     in tools/check-lite-links.py. */
  const FULL_APP = '../biochemie_pro/index.html#/t/';

  const store = {
    get(k, d) {
      try { const r = localStorage.getItem('biolite.' + k); return r === null ? d : JSON.parse(r); }
      catch (e) { return d; }
    },
    set(k, v) {
      try { localStorage.setItem('biolite.' + k, JSON.stringify(v)); } catch (e) {}
    }
  };
  ...
  /* ------------------------------------------------- ONE SUBJECT, ONE SPINE 按主题
     The two books are not two subjects. 281 nodes resolve into 78 topicKeys — 44 of
     them holding both a Lehninger node and a Czech one about the same thing, 18 Czech
     only, 16 Lehninger only. Listing all 281 puts the same subject on the shelf twice
     and makes the reader choose between two accounts of it, which is the opposite of
     a thread.

     So a spine may be keyed by TOPIC (`key:<topicKey>`) instead of by node, and a
     topic that has one is shown once, with the sections it covers named in its header.
     Per-node spines still resolve, so the two can coexist while the merge proceeds. */
  ...
  /* Grouped by topicKey, not by book, and Lehninger first inside each group.
     Requested, and the reason given was that Lehninger is the better and more
     current source — so it should be the node you land on, with the Czech section on
     the same subject sitting under it rather than in a separate half of the app. */
  function groups() {
    const by = new Map();
    ...
    by.forEach((members, key) => {
      members.sort((a, b) =>
        (a.book === 'lehninger' ? 0 : 1) - (b.book === 'lehninger' ? 0 : 1) ||
        secKey(a).localeCompare(secKey(b)));
      out.push({ key: key, members: members });
    });
    // Order the groups by their leading member, so the sidebar is stable and
    // Lehninger chapter order drives it.
    out.sort((a, b) => {
      const A = a.members[0], B = b.members[0];
      return (A.book === 'lehninger' ? 0 : 1) - (B.book === 'lehninger' ? 0 : 1) ||
    ...
    groupsByArea().forEach((block) => {
      ...
      shown.forEach((n) => {
        ...
        html += '<button class="topic-item' + (n.id === state.id ? ' current' : '')
              ...
              + '<span class="ti-src src-' + esc(n.book) + '">'
              + (n.book === 'lehninger' ? 'LEH' : 'CZ') + '</span>'
  ...
  /* "这些东西一般有啥用？…有没有 app 其他章节都在什么地方有详细讲解？"
     ...
     A target that has its own spine opens HERE; one that does not opens in the full
  ...
  function seeHtml(s) {
    ...
    return '<span class="ch-see"><span class="see-label">详见</span>' + refs.map((r) => {
      ...
      const href = here ? '#' + encodeURIComponent(r.id)
                        : FULL_APP + encodeURIComponent(r.id);
      return '<a class="see-link' + (here ? ' see-here' : '') + '" href="' + href + '"'
        + ' data-goto="' + (here ? esc(r.id) : '') + '" title="' + esc(n.enTitle || '') + '">'
        + '<span class="see-sec">' + esc(n.section) + '</span> ' + esc(note)
        + (here ? '' : ' <span class="see-full">📖</span>') + '</a>';
    }).join('') + '</span>';
  }
  ...
  function renderMinimal(n, sp, head, full) {
    ...
    html += '<p class="to-full"><a class="btn-ghost" href="' + full + '">📖 完整版 · '
          + n.nPoints + ' 条要点</a></p>';
  ...
  function renderStudy() {
    ...
    const sp = spineOf(n);
    const full = FULL_APP + encodeURIComponent(n.id);
    /* A merged topic names every section it covers, each a link into the full app —
    ...
    let html = '<article class="topic"><div class="topic-head"><div class="th-meta">'
      + (isMerged(n)
          ? '<span class="book-pill is-merged">两书合并 · merged</span>'
            + family.map((x) => '<a class="th-secref" href="' + FULL_APP + encodeURIComponent(x.id)
                + '" title="' + esc(x.enTitle || '') + '">'
                + (x.book === 'lehninger' ? 'LEH ' : 'CZ ') + esc(x.section) + '</a>').join('')
          : '<span class="book-pill' + (n.book === 'cz' ? ' is-cz' : '') + '">'
            + esc(n.book === 'cz' ? 'Czech textbook' : 'Lehninger 8') + '</span>'
    ...
    if (!sp) {
      html += '<div class="no-spine-note"><p><strong>这一节的主线还没写。</strong></p>'
        + '<p class="muted">完整版里有 ' + n.nPoints + ' 条要点、约 '
        + Math.round(n.detail / 1000) + ' 千字符正文、' + n.nTerms + ' 个术语。</p>'
        + '<p><a class="btn-ghost" href="' + full + '">📖 到完整版看这一节</a></p></div>';
      pane.innerHTML = html + '</article>';
      wireSay(pane);
      return;
    }

    if (state.mode === 'quiz') { renderQuiz(n, sp, html, full); return; }
    if (state.mode === 'min')  { renderMinimal(n, sp, html, full); return; }
    ...
    html += '<p class="to-full"><a class="btn-ghost" href="' + full + '">📖 完整版 · '
          + n.nPoints + ' 条要点、' + n.nTerms + ' 个术语</a></p>';
    pane.innerHTML = html + '</article>';
    wireSay(pane); wireMarks(pane);
