...
GROUP_META = collections.OrderedDict([
 ("sugar", ("sugars_more.js", "sugar-more", "糖（其余）", "Sugars, the rest",
   ...
   "　｜　"
   "为什么有几个糖只有开链、没有画成环？分三种情况，别混在一起："
   "（1）成不了环——丙糖（甘油醛、二羟丙酮，以及它们的磷酸酯 GAP、DHAP、甘油-3-磷酸）。"
   "成环靠的是链上后面某个羟基去进攻羰基碳：五元的呋喃糖需要 C4 上的 -OH，"
   "六元的吡喃糖需要 C5 上的 -OH。三个碳两个都没有，所以它们根本没有环式——"
   "这是化学，不是漏画。"
   "（2）开链是一对里的一半——葡萄糖、半乳糖、甘露糖、果糖、核糖、脱氧核糖、木糖、"
   "阿拉伯糖：开链（Fischer）和 α/β 环式（Haworth）都画了，配成一套看。"
   "（3）PubChem 上没有对应的环式条目——赤藓糖和木酮糖的呋喃型查不到，所以只画了开链，"
