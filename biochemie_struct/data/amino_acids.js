...
window.STRUCT.groups = (window.STRUCT.groups || []).concat([
{
  ...
  note: {
    cn: "按侧链性质分成五组——这个分组本身就是考点，比逐个背结构有用。每张卡并排画两个形式：左边中性未解离型，右边水溶液中的两性离子（zwitterion，α-COO⁻ 加 α-NH₃⁺）。右边才是水里真实存在、考试要你写的那个；中性型在水中的平衡里只占极小一部分，见 2-1-2。每张卡右上角标的是 pH 7 下的净电荷：Asp/Glu 为 −1，Lys/Arg 为 +1，其余 16 个为 0。⚠️ 注意「碱性」不等于「带正电」——His 的侧链是碱性的，但它的咪唑 pK 接近生理 pH，在 pH 7 下以未质子化为主，所以 His 的净电荷是 0，和 Lys/Arg 的 +1 不同。这一条常考。",
    en: "Grouped by side-chain character, because the grouping is what gets examined. Drawn as the aqueous zwitterion (alpha-COO- / alpha-NH3+), which is the form exams ask for. Each card carries its net charge at pH 7: -1 for Asp and Glu, +1 for Lys and Arg, 0 for the other sixteen. Note that basic does not mean positively charged: histidine's side chain is basic but mostly unprotonated at pH 7, so histidine sits at 0, not +1."
  },
  items: [
    /* ---- nonpolar, aliphatic ---- */
    { key: "gly", cn: "甘氨酸", en: "Glycine",       tlc: "Gly", olc: "G", cls: "nonpolar",
      ...
      smiles: "NCC(=O)O",                 formula: "C2H5NO2",    cid: 750, q7: 0,
    ...
    { key: "ala", cn: "丙氨酸", en: "Alanine",       tlc: "Ala", olc: "A", cls: "nonpolar",
      ...
      smiles: "C[C@@H](N)C(=O)O",         formula: "C3H7NO2",    cid: 5950, q7: 0 },
    { key: "val", cn: "缬氨酸", en: "Valine",        tlc: "Val", olc: "V", cls: "nonpolar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "C", x: 0.00, y: 2.0 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[5,7]] },
      smiles: "CC(C)[C@@H](N)C(=O)O",     formula: "C5H11NO2",   cid: 6287, q7: 0, essential: true },
    { key: "leu", cn: "亮氨酸", en: "Leucine",       tlc: "Leu", olc: "L", cls: "nonpolar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "C", x: 1.73, y: 3.0 }, { el: "C", x: 2.60, y: 1.5 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7],[6,8]] },
      smiles: "CC(C)C[C@@H](N)C(=O)O",    formula: "C6H13NO2",   cid: 6106, q7: 0, essential: true },
    { key: "ile", cn: "异亮氨酸", en: "Isoleucine",  tlc: "Ile", olc: "I", cls: "nonpolar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 0.00, y: 2.0 }, { el: "C", x: 1.73, y: 2.0 }, { el: "C", x: 1.73, y: 3.0 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[5,7],[7,8]] },
      smiles: "CC[C@H](C)[C@@H](N)C(=O)O", formula: "C6H13NO2",  cid: 6306, q7: 0, essential: true,
      note: { cn: "侧链上还有第二个不对称碳，所以它有四种立体异构体，而蛋白质里只用其中一种。" } },
    { key: "pro", cn: "脯氨酸", en: "Proline",       tlc: "Pro", olc: "P", cls: "nonpolar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 1 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.62, y: 1.45 }, { el: "C", x: -0.38, y: 1.53 }, { el: "C", x: -0.79, y: 0.62 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7],[7,0]] },
      smiles: "OC(=O)[C@@H]1CCCN1",       formula: "C5H9NO2",    cid: 145742, q7: 0,
    ...
    { key: "met", cn: "甲硫氨酸", en: "Methionine",  tlc: "Met", olc: "M", cls: "nonpolar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "S", x: 0.87, y: 2.5 }, { el: "C", x: 1.73, y: 3.0 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7],[7,8]] },
      smiles: "CSCC[C@@H](N)C(=O)O",      formula: "C5H11NO2S",  cid: 6137, q7: 0, essential: true,
    ...
    { key: "phe", cn: "苯丙氨酸", en: "Phenylalanine", tlc: "Phe", olc: "F", cls: "aromatic",
      ...
      smiles: "N[C@@H](Cc1ccccc1)C(=O)O", formula: "C9H11NO2",   cid: 6140, q7: 0, essential: true },
    { key: "tyr", cn: "酪氨酸", en: "Tyrosine",      tlc: "Tyr", olc: "Y", cls: "aromatic",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 0.87, y: 2.5 }, { el: "C", x: 1.73, y: 3.0 }, { el: "C", x: 1.73, y: 4.0 }, { el: "C", x: 0.87, y: 4.5 }, { el: "C", x: 0.00, y: 4.0 }, { el: "C", x: 0.00, y: 3.0 }, { el: "O", x: 0.87, y: 5.5, h: 1 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7,2],[7,8],[8,9,2],[9,10],[10,11,2],[11,6],[9,12]] },
      smiles: "N[C@@H](Cc1ccc(O)cc1)C(=O)O", formula: "C9H11NO3", cid: 6057, q7: 0,
      note: { cn: "苯环上多一个 -OH，于是它可以被磷酸化——酪氨酸激酶就是干这个的，信号传导里到处是它。" } },
    { key: "trp", cn: "色氨酸", en: "Tryptophan",    tlc: "Trp", olc: "W", cls: "aromatic",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 0.87, y: 2.5 }, { el: "C", x: 1.80, y: 2.85 }, { el: "N", x: 1.55, y: 3.82, h: 1 }, { el: "C", x: 0.52, y: 3.90 }, { el: "C", x: 0.06, y: 2.98 }, { el: "C", x: -0.94, y: 2.85 }, { el: "C", x: -1.48, y: 3.71 }, { el: "C", x: -1.02, y: 4.63 }, { el: "C", x: -0.02, y: 4.76 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7,2],[7,8],[8,9],[9,10,2],[10,6],[10,11],[11,12,2],[12,13],[13,14,2],[14,9]] },
      smiles: "N[C@@H](Cc1c[nH]c2ccccc12)C(=O)O", formula: "C11H12N2O2", cid: 6305, q7: 0, essential: true,
    ...
    { key: "ser", cn: "丝氨酸", en: "Serine",        tlc: "Ser", olc: "S", cls: "polar",
      ...
      smiles: "N[C@@H](CO)C(=O)O",        formula: "C3H7NO3",    cid: 5951, q7: 0,
      note: { cn: "侧链羟基能被磷酸化，也是丝氨酸蛋白酶活性中心里那个亲核基团。" } },
    { key: "thr", cn: "苏氨酸", en: "Threonine",     tlc: "Thr", olc: "T", cls: "polar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "O", x: 1.73, y: 2.0, h: 1 }, { el: "C", x: 0.00, y: 2.0 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[5,7]] },
      smiles: "C[C@@H](O)[C@@H](N)C(=O)O", formula: "C4H9NO3",   cid: 6288, q7: 0, essential: true },
    { key: "cys", cn: "半胱氨酸", en: "Cysteine",    tlc: "Cys", olc: "C", cls: "polar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "S", x: 1.73, y: 2.0, h: 1 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6]] },
      smiles: "N[C@@H](CS)C(=O)O",        formula: "C3H7NO2S",   cid: 5862, q7: 0,
    ...
    { key: "asn", cn: "天冬酰胺", en: "Asparagine",  tlc: "Asn", olc: "N", cls: "polar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "O", x: 1.73, y: 3.0 }, { el: "N", x: 2.60, y: 1.5, h: 2 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7,2],[6,8]] },
      smiles: "NC(=O)C[C@@H](N)C(=O)O",   formula: "C4H8N2O3",   cid: 6267, q7: 0,
      note: { cn: "N-连接糖基化就接在它的酰胺氮上（见 2-2-5）。" } },
    { key: "gln", cn: "谷氨酰胺", en: "Glutamine",   tlc: "Gln", olc: "Q", cls: "polar",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "C", x: 1.73, y: 3.0 }, { el: "O", x: 1.73, y: 4.0 }, { el: "N", x: 2.60, y: 2.5, h: 2 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7],[7,8,2],[7,9]] },
      smiles: "NC(=O)CC[C@@H](N)C(=O)O",  formula: "C5H10N2O3",  cid: 5961, q7: 0,
    ...
    { key: "asp", cn: "天冬氨酸", en: "Aspartate",   tlc: "Asp", olc: "D", cls: "acidic",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "O", x: 1.73, y: 3.0 }, { el: "O", x: 2.60, y: 1.5, h: 1 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7,2],[6,8]] },
      smiles: "N[C@@H](CC(=O)O)C(=O)O",   formula: "C4H7NO4",    cid: 5960, q7: -1, sc7: { at: 8, q: -1 },
      note: { cn: "嘌呤环的第四个氮来自它（见 4-1-1-1）；糖异生绕过丙酮酸那一步也经过它的产物草酰乙酸。" } },
    { key: "glu", cn: "谷氨酸", en: "Glutamate",     tlc: "Glu", olc: "E", cls: "acidic",
      mol: { atoms: [{ el: "N", x: 0, y: 0, h: 2 }, { el: "C", x: 0.87, y: 0.5 }, { el: "C", x: 1.73, y: 0 }, { el: "O", x: 1.73, y: -1 }, { el: "O", x: 2.60, y: 0.5, h: 1 }, { el: "C", x: 0.87, y: 1.5 }, { el: "C", x: 1.73, y: 2.0 }, { el: "C", x: 1.73, y: 3.0 }, { el: "O", x: 1.73, y: 4.0 }, { el: "O", x: 2.60, y: 2.5, h: 1 }],
             bonds: [[0,1],[1,2],[2,3,2],[2,4],[1,5],[5,6],[6,7],[7,8,2],[7,9]] },
      smiles: "N[C@@H](CCC(=O)O)C(=O)O",  formula: "C5H9NO4",    cid: 33032, q7: -1, sc7: { at: 9, q: -1 },
    ...
    { key: "lys", cn: "赖氨酸", en: "Lysine",        tlc: "Lys", olc: "K", cls: "basic",
      ...
      smiles: "NCCCC[C@@H](N)C(=O)O",     formula: "C6H14N2O2",  cid: 5962, q7: 1, sc7: { at: 9, q: 1 }, essential: true },
    { key: "arg", cn: "精氨酸", en: "Arginine",      tlc: "Arg", olc: "R", cls: "basic",
      ...
      smiles: "NC(=N)NCCC[C@@H](N)C(=O)O", formula: "C6H14N4O2", cid: 6322, q7: 1, sc7: { at: 10, q: 1 },
    ...
    { key: "his", cn: "组氨酸", en: "Histidine",     tlc: "His", olc: "H", cls: "basic",
      ...
      smiles: "N[C@@H](Cc1c[nH]cn1)C(=O)O", formula: "C6H9N3O2", cid: 6274, q7: 0, essential: true,
