...
>
> **2026-09-19 更新：这一份不再是存档。**
> 这七道题**现在都有能说出口的短答案了**（在 `app/data/_ans_basic_*.js` 和 `_ans_micro_*.js` 里，
> 按题面原文可以查到）。而 PESB 那门课的卡也开工了：`pe-expr` 表达、`pe-pur` 纯化。
> **这份表的用处变了**——它告诉你这七道题在生化的题目单上，但实质是 protein engineering，
> 所以写 PESB 卡的时候要跟它们对上，别让同一道题在两边出现两套说法。
> 对应关系：表达流程和表达菌株 → `pe-expr`；加标签和 His-tag → `pe-pur`；
> 聚合酶保真 → 卡 `taq`；多顺反子 mRNA → 卡 `12` 和 `x-trans`。
> 不做新材料的日期是 **2026-09-26**，不是 9/21。
