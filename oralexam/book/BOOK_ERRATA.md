...
## 10 · p094 说遗传密码「对所有生物通用」，而 p093 表 4.3 自己的脚注已经排除了 UGA

| | |
|---|---|
| 矛盾在哪 | p094 正文写密码**对所有生物通用**；**同一张表**（表 4.3，p093）的脚注就写着 UGA 在某些情况下编码硒代半胱氨酸。 |
| 怎么用 | 别替书打圆场说「近乎通用」。**把两句并排念出来**，让它们自己撞：「你这本书 p094 说通用，可 p093 表 4.3 的脚注自己就给了一个例外。」 |
| 怎么发现的 | 写横向卡 x-trans（转录翻译）时，逐行核表 4.3 发现的。 |

---

## 11 · 同一条链，书用了三个名字，其中一个听起来像反义

| | |
|---|---|
| 三个名字 | **p088** 叫 codogenic、**p090** 叫 template、**图 4.30** 标 antisense——**指的是同一条链**。 |
| 陷阱在哪 | **codogenic 听起来像 coding，而 coding 是另一条。** 她考场上很可能说反。 |
| 视觉证据 | **图 4.35**：mRNA 跟**上面**那条链一样，所以被读的是**下面**那条。 |
| 怎么用 | 三个页码互相佐证，不用搬别的教材。说「你这本书三处用了三个词指同一条链，图 4.35 能看出来读的是哪条」。 |

---

## 12 · p200 说简单扩散「没有膜蛋白参与」，同一句里却列出了由整合蛋白构成的通道

| | |
|---|---|
| 矛盾在哪 | p200 §8.5.3 说简单扩散**完全没有膜蛋白参与**，同一处却把整合蛋白构成的通道算进去；p201 又把离子通道放在**易化**扩散底下。 |
| 怎么用 | **不按结构分，按行为分**：有没有特异性、会不会饱和、能不能被抑制。书自己也说易化扩散「表现得像酶，可以用酶动力学描述」（p201）——那正是判据。 |
| 第二处 | p197 说「**只有**线粒体内膜负责产 ATP」，而 p208 自己就有类囊体的 ATP 合成酶。「只有」又一次是邀请函。 |
| 怎么发现的 | 写横向卡 x-mem（区室与膜）时发现的。 |
