...
{r:'高', q:'Why does an amino acid need a transfer RNA at all?',
    ez:'“Because an amino acid has no affinity for a codon. Three bases and a side chain have no chemistry in common. So there is nothing for the codon to grip. The cell puts an adaptor in between. It has an anticodon at one end. It has the amino acid at the other. So the recognition step becomes R N A pairing with R N A. The textbook makes the consequence explicit. From the formation of the aminoacyl transfer R N A onwards, each amino acid is recognised by the anticodon. It is not recognised by its own chemical composition.”',
