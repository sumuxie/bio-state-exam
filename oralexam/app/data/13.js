...
{r:'中', q:'How was it shown that a protein folds by itself, and that its disulfide bonds re-form correctly?',
    ez:'“The experiment used ribonuclease A. That enzyme has four disulfide bonds. Anfinsen denatured it with urea. He also reduced the four bonds with mercaptoethanol. The chain became an inactive random coil. Then he removed both reagents. The chain refolded on its own. Full activity came back. The same four disulfide bonds had re-formed. Many other pairings were possible. Only the native set appeared. So the sequence carries the folding information. Nothing outside the chain was needed. The disulfide bonds follow the fold. They do not create it.”',
    ezcn:'<b>实验用的是<b>核糖核酸酶 A</b>，它有四对二硫键。</b><b>Anfinsen 用尿素让它变性，同时用巯基乙醇把四对二硫键还原掉——链变成没有活性的无规卷曲。</b><b>然后把两种试剂都去掉：链自己重新折叠，<b>活性完全恢复，而且重新形成的正是原来那四对二硫键</b>。</b>⚠ <b>要紧的是分母：可能的配对方式有很多种，出现的却只有天然那一组。</b><b>结论：<b>折叠所需的信息全写在序列里</b>，链之外什么都不需要。</b>⚠ <b>所以二硫键是<b>跟着折叠走的</b>，不是它造出折叠。</b>',
 en:'“The experiment that settled it used ribonuclease A, an enzyme with four disulfide bonds. Anfinsen denatured the protein with urea and at the same time reduced all four bonds with mercaptoethanol, which left an inactive random coil. Removing both reagents let the chain refold on its own, and full enzymatic activity came back with the same four disulfide bonds re-formed. The number of other pairings that were chemically possible is large, and only the native set appeared. The conclusion is that the amino acid sequence carries all the information needed for folding, and that nothing outside the chain is required. So the disulfide bonds follow the fold rather than creating it. The work was done at the end of the nineteen fifties and into the sixties, and it is the origin of what is usually called the thermodynamic hypothesis.”',
 warn:'⚠ <b>别把年份说死</b>：Anfinsen 这批工作跨了好几年，诺奖是 1972 年。说 “at the end of the nineteen fifties” 就够，<b>具体年份待核</b>。',
 cn:'<b>通用知识，标 general。</b>⚠ <b>一个漂亮的书内接口</b>：书的年表 <span class="pg">p009</span> 里 <b>1969 年「第一次合成酶（RNase）」（Merrifield、Denkenwalter）</b>——<b>跟 Anfinsen 用的是同一个蛋白</b>。被问到就说这一句，等于把通用故事挂回了书上。<b>接口</b>：这一条也是 <b>land</b>（折叠地形）那张卡的起点。'},
{r:'中', q:'How was it discovered where glycosylation happens?',
    ez:'“The answer came from the signal hypothesis. A protein headed for secretion carries an N terminal signal sequence. That sequence takes the ribosome to the E R membrane. The chain is then pushed across as it is made. Glycosylation starts there. The evidence was a cell free experiment. You translate a secretory m R N A in a tube. You add microsomal membranes to one reaction. That product is processed. The signal is cut off. Sugar is attached. You leave the membranes out of the other reaction. That product is a larger unprocessed precursor. So the membrane is what makes the difference. Blobel proposed this. He got the Nobel Prize for it in nineteen ninety nine.”',
    ezcn:'<b>答案来自<b>信号假说</b>：要被分泌的蛋白带一段 N 端信号序列，这段序列把核糖体带到内质网膜上，肽链一边合成一边被推过膜——<b>糖基化就是在那里开始的</b>。</b>⚠ <b>证据是一个<b>无细胞</b>实验：在管子里翻译一条分泌蛋白的 mRNA。加了微粒体膜的那一管，产物被加工过——信号肽被切掉、糖被接上；不加膜的那一管，得到的是一个更大的、未加工的前体。</b><b>所以差别就是那张膜。</b><b>Blobel 提出的，1999 年诺贝尔奖。</b>',
 en:'“The answer came from the signal hypothesis. A protein destined for secretion carries an N-terminal signal sequence, and that sequence takes the ribosome to the membrane of the endoplasmic reticulum. The chain is then translocated across the membrane while it is still being made, and glycosylation begins there. The evidence was a cell-free experiment. A secretory messenger RNA is translated in a tube. One reaction gets microsomal membranes added and the other does not. The reaction with membranes gives a processed product, with the signal sequence cut off and sugar attached. The reaction without membranes gives a larger, unprocessed precursor. So the membrane is the whole difference. Blobel proposed the hypothesis and received the Nobel Prize for it in nineteen ninety-nine.”',
 cn:'<b>通用知识，标 general。</b>⚠ <b>三个接口一起说</b>：① <b>卡 08</b>「propeptide 与信号肽」讲的就是那段序列；② <b>pe-cfps</b> 那张卡用的正是同一个<b>无细胞翻译</b>体系；③ 这也解释了为什么<b>大肠杆菌做不了糖基化</b>——见 <b>pe-sys</b>「宿主由蛋白需要被做什么来定」。'},
 ]}
]
});
