...
{r:'高', q:'Is the isoelectric point the same kind of number as a pKa?',
    ez:'“No. A p K a belongs to one group. An isoelectric point belongs to the whole molecule. And it is derived from the p K a values. It is not measured as one of them. For a simple amino acid it is an average. It is the average of the two p K a values flanking the uncharged form. But it is not always the outer two. The rule is general. Write the charge ladder. Find the form with net charge zero. Average the two p K a values on either side of it. For an acidic amino acid like aspartate those two are both carboxyls. So the amino group never enters it. Aspartate is two point zero nine and three point eight six, so p I is two point nine eight. For a basic amino acid like lysine the two flanking values are both amino groups, the two highest ones. Lysine is eight point nine five and ten point five three, so p I is nine point seven four. Those side chain numbers are general values, not from my textbook.”',
    ezcn:'<b>不是同一类数。</b><b>pK<sub>a</sub> 属于某一个基团；等电点属于整个分子，而且它是从那些 pK<sub>a</sub> 推算出来的，不是直接测出来的某一个 pK<sub>a</sub></b>。<b>对一个简单氨基酸，它是「夹住不带电形式的那两个 pK<sub>a</sub>」的平均</b>。⚠ <b>但不总是取最外面那两个。铁律只有一条：先把电荷梯子写出来，找到净电荷为零的那一形态，再把<b>夹住它的那两个 pK<sub>a</sub></b>取平均。</b><b>酸性氨基酸（Asp、Glu）：夹住零的那两个都是<b>羧基</b>，也就是<b>最小的那两个</b> —— 氨基根本不参与。天冬氨酸 2.09 和 3.86，pI ＝ 2.98。</b><b>碱性氨基酸（Lys、Arg、His）：夹住零的那两个都是<b>氨基</b>，也就是<b>最大的那两个</b> —— 赖氨酸 8.95 和 10.53，pI ＝ 9.74。</b>⚠ <b>侧链那几个数是通用值，不是这本书印的，报的时候要说明。</b>',
 en:'“No. A pKa belongs to one group. An isoelectric point belongs to the whole molecule, and it is derived from the pKa values rather than measured as one of them. For a simple amino acid it is the average of the two pKa values that flank the uncharged form. And it is not always the average of the outer two. The rule is general: write the charge ladder, find the form with net charge zero, and average the two pKa values on either side of it. For an acidic amino acid such as aspartate both of those are carboxyls, so the amino group never enters it: 2.09 and 3.86 give a pI of 2.98. For a basic amino acid such as lysine both flanking values are amino groups, the two highest ones: 8.95 and 10.53 give a pI of 9.74. Those side-chain values are general numbers rather than ones my textbook prints.”',
 cn:'<b>「pI 是所有 pK 的平均」是最常见的错。</b>正确的做法是先排电荷序列、找到净电荷为零那一形态，再平均<b>夹住它</b>的那两个。<br><b>卡 <code>aa</code> 里有 Gly、Asp、Lys 三个完整算例」—— ⚠ <b>2026-09-26 查过：没有。</b>aa 卡上只有甘氨酸那一个，<b>2.09 / 3.86 / 2.98 / 8.95 / 10.53 / 9.74 全套卡 0 命中</b>，所以两个算例现在直接写进这一条。</b><br>书 p024 只印了 <code>pI = (pK₁ + pK₂)/2</code> 这一条，它讲的是甘氨酸那种两个基团的情形。<span class="pg">p024</span>'},
