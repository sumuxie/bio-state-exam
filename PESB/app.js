/* =========================================================================
   ...
   porting a fix between the two:
     ...
     * localStorage keys are prefixed "pesb." — both apps are served from the
       same github.io origin and therefore share one localStorage, so a
       shared prefix would let each app overwrite the other's progress.
   ========================================================================= */
...
(function () {
  ...
  const store = {
    get(key, fallback) {
      try {
        const raw = localStorage.getItem('pesb.' + key);
        return raw === null ? fallback : JSON.parse(raw);
      } catch (e) { return fallback; }
    },
    set(key, value) {
      try { localStorage.setItem('pesb.' + key, JSON.stringify(value)); } catch (e) { /* private mode */ }
    }
  };
  ...
  function renderStudy() {
      ...
      body.innerHTML = `
        <div class="welcome">
          ...
          <p class="welcome-note">🔊 A speaker icon plays browser-synthesised speech — not a recording — so you can
             check pronunciation. 🧬 <em>Bionic</em> in the top bar bolds the lead of each Latin-script word, a
    ...
    if (t.points && t.points.length) {
      ...
      t.points.forEach((p) => {
        html += `<li>${p.term ? `<span class="term-anchor">${esc(p.term)}</span>` : ''}${bi(p.en, p.cn)}</li>`;
      });
      html += `</ol></section>`;
    }
  ...
  function validateTopics() {
    ...
    TOPICS.forEach((t, i) => {
      ...
      quiz.forEach((q, j) => {
        if (!q.q_en || !q.q_cn) problems.push(`${where}: quiz ${j + 1} needs both q_en and q_cn`);
        if (q.type === 'mcq') {
          if (!Array.isArray(q.options) || q.options.length < 2) {
            problems.push(`${where}: quiz ${j + 1} mcq needs at least 2 options`);
          } else if (!Number.isInteger(q.answer) || q.answer < 0 || q.answer >= q.options.length) {
            problems.push(`${where}: quiz ${j + 1} answer index ${q.answer} is out of range`);
          }
        } else if (q.type === 'short') {
          if (!Array.isArray(q.accept) || !q.accept.length) problems.push(`${where}: quiz ${j + 1} short needs accept[]`);
          if (!q.answer_en || !q.answer_cn) problems.push(`${where}: quiz ${j + 1} needs answer_en and answer_cn`);
        } else {
          problems.push(`${where}: quiz ${j + 1} has unknown type "${q.type}"`);
        }
      });
