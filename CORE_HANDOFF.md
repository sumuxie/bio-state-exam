...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **内容 30 / 117 section**；**层：`trace` 27 / 30** |
...
| **`trace`** | **27 / 30** | **唯一还开着的一层**，2026-08-09 开工，一批 3 个（已做九批，**还剩最后 3 个**） |
| 行内引用进检查协议 | **26 / 30** | 差 `L-9-1-1`/`L-9-2-1`/`L-22-2-1`/`L-24-2-1` 四个 |
...
- **CORE2**：**内容这条轴上，她点名要的已经零欠账**；剩下的是「一笔带过」19 个和 depth queue
  的长尾。**真正还开着的是「层」那条轴**：`trace` 27/30（**推荐的下一步，再一批就收官**），以及最早四个
  Lehninger-only 节点的引用**至今不在检查器的 625 条里**。
...
**如果她没有新指示，建议接着做 `trace`**（CORE2 的层，27/30，最后一批 3 个），
或者 CORE3 剩下那 15 个只能手画的结构。⚠️ **旧版本这里写过「画了她看不见 = 已完成的工作正在白费」——那是说过头了，
...
| 想做哪条 | 填这一行 |
|---|---|
| **CORE2 的层（推荐）** | `继续补 trace，现在 27/30，最后一批是 L-3-4-1 / L-22-2-1 / L-22-3-1。一批 3 个，一个节点一张卡。做法见 CORE2「写 Lehninger trace 的做法」：先 grep 该节点自己的正文，句子从那里拉出来，numbers 只填已过检的数。` |
| CORE2 的层（补债） | `给 L-9-1-1 / L-9-2-1 / L-22-2-1 / L-24-2-1 补行内 (A p.N) 引用，让它们进入 verify_citations 的协议——现在 26/30 个节点在协议里。` |
...
实测（2026-08-10，trace 第七批之后）：parse 全部 **40** 个数据文件通过；
`step5_check` exit 0，240 topics；`verify_citations` **625 ok / 0 elsewhere / 0 unchecked**
