...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **内容 47 / 117 section**（31 深 + 16 浅）；**层：深度节点全满**；**Lehninger-only 清单已清空** |
...
**内容（新节点）：117 个 section 做了 47 个**，2026-08-09 用两份 tsv 逐个核对过，2026-08-10 又加了 11 个浅节点。
...
**唯一还在动的内容线是 depth queue 的 65 个**（**按 ratio ≥1.5 真正值得做的约 38 个**）。

**层（在已有 47 个 Lehninger 节点上）：**

| 层 | 进度 | 说明 |
|---|---|---|
| `mustKnow` | **47 / 47** ✅ | 全项目 257/257，零缺口。**浅节点也有**——她要的就是这个 |
| `points`/`terms`/`quiz`/`oral` | **31 / 31 深度节点** ✅ | ⚠️ 另有 **16 个浅节点有意不带**，不是缺口，见下 |
| **`trace`** | **31 / 31 深度节点** ✅ | 十批收官（2026-08-09→10）。⚠️ **16 个浅节点有意不带**。全项目 65 张卡 / 64 个节点 |
| 行内引用进检查协议 | **44 / 47** | 仍差 `L-9-1-1`/`L-9-2-1`/`L-22-2-1` 三个；**新写的节点一律从一开始就带引用**，浅节点也带（各 1 条，全 OK）。全项目 **660 条，0 ELSEWHERE，0 UNCHECKED** |
...
- **CORE2**：⚠️ **内容这条轴上，「她点名要的」和「一笔带过」两桶现在都是零欠账**（2026-08-10
  收官）。**Lehninger-only 32 个里写了 27 个，剩下 5 个全是刻意不写的**，见上面那段。
  **剩下的只有 depth queue 的长尾**（65 个，ratio ≥1.5 的约 38 个），而 CORE2 自己写着
  **「ratio 不是重要性评分」、这不是待办清单**——所以**这条线接下来做什么，需要 Ruojin 拍板**。
  ⚠️ **「层」那条轴也基本关掉了**：`trace` 已 **31/31 深度节点收官**，`mustKnow` 47/47 满，
  `points`/`terms`/`quiz`/`oral` 31/31 满。**层里唯一还欠的是行内引用 44/47**——还有三个
  Lehninger-only 节点（`L-9-1-1`/`L-9-2-1`/`L-22-2-1`）的引用**不在检查器的 660 条里**。
...
**`trace` 已经收官，「一笔带过」也已经收官，所以那两条建议都用完了。** 如果她没有新指示，现在成本最低的是
**给剩下三个节点补行内引用**（CORE2 的层，44/47 → 47/47，照抄 `L-24-2-1`）；工作量大一些但确定的是
...
| 想做哪条 | 填这一行 |
|---|---|
| **CORE2 的层（推荐，成本最低）** | `给 L-9-1-1 / L-9-2-1 / L-22-2-1 三个节点补行内引用，让它们进入 verify_citations 的检查协议（现在 44/47）。做法照抄 L-24-2-1：用检查器自己的页索引 doc[printed + 36 - 1] 把每句话在 A 上搜出来、拿它返回的页码，绝不要从 A↔B 映射推；引语用弯引号，长度控制在 18–140 字符之间。补完 verify_citations 应为 0 UNCHECKED。` |
| ~~CORE2 内容：一笔带过~~ | ✅ **已于 2026-08-10 收官，17 个全部写完，另 2 个（§12.2/§12.4）实测 Czech 已覆盖故意不写。这一桶不要再看了。** |
| CORE2 内容（需拍板） | `depth queue 还剩 65 个，但 CORE2 自己写着「ratio 不是重要性评分、这不是待办清单」，已经有一个 session 因为把 top-20 当队列交出去而让 Ruojin 误以为欠了 65 笔账。所以动手之前先问她要哪一个主题，不要自己按 ratio 排。` |
...
⚠️ **上一个 session 停在哪**（2026-08-10，第二次）：做的是**内容线**，**「一笔带过」这一桶已经清空**——
...
**唯一还没做完的仍然是**：`L-22-2-1`/`L-9-2-1`/`L-9-1-1` 三个节点的**行内引用**（`L-24-2-1` 已补完可照抄）。
