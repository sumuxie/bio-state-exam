...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **内容 31 / 117 section**；**层：`trace` 31 / 31 ✅** |
...
**内容（新节点）：117 个 section 做了 31 个**，2026-08-09 用两份 tsv 逐个核对过。
...
| `lehninger_only_scope.tsv` | 32 | **11** | = 她圈定的「主要需要的」7 个 + merge 桶实测的 3 个真缺口 + 「一笔带过」的 §12.1 |
...
剩下的 86 个都是**她说过优先级更低的**：Lehninger-only 还剩 21 个（其中 **18 个是「一笔带过
...
| 层 | 进度 | 说明 |
|---|---|---|
| `mustKnow` | **31 / 31** ✅ | 全项目 241/241，零缺口 |
| `points`/`terms`/`quiz`/`oral` | **31 / 31** ✅ | 每个节点都满 |
| **`trace`** | **31 / 31** ✅ | 十批收官（2026-08-09→10）；此后**新节点自带 trace**，`L-12-1-1` 即如此。全项目 65 张卡 / 64 个节点 |
| 行内引用进检查协议 | **28 / 31** | 仍差 `L-9-1-1`/`L-9-2-1`/`L-22-2-1` 三个；**新写的节点从一开始就带引用**（`L-12-1-1` 7 条全 OK）|
...
`step5_check` exit 0，241 topics；`verify_citations` **643 ok / 0 elsewhere / 0 unchecked**
