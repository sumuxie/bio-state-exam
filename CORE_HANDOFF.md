...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **内容 30 / 117 section**；**层：`trace` 12 / 30** |
...
| **`trace`** | **12 / 30** | **唯一还开着的一层**，2026-08-09 开工，一批 3 个（已做四批） |
...
- **CORE2**：**内容这条轴上，她点名要的已经零欠账**；剩下的是「一笔带过」19 个和 depth queue
  的长尾。**真正还开着的是「层」那条轴**：`trace` 12/30（**推荐的下一步**），以及最早四个
...
**如果她没有新指示，建议接着做 `trace`**（CORE2 的层，12/30，一批 3 个），
...
| 想做哪条 | 填这一行 |
|---|---|
| **CORE2 的层（推荐）** | `继续补 trace，现在 12/30。一批 3 个，一个节点一张卡。做法见 CORE2「写 Lehninger trace 的做法」：先 grep 该节点自己的正文，句子从那里拉出来，numbers 只填已过检的数。` |
