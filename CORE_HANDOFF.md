...
- **CORE2**：**内容这条轴上，她点名要的已经零欠账**；剩下的是「一笔带过」19 个和 depth queue
  ...
  `points`/`terms`/`quiz`/`oral` 本来就满。**层里唯一还欠的是行内引用 26/30**——最早四个
  Lehninger-only 节点（`L-9-1-1`/`L-9-2-1`/`L-22-2-1`/`L-24-2-1`）的引用**至今不在检查器的
  625 条里**，**这是现在成本最低、收益最明确的一件事**。
...
**给那四个节点补行内引用**（CORE2 的层，26/30 → 30/30）；工作量大一些但确定的是
...
| 想做哪条 | 填这一行 |
|---|---|
| **CORE2 的层（推荐）** | `trace 已 30/30 收官，不用再补。改做「层」里唯一还欠的一项：给 L-9-1-1 / L-9-2-1 / L-22-2-1 / L-24-2-1 补行内 (A p.N) 引用，让它们进入 verify_citations 的协议——现在 26/30 个节点在协议里。` |
