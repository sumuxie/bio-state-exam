...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **内容 48 / 117 section**（32 深 + 16 浅）；**层：深度节点全满**；**Lehninger-only 清单已清空**；**depth queue 开工（Ruojin 选了光合作用组）** |
...
**内容（新节点）：117 个 section 做了 48 个**，2026-08-09 用两份 tsv 逐个核对过，2026-08-10 又加了 11 个浅节点，2026-08-10 晚开工 depth queue 的光合作用组、第一个是 `L-20-6-1`。
⚠️ **上面那句话的措辞不要改**——`audit_handoff.py` 用正则 `117 个 section 做了 (\d+) 个`
从这里读实时计数并与数据库比对；改写这一段时它当场报了 `COUNT MISSING`，**说明它在正常工作**，别把它绕过去。

| 来源清单 | 总数 | 已做 | 说明 |
|---|---|---|---|
| `depth_queue.tsv` | 85 | **21** | 原本正好是 rank 第 1–20；2026-08-10 加了 §20.6（rank 39），**是 Ruojin 亲自选的主题，不是按 ratio 取的** |
...
**唯一还在动的内容线是 depth queue**，做掉 §20.6 之后还剩 **64 个**。
...
合计 65），**但 13+25+12 = 50，不是 38**。所以 ratio ≥1.5 的池子是 **50 个**（做掉 §20.6 后 49 个）。
**这个数字本身没什么用**——见下面「ratio 不是重要性评分」那一条，**它只是不该是错的**。

**层（在已有 48 个 Lehninger 节点上）：**

| 层 | 进度 | 说明 |
|---|---|---|
| `mustKnow` | **48 / 48** ✅ | 全项目 258/258，零缺口。**浅节点也有**——她要的就是这个 |
| `points`/`terms`/`quiz`/`oral` | **32 / 32 深度节点** ✅ | ⚠️ 另有 **16 个浅节点有意不带**，不是缺口，见下 |
| **`trace`** | **32 / 32 深度节点** ✅ | 十批收官（2026-08-09→10），`L-20-6-1` 自带第 32 张。全项目 66 张卡 / 65 个节点 |
| 行内引用进检查协议 | **48 / 48** ✅ | **2026-08-10 收官**，最后三个 `L-9-1-1`/`L-9-2-1`/`L-22-2-1` 已补完；`L-20-6-1` 写的时候就在协议内。全项目 **726 条，0 ELSEWHERE，0 UNCHECKED** |
...
- **CORE2**：⚠️ **内容这条轴上，「她点名要的」和「一笔带过」两桶都是零欠账**（2026-08-10
  ...
  **做完之后要再问一次**。
  ⚠️ **「层」那条轴是关着的**：`trace` **32/32 深度节点**、`mustKnow` **48/48**、
  `points`/`terms`/`quiz`/`oral` **32/32**、**行内引用 48/48**（全项目 **726 条 / 0 ELSEWHERE / 0 UNCHECKED**）。
...
⚠️ **`trace`、「一笔带过」、行内引用三条建议全部用完了。** CORE2 现在唯一在动的是
**Ruojin 指定的光合作用组**（§20.2–20.6，已完成 §20.6）。那一组做完之后如果她没有新指示，
...
| **CORE2 内容：光合作用组（已拍板，进行中）** | `Ruojin 2026-08-10 选定 depth queue 的光合作用组 §20.2-20.6。§20.6 已完成（L-20-6-1）。接着做 §20.2、§20.4、§20.3、§20.5，每一节的写作范围都已实测并写在「上一个 session 停在哪」那一段里——照那个范围写，不要重新判断捷克层的覆盖。` |
| CORE2 内容：光合作用组做完之后（需再拍板） | `depth queue 还剩 64 个，但 CORE2 自己写着「ratio 不是重要性评分、这不是待办清单」，已经有一个 session 因为把 top-20 当队列交出去而让 Ruojin 误以为欠了 65 笔账。所以动手之前先问她要哪一个主题，不要自己按 ratio 排。` |
...
⚠️ **上一个 session 停在哪**（2026-08-10，第三次）：**Ruojin 亲自选了 depth queue 里的「光合作用组」
（§20.2–20.6 五节）**，理由是她自己说过的那句话——**「中国是不考光合作用的，但我在捷克参加国考的同学被考了
光合作用」**——这是整个队列里**唯一有真实考题证据**的一组。**已完成 `L-20-6-1`（§20.6），已 commit 并 push。**
**剩下四节按这个顺序做，每一节的范围都已经实测过、写在下面，不要重新判断覆盖：**
