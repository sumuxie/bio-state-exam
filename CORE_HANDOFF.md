...
| 想做哪条 | 填这一行 |
|---|---|
| **CORE2 的层（推荐）** | `继续给 L-22-2-1 补行内 (A p.N) 引用，然后 L-9-2-1、L-9-1-1。现在 27/30 在协议里。做法照抄 L-24-2-1（已补完，20 条引语→11 行全 OK）：先用检查器自己的页索引 doc[printed+36-1] 把每句话在 A 上搜出来、取它返回的页码，绝不从 A↔B 映射推；引语用弯引号「“…”」不用转义。§22.2 = A pp.805-816，边界已实测过，节点声明的范围这次是对的。` |
...
⚠️ **上一个 session 停在哪**（2026-08-10）：`L-24-2-1` 已补完并推送（`7b1239d`）；`L-22-2-1` **只做到「在 A 上把句子搜出来」，一个字都没写进数据文件**，工作树是干净的。重做那一步只要几分钟，照上面那行的做法即可。
