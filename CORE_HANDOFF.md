...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **内容 36 / 117 section**（31 深 + 5 浅）；**层：深度节点全满** |
...
**内容（新节点）：117 个 section 做了 36 个**，2026-08-09 用两份 tsv 逐个核对过。
...
| `lehninger_only_scope.tsv` | 32 | **16** | = 她圈定的 7 个 + merge 桶 3 个 + 「一笔带过」的 ch12 六节（§12.1 深、§12.3/12.5/12.6/12.8/12.9 浅）|
...
剩下的 81 个都是**她说过优先级更低的**：Lehninger-only 还剩 16 个（其中 **13 个是「一笔带过
...
| 层 | 进度 | 说明 |
|---|---|---|
| `mustKnow` | **36 / 36** ✅ | 全项目 246/246，零缺口。**浅节点也有**——她要的就是这个 |
| `points`/`terms`/`quiz`/`oral` | **31 / 31 深度节点** ✅ | ⚠️ 另有 **5 个浅节点有意不带**，不是缺口，见下 |
| **`trace`** | **31 / 31 深度节点** ✅ | 十批收官（2026-08-09→10）。⚠️ **5 个浅节点有意不带**。全项目 65 张卡 / 64 个节点 |
| 行内引用进检查协议 | **33 / 36** | 仍差 `L-9-1-1`/`L-9-2-1`/`L-22-2-1` 三个；**新写的节点一律从一开始就带引用**，浅节点也带（各 1 条，全 OK）|
...
**不要「后来把它们补全」**——浅是规格本身。目前 5 个：`L-12-3-1` `L-12-5-1` `L-12-6-1`
`L-12-8-1` `L-12-9-1`。⚠️ **`L-12-1-1` 是例外**：它写在她说这句话**之前**，所以带全套层，
**那是历史遗留，不是要照抄的模板**。
...
| 想做哪条 | 填这一行 |
|---|---|
| **CORE2 内容（推荐）** | `继续写「一笔带过」的浅节点。还剩 11 个：§2.1 §2.3 §4.5 §5.2 §5.3 §8.4 §9.3 §23.4 §23.5 §26.3 §26.4（§12.2/§12.4 实测 Czech 已覆盖，不要写）。写法照抄 leh_ch12b.js：只带 summary + mustKnow，coverage 用 partial，每个节点配 1 条已在 A 上搜过的行内引用。动手前先按词边界 grep Czech 层。` |
...
⚠️ **上一个 session 停在哪**（2026-08-10）：做的是**内容线**，ch12 已清空——`L-12-1-1` 深节点 + `L-12-3-1`/`L-12-5-1`/`L-12-6-1`/`L-12-8-1`/`L-12-9-1` 五个**浅节点**，全部已推送（`9b00d8f`），工作树干净。⚠️ **`§12.2` 和 `§12.4` 在 tsv 里仍标着 `brief` 未做，但那是故意不写的**——实测 Czech `10-18`/`10-19`/`10-20` 已覆盖，**别再去写它们**。
另有一件**没做完的**：`L-22-2-1`/`L-9-2-1`/`L-9-1-1` 三个节点的**行内引用**还没补（`L-24-2-1` 已补完可照抄）。
