...
| 结构式 | `CORE3_STRUCTURES.md` | **已画 25 / 清单 205**（清单 2026-08-09 写出来了，带 grep 证据，待 Ruojin 删减） |
...
- **CORE3**：**分母已经有了**——`biochemie_struct/STRUCTURE_LIST.md`，205 行，每行带两个
  layer 的 grep 命中数，可由 `tools/build_structure_list.py` 重新生成。**现在卡在一件只需要
  她读、不需要写代码的事上：过一遍清单删掉不考的**，分母才算定稿。入口那件事（从 basic 走不到
  结构式）是**她按习惯选**，不阻塞画图。
