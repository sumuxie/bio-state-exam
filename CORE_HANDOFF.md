...
| Lehninger 额外内容 | `CORE2_LEHNINGER.md` | **117 个 section 做了 23 个**（§9.1/§9.2/§22.2 刚补上，前 20 个做偏了） |
...
4. **一批 3 个。** 整章一次写完会撞输出上限，留下半截 JavaScript；这台机器上没有 JS runtime，
   坏掉的表现只是浏览器一片空白。**这条规矩不只适用于「一整章」——`L-9-2-1` 一个节点自己就在
   `points` 数组中间被截断过一次**（16 条 point 加 15 个 term 加 quiz/oral 一次性生成，超出了
   单次输出上限），事后靠续写加 esprima parse check 才发现并补完。写完任何大节点后先 parse
   check，不要假设 Write 工具没报错就等于内容完整。
