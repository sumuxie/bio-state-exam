...

{
  ...
  points: [
    ...
    { cz: "fáze II — společná dráha ke acetylkoenzymu A", en: "PHASE II funnels the building blocks toward a single hub, ACETYL-CoA. Hexoses/pentoses pass through the shared sequence glyceraldehyde-3-phosphate ⇌ phosphoenolpyruvate ⇌ pyruvate; the diagram shows glycerol joining this route near glyceraldehyde-3-phosphate, while fatty acids (and, per the diagram's separate arrows, part of the amino acid pool) bypass more directly toward the acetyl-CoA/pyruvate end of the route. NOTABLY, the final arrow of this phase, pyruvate→acetyl-CoA, is drawn as ONE-WAY ONLY, unlike every other double-headed arrow in the figure — a diagram-level signal of this step's physiological irreversibility (pyruvate dehydrogenase), consistent with the standard fact that fatty-acid-derived acetyl-CoA cannot be net-converted back to glucose.", cn: "第Ⅱ阶段将各构件单位汇集到一个共同枢纽——乙酰辅酶A。己糖/戊糖经共同的序列甘油醛-3-磷酸⇌磷酸烯醇丙酮酸⇌丙酮酸；图中显示甘油在靠近甘油醛-3-磷酸处汇入这条路线，而脂肪酸（以及图中另有箭头显示的部分氨基酸库）则更直接地绕道汇入丙酮酸/乙酰辅酶A一端。值得注意的是，这一阶段的最后一步——丙酮酸→乙酰辅酶A——图中画成了唯一的单向箭头，不同于图中其余所有双向箭头——这在图示层面提示了该步骤在生理上的不可逆性（丙酮酸脱氢酶），与「脂肪酸来源的乙酰辅酶A无法净转化回葡萄糖」这一标准事实相符。" },
  ...
  quiz: [
    ...
    {
      type: "mcq",
      q_en: "In Obr. 6.2, every arrow between boxes is drawn double-headed (reversible) EXCEPT one. Which step is drawn as a one-way arrow only, and what does that most likely signal?",
      q_cn: "在图6.2中，各方框之间的箭头都画成双向（可逆）的，唯有一步例外。是哪一步被画成单向箭头？这最可能提示什么？",
      options: [
        "Citrate → isocitrate, most likely signalling that the citrate cycle only runs once per day",
        "Hexoses/pentoses → glyceraldehyde-3-phosphate, signalling that sugars can never be resynthesised from this intermediate",
        "Pyruvate → acetyl-CoA, most likely signalling this step's physiological irreversibility",
        "Amino acids → proteins, signalling that protein synthesis never occurs in this pathway"
      ],
      answer: 2,
      why_en: "The single one-way arrow in Obr. 6.2 is pyruvate→acetyl-CoA, consistent with the physiological irreversibility of this step (catalysed by pyruvate dehydrogenase) — the standard biochemical basis for why net fatty-acid carbon cannot be converted back to glucose. The other three options describe arrows that ARE drawn double-headed in the diagram.",
      why_cn: "图6.2中唯一的单向箭头是丙酮酸→乙酰辅酶A，这与该步骤（由丙酮酸脱氢酶催化）在生理上的不可逆性相符——这也是「脂肪酸碳骨架无法净转化回葡萄糖」这一标准生化事实的基础。其余三个选项所描述的箭头在图中实际上都是双向的。"
    },
    {
      type: "short",
  ...
  oral: {
    ...
    checklist: [
      ...
      "Noted the pyruvate→acetyl-CoA arrow is uniquely one-way and why that matters physiologically",
